...
//console.log("initial editableOptions: ",editableOptions);
			...
			if ((editableOptions.type === 'select' || editableOptions.type === 'select2' 
            || editableOptions.type === 'checklist'|| editableOptions.type === 'typeahead') 
          && !editableOptions.source && editableOptions.options) {
      ...
      //console.log("final editableOptions: ",editableOptions);
			//create editable
      ...
      $editable.on('shown.ko', function(event, editableInner){
        if(allBindings.typeaheadSelected){
          console.log("found typeahead selected function 42");
          editableInner.input.$input.on('typeahead:selected',function(el,datum){
            console.log("selection made",datum);
            allBindings.typeaheadSelected(datum,value,allBindings);
          });
        }
        //editableInner.input.$input.on('typeahead:closed',function(el,datum){
        //  console.log("typeahead closed",datum);
        //  editableInner.input.$input.typeahead("destroy");
        //});
...
//console.log("done with editable binding init");
		}
	};
});
