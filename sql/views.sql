...
CREATE OR REPLACE VIEW web.profile_search AS   
    SELECT user_key,email,first_name||' '||last_name as missionary_name,
           ...
           picture_url,mp.created_on,
           first_name||' '||last_name||' '||email||' '||org_name||' '||org_description||' '||coalesce(string_agg(catagory,'|'),'')||' '||
            location||' '||mp.description as search_text
