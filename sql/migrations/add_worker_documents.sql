...
-- Worker document uploads: one row per file per worker per period
CREATE TABLE IF NOT EXISTS web.worker_documents (
    ...
    UNIQUE (user_key, document_type, submission_period)
);
ALTER TABLE web.worker_documents OWNER TO ergatas_dev;
ALTER TABLE web.worker_documents ENABLE ROW LEVEL SECURITY;

-- Reminder tracking: one row per missed (user, doc_type, period), updated each month until resolved
...
CREATE INDEX idx_worker_documents_user_key ON web.worker_documents(user_key);
CREATE INDEX idx_worker_document_reminders_open
    ON web.worker_document_reminders(user_key)
