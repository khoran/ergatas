...
export function peopleGroupSearch(server,query,spinnerObs,limit=200){
    if(query !=  null && query.length >= 3 ){
        if(spinnerObs) spinnerObs(true)
        return server.postJson("/api/peopleGroupSearch",{
                query:query ,
                limit:  limit,
            }).fail( error =>{
                console.error("people group query failed: "+error.message,error);
            }).always(() =>{
...
export function peopleGroupNames(server,peopleID3Codes,spinnerObs){
    if(  Array.isArray(peopleID3Codes)){
        if(spinnerObs) spinnerObs(true);
        return server.postJson("/api/peopleGroupNames",{
                people_id3_codes: peopleID3Codes.map(x => parseInt(x))
            }).always(() =>{
                if(spinnerObs) spinnerObs(false)
            });
    }else
