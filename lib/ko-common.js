 ...
 ko.bindingHandlers.urlSuffix= {
  init: function(element, valueAccessor, allBindingsAccessor, viewModel, bindingContext) {
      ...
      $element.change(function(){
        ...
        if(urlBase[urlBase.length -1 ]=== '/' && inputVal[0] ==='/' ){
          inputVal = inputVal.replace("/",""); //remove leading slash
        }
        else if(urlBase[urlBase.length -1 ] !== '/' && inputVal[0] !== '/' ){
          inputVal = "/"+inputVal;
        }//else one of them has a slash, so we're good.
        value(urlBase+inputVal);
      });
      ...
      $(window).on("scroll", function() {
        ...
        if(((scrollHeight - 300) >= scrollPos) / scrollHeight == 0){
            if(value != null && window.ergatas_triggered === false){
              ...
              //console.log("scrollPos: "+scrollPos+", scrollHeight: "+scrollHeight);
              callback();
            }
        }else{
          window.ergatas_triggered=false;
