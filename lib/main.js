...
initViewModel(authBaseUrl,clientId,redirect,bucketBase,appBase){
    ...
    const viewModel = {
        ...
        filter: {
            ...
            kidsBirthYears: new Filter("birth_years",null)
        },
        pageParameters:{},
        playSample: function(){
    ...
    viewModel.birthYearsSelectizeOptions = {
        create: true,
        maxItems:100,
        plugins:['remove_button'],
        createFilter: /^\d{4}$/,
        onInitialize: function(){
            ...
            if(profile != null && profile.data.kids_birth_years() != null){
                profile.data.kids_birth_years().forEach(year => 
                    api.addOption({value: year, text: year }));
                ko.bindingHandlers.selectize.utils.setItems(api,profile.data.kids_birth_years);
            }
        }
    }
    ...
    viewModel.toggleAgeGroup= function(obs,start,end,event){
        console.log("toggling age group ",start,end);
        var year = new Date().getFullYear();
        var currentSet = obs() || []; //current array of years
        var givenYears =[...Array(end-start+1)].map((_, i) => year - (start + i) );
        var target = event.target;
        var added=false, removed=false;

        console.log(target);
        //toggle each year in currentSet
        givenYears.forEach( x =>{
            var index = currentSet.indexOf(x);
            if( index === -1){
                currentSet.push(x);
                added=true;
            }
        ...
        console.log("states: ",added,removed);

        if(added && removed){
            console.warn("toggleAgeGroup added and removed stuff, not supposed to happen",start,end,currentSet);
        }else if(added && target != null){

            console.log("checking group");
            target.checked=true;
        }
        else if(removed && target != null)
            target.checked = false;

        console.log("checked state: ",target.checked);
