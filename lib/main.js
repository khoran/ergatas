...
import {DataAccess, ILikeFilterAppender, OverlapsFilterAppender, LessThanFilterAppender, 
...
constructor(){
    ...
    this.da = new DataAccess(postgrestBase,jQuery.ajax);
    ...
    //TODO: do a test login here to see if  have a cookie that will keep us logged in
    // also, encrypt the cookies
 //   if(code != null && userState != null && userState==="Authenticated"){
        this.doLogin(state || window.location.pathname.replace(appBase,""),code);
  //  }else{
...
setupPWA(){
    window.addEventListener('beforeinstallprompt', function(evt){
        alertify.confirm("Install App","Would you like to install this site as an app?",
            function(){
                ...
                console.log("delclined to install app");
...
initViewModel(authBaseUrl,clientId,redirect,bucketBase,appBase){
    ...
    const viewModel = {
     ...
     //   }),
        ...
        filter: {
            query: new Filter("search_text",new ILikeFilterAppender()),
        ...
        signOut: async function(){
            await utils.postJson("/api/signOut");
        ...
        submitOrgApplication: async function(){
            ...
            const ein = viewModel.organizationApplication().ein();
            try{
                //TOOD: check to see if org exists and is denied, and inform user
                var status = await self.da.organizationStatus(ein);
                if(status != null && status.length > 0){
                    ...
                    if(status[0].status === "pending"){
                        //TODO:: add this user to a list to be notified of review result
                ...
                await utils.postJson("/api/orgAppNotify",{
    ...
    viewModel.donate = function(profile,source){
        ...
        var dialogContent = jQuery("<donate-popup params=\"donationUrl:'"+profile.data.donation_url+"',"+
                                "instructions: '"+ (profile.data.donate_instructions || "") +"',"+
    ...
    alertify.dialog("messagePopup",function(){},true,'prompt');
    viewModel.contact= function(profile){
            console.log(" 1 contacting profile: ",profile);
            var fromEmail="khoran512@gmail.com";
            var name="test user";
            var message="test message 1";
            var htmlSnippet = self.getSnippet("message-form").clone();
            var vm = {
                name: ko.observable(),
                email: ko.observable(),
            ...
            ko.applyBindings(vm,htmlSnippet[0]);

            alertify.messagePopup("").set({
                    transition:"zoom",
                    title: "Send a Message",
                    //buttonFocus: "cancel",
                    onfocus: function(){
                        console.log("message popup focused");
                        htmlSnippet.focus();
                    ...
                    onok:  async function(){
                        ...
                        try{
                            if(vm.email() != null && vm.name() != null && vm.message() != null && vm.message() != "")
                                await utils.postJson("/api/contact/setup",{
                                    fromEmail: vm.email(),
                                    name: vm.name(),
                                    message: vm.message(),
                            ...
                            console.log("failed to send contact message to server: ",error);
                            alertify.error("Failed to send message, terribly sorry about that!");
                        }
                    },
                    label: "Send",
                }).
                setContent(htmlSnippet[0]).
                resizeTo("30%","50%");
    ...
    viewModel.deleteAccount = function(){
        ...
        alertify.prompt("Confirm Deletion of Account","Are you sure you want to delete your account? This CANNOT be undone."+
                ...
                async function(evt,value){
                    ...
                    try{
                        await self.da.deleteUser(viewModel.loggedInUser().user_key());
                        await utils.postJson("/api/deleteUser",{token: viewModel.token()});
                        //log user out
                        viewModel.signOut();
                    }catch(error){
                        console.error("failed to delete user account for user_key "+viewModel.loggedInUser().user_key(),error);
    ...
    viewModel.showFilePicker = function(profile){
        console.log("showing robodog file picker",profile);
        ...
        uppy.on('transloadit:complete', (assembly) => {
            ...
            if(assembly.results && assembly.results.resize_image &&
                assembly.results.resize_image.length > 0 && assembly.results.resize_image[0].url != null){
                    //TODO: the name field may be modified to fit requirments by google.
                    // so extract the final name from the 'url' field.
                //var path = userId+"/"+assembly.results.resize_image[0].name;
...
/*
        var uppy = window.robodog.pick({
            ...
            params: {
              ...
              template_id: '32a3dd9ec4be4a269ef1d19edabfa886'
            },
            fields:{
                userId: viewModel.loggedInUser().external_user_id(),
            }
    ...
    viewModel.betaTesterLogin = function(){
        alertify.prompt("Beta Tester Login","Enter the password: ","",function(event,value){
            ...
            if(value === "theharvestisplentiful" ){
...
initRouter(appBase,authBaseUrl,authClientID){
    ...
    router.add('profile-post-sof/', async function () {
        ...
        if(viewModel.loggedIn()){
            ...
            signInWithRedirect("profile-post-sof");
    ...
    router.add('organization-review/(:num)', function (organization_key) {
        if(viewModel.loggedIn())
            ...
            signInWithRedirect("organization-review");
    });
    ...
    try{
        ...
        if(tokenResult.access_token != null){
            ...
            try{
                ...
                throw new AppError("failed to create new user with userId "+tokenResult.userId,error);
            }
        }else{
        ...
        }else{ //if no code given, then this is  test login 
            console.info("while testing for login cookie, failed to log in user. ");
            self.router.navigateTo(state);
        }
    }
}
