...
initViewModel(authBaseUrl,clientId,redirect,bucketBase,appBase){
    ...
    viewModel.setOrgStatus=async function(organization_key,status,message){
        try{
            await self.da.setOrganizationApprovalStatus(organization_key,status);
            await self.server.authPostJson("/api/notifyOrgUpdate",{organization_key:organization_key,message:message});
