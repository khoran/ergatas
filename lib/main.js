...
class Client {
    ...
    initViewModel(){
        ...
        const viewModel = {
            ...
            submitOrgApplication: async function(){
                ...
                try{
                    ...
                    self.postJson("/api/orgAppNotify",{profile_key:newOrg.missionary_profile_key});
                    self.router.navigateTo("");
                }catch(error){
                    if(error.status===409)
        ...
        viewModel.initProfileForm= function(){
            console.log("initProfileForm");
            var el = jQuery("#extra-file-uploads")[0];
            console.log("file pond el: ",el);
            ...
            pond.setOptions({
                ...
                imageResizeTargetWidth: self.RESIZE_WIDTH,
                instantUpload: false,
                oninitfile: file =>{
                    console.log("init file: ",file);
                },
                onaddfile: (error,file) =>{
                    console.log("on add file: ",error,file);
                },
                ...
                onremovefile: (error,file) =>{
                    console.log("on remove file: ",error,file);
                },
                server: {
                    process: async function (fieldName, file, metadata, load, error, progress, abort) {
                        var bucketKey = viewModel.loggedInUser().user_key()+"/"+file.name;

                        var signedUrl = await self.postJson("api/getSignedUrl",{filename:file.name,token:self.token});
...
/*
    ...
    async uploadProfilePicture(form){
        ...
        try{
            ...
            signedUrl = await self.postJson("api/getSignedUrl",{filename:file.name,token:self.token});
            signedUrl=signedUrl.url;
    ...
    async doLogin(code,state){
        ...
        try{
            ...
            if(tokenResult.access_token != null){
                self.token=tokenResult.access_token; 
