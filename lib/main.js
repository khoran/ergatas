...
initViewModel(authBaseUrl,clientId,redirect,bucketBase,appBase){
    ...
    const viewModel = {
        ...
        saveProfile: async function(form){
            ...
            try{
                ...
                if(error.status === 401){
                    alertify.error("Session expired, could not save your profile. Please login or re-login again.");
                }else
    ...
    viewModel.refreshJWT = function(){
        //TODO: check token expiration and only refresh when required
        jQuery.get("/api/refresh");
    }
