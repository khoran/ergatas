...
constructor(){
    ...
    if(code)
        ...
        this.doRefresh();
        if(state){
            //this.router.navigateTo(state,null,true);
            ////this.router.back();
            this.router.check();
        }
    }
    ...
    function promptUserToRefresh(reg) {
        ...
        self.viewModel.updateApp = function(){
            ...
            if(reg && reg.waiting && reg.waiting.postMessage)
                ...
                log.error("Failed to update at users request, reg.waiting not defined");
...
initViewModel(authBaseUrl,clientId,redirect,bucketBase,appBase){
    ...
    const viewModel = {
        ...
        orgApplicationStatus: ko.observable(),
        submitOrgApplication: async function(){
            ...
            try{
                ...
                var status = await self.da.unapprovedOrganizationStatus(country_org_id);
                if(status != null && status.length > 0){
                    if(status[0].status === "denied"){
                        //alertify.error("Sorry, this organization has been previously reviewed, but was not approved");
                        viewModel.orgApplicationStatus("denied");
                        return;
                    }
                    if(status[0].status === "pending"){
                        await self.da.insertOrganizationListener(status[0].organization_key,user_key);
                        //alertify.success("This organization has already been submitted and is pending review.");
                        viewModel.orgApplicationStatus("pending");
                        //self.router.navigateTo("");
                        return;
                    }
                }
                ...
                viewModel.orgApplicationStatus("submitted");
                //self.router.navigateTo("");
            }catch(error){
                if(error.status===409)
...
initRouter(appBase,authBaseUrl,authClientID){
    ...
    router.add('profile-detail/(:num)', async function (missionary_profile_key) {
        ...
        //if(viewModel.selectedProfile() == null){
            try{
                if(viewModel.approvedOrganizations() == null || viewModel.approvedOrganizations().length===0)
                    await self.loadApprovedOrganizations();
    ...
    router.add('org-application/', async function () {
        try{
            ...
            viewModel.orgApplicationStatus("");
