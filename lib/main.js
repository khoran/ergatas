...
constructor(){
    ...
    //this.pageSize = 40; 
    this.pageSize = utils.computePageSize();
...
initViewModel(authBaseUrl,clientId,redirect,bucketBase,appBase){
    ...
    const viewModel = {
        ...
        searchListView: function(){
            viewModel.search.filter.bounds.clear();
    ...
    viewModel.jobCatagoryArray = function(job_catagory_keys,asObject){
        return utils.selectedKeysAsArray(job_catagory_keys,viewModel.jobCatagories(),"job_catagory_key","catagory",asObject);
    };
    viewModel.selectedOrgsArray = function(organization_keys,asObject){
        return utils.selectedKeysAsArray(organization_keys,viewModel.orgsWithProfiles(),"organization_key","name",asObject);
    };
    viewModel.tagArray = function(tag_keys,asObject){
        return utils.selectedKeysAsArray(tag_keys,viewModel.tags(),"tag_key","name",asObject);
    };
    ...
    viewModel.collectionControls = {
        ...
        searchInProgress: viewModel.search.searchInProgress,
        searchPageNumber: viewModel.search.searchPageNumber,
...
initRouter(appBase,authBaseUrl,authClientID){
    ...
    router.add('search/', async function () {
        ...
        if(viewModel.search.filter.query.obs() === "")
            viewModel.search.filter.query.obs().valueHasMutated();
        else
            viewModel.search.filter.query.obs()("");
    ...
    router.add(/^search\/([-a-zA-Z0-9"' ]+)$/, async function (query) {
        ...
        viewModel.search.filter.query.obs()(query);
