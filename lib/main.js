    ...
    initViewModel(){
        ...
        const viewModel = {
            ...
            initResultsMap: function(){
                ...
                self.resultsMap.addListener("idle",function(){
                    var ne,sw, bound;
                    if(doSearchWhenIdle){
        ...
        return jQuery.get("api/token/"+code).
            then(function(result){
                ...
                if(result.access_token != null){
                    ...
                    self.db.get("/users_view").auth(viewModel.token()).
                        ...
                        then(function(result){
                            ...
                            if(String(error).indexOf("Not Acceptable")!==-1){ // indicates no results returned, but otherwise call was fine
                                self.db.post("/users_view").auth(viewModel.token()).
                                    set("Prefer","return=representation").
