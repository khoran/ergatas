...
initViewModel(authBaseUrl,clientId,redirect,bucketBase,appBase){
    ...
    const viewModel = {
        ...
        orgsWithProfiles: ko.observableArray(),
        ...
        filter: {
            ...
            movementStage: new Filter("movement_stage",null),
        ...
        saveProfile: async function(form){
            ...
            try{
                ...
                if( ! viewModel.hasProfile()){
                    ...
                    dataLayer.push({event:'profile','profile-action':"saved-new",user_id: viewModel.userId()});
                }else{
                    reloadedProfile =  await self.da.updateProfile(profile.missionary_profile_key(),data);
                    dataLayer.push({event:'profile','profile-action':"saved-existing",user_id: viewModel.userId()});
                }
    ...
    viewModel.orgSelectizeOptions= {
        ...
        onInitialize: function(){
            ...
            if(viewModel.userProfile() != null && viewModel.userProfile().data.organization_key != null){
                viewModel.userProfile().data.organization_key.subscribe((newValue) =>{
                    if(newValue != null && newValue !== ""){
                        dataLayer.push({event:'user','user-action':"profile-org-selected",user_id: viewModel.userId()});
                    }
                });
            }
    ...
    viewModel.tagSelectizeOptions= {
        ...
        onInitialize: function(){
            ...
            ko.bindingHandlers.selectize.utils.setOptions(api,viewModel.tags);
            ko.bindingHandlers.selectize.utils.watchForNewOptions(api,viewModel.tags);

            if(viewModel.userProfile() != null && viewModel.userProfile().data.tag_keys() != null)
                ko.bindingHandlers.selectize.utils.setItems(api,viewModel.userProfile().data.tag_keys);
        },
        onObsUpdate: function(api, value){
            //console.log("new value set on observable: ",value());
            api.clear(true);
    ...
    viewModel.tagSelectizeFilterOptions= {
        create:false,
        valueField: "tag_key",
        labelField:"name",
        searchField: "name",
    ...
    viewModel.fixedSelectizeFilterOptions = {
        //plugins:['remove_button'],
    ...
    viewModel.orgSelectizeFilterOptions= {
        create:false,
        valueField: "organization_key",
        labelField:"name",
        searchField: "name",
        ...
        onInitialize: async function(){
            var api=this;
            ko.bindingHandlers.selectize.utils.setOptions(api,viewModel.orgsWithProfiles);
            ko.bindingHandlers.selectize.utils.watchForNewOptions(api,viewModel.orgsWithProfiles);
        },
        onObsUpdate: function(api, value){
            console.log("new value set on observable: ",value());
            api.clear(true);
    ...
    viewModel.jobCatagoryArray = function(job_catagory_keys,asObject){
        return viewModel.selectedKeysAsArray(job_catagory_keys,viewModel.jobCatagories(),"job_catagory_key","catagory",asObject);
    };
    viewModel.selectedOrgsArray = function(organization_keys,asObject){
        return viewModel.selectedKeysAsArray(organization_keys,viewModel.orgsWithProfiles(),"organization_key","name",asObject);
    };
    viewModel.tagArray = function(tag_keys,asObject){
        return viewModel.selectedKeysAsArray(tag_keys,viewModel.tags(),"tag_key","name",asObject);
    };
    viewModel.selectedKeysAsArray = function(selectedKeys, keyLookup,keyFieldName,nameFieldName,asObject){
        ...
        return async function(name, email,message){
            ...
            try{
                if(email != null && name != null && message != null && message != "")
                    ...
                    dataLayer.push({event:'message-to-missionary',user_id: viewModel.userId()});
    ...
    viewModel.doSearch=ko.computed(async function(){
        ...
        for( var f in viewModel.filter){
            ...
            if(viewModel.filter.hasOwnProperty(f) && filter.isDefined()){
                if(filter.name() === "ageGroups"){
                    ...
                    filter.obs()(viewModel.ageGroupsToBirthYears(ageGroups));
                }
                filters.push(filter);
        ...
        //this needs to stay below the above filter block
    ...
    //}).extend({rateLimit:300,method:"notifyWhenChangesStop"});
    viewModel.onLastPage = ko.computed(function(){
        return viewModel.allResults() == null || ((self.pageSize * viewModel.searchPageNumber()) >= viewModel.allResults().length);
    })
    ...
    viewModel.collectionControls = {
        ...
        getProfile: async function(missionary_profile_key){
            return await self.da.getProfileByKey(missionary_profile_key);
        },
        getProfiles: async function(missionary_profile_keys){
            return await self.da.getProfilesByKey(missionary_profile_keys);
        }
    };
...
//            skills(undefined);
//    };
//    viewModel.removeOrgFromFilter=function(orgKey){
//        var organizations = viewModel.filter.organizations.obs();
//        if(organizations() == null)
//            return;
//        organizations(organizations().filter((key) => {
//            return parseInt(key) !== parseInt(orgKey);
//        }));
//        if(organizations().length === 0)
//            organizations(undefined);
//    };
    viewModel.removeFromFilter=function(obs,itemKey){
        console.log("removing "+itemKey+" from filter ",obs());
        if(obs() == null)
            return;
        obs(obs().filter((key) => {
            return parseInt(key) !== parseInt(itemKey);
        }));
        if(obs().length === 0)
            obs(undefined);
    };
    ...
    viewModel.scrollToFn = function(id){
        return function(){
    ...
    viewModel.ageGroupsToBirthYears = function(ageGroups){
        var years = [];
    ...
    viewModel.birthYearsToAges = function(years){
        var ages = [];
        var currentYear = new Date().getFullYear();
...
async initOrganizations(){
    ...
    try{
        ...
        viewModel.tags(await this.da.tagList());
        viewModel.orgsWithProfiles( await this.da.selectOrganizationsWithProfiles());
...
initRouter(appBase,authBaseUrl,authClientID){
    ...
    router.add('search/', async function () {
        ...
        try{
            await self.loadApprovedOrganizations(true);
        ...
        if( ! await self.isUserVerified()){
            ...
            if(viewModel.loggedIn()){
                try{
                    ...
                    var data = ko.mapping.toJS(viewModel.loggedInUser());
                    delete data.email;
                    data.agreed_to_sof=true;
                    self.da.updateUser(viewModel.loggedInUser().user_key(),data);
                }catch(error){
                    console.warn("failed to update user on profile-post-sof",error);
                }
            } 
