...
class Client {
    ...
    initViewModel(){
        ...
        viewModel.profilePictureUrl = function(){
            var currentFilename = viewModel.userProfile().data.picture_url();
            var url;
            ...
            if(currentFilename == null || currentFilename === "")
                ...
                url = self.bucketBase+
                    viewModel.loggedInUser().userKey()+"/"+ currentFilename;
        ...
        viewModel.replaceProfilePicture= async function(){
            ...
            var filename = viewModel.userProfile().data.picture_url();
            //var filename = url.replace(/^.*?([^/\\&\?]+\.\w{3,4}(?=([\?&].*$|$))).*$/,"$1");
        ...
        viewModel.profilePicturePondOptions = {
            ...
            onprocessfile:function(error,file){
                if(error != null){
                    ...
                    var filename = file.filenameWithoutExtension+"."+file.fileExtension.toLowerCase();
                    console.log("new URL for profile picture: ",filename);
                    viewModel.userProfile().data.picture_url(filename);
                }
            }
        ...
        try{
            ...
            if(tokenResult.access_token != null){
                ...
                try{
                    ...
                    user.userKey = tokenResult.userKey;
