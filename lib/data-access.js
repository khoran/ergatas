...
export class DataAccess {
    ...
    updateProfileFields(profile){
        ...
        if(Array.isArray(profile)){ // update each element of array
            ...
            if(profile.data.published == null)
                profile.data.published = true; //profiles from before update should be assumed published
            if(profile.data.cause_keys== null)
                profile.data.cause_keys = [];
            if(profile.data.people_id3_codes == null)
                profile.data.people_id3_codes = [];
            if(profile.data.rol3_codes == null)
                profile.data.rol3_codes = [];
            if(profile.data.video_url == null)
                profile.data.video_url = "";
            if(profile.data.search_terms== null)
                profile.data.search_terms= "";
    ...
    primarySearch(filters,pageSize,sortField){
        ...
        var arraysToCast= ["job_catagory_keys","organization_keys","kids_birth_years","movement_stages"];
