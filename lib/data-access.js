...
export class DataAccess {
    ...
    updateProfileFields(profile){
        ...
        if(Array.isArray(profile)){ // update each element of array
            ...
            if(data.published == null)
                data.published = true; //profiles from before update should be assumed published
    ...
    primarySearch(filters,pageSize,sortField){
        ...
        var castToBoolean = ["limit_social_media","published"];
        for( var i in filters){
            if( arraysToCast.indexOf(filters[i].name()) !== -1 ){
                //cast values to int
                params[filters[i].name()] = filters[i].obs()().map((x)=>parseInt(x));
            }else if(castToBoolean.includes(filters[i].name()))
                params[filters[i].name()] = filters[i].obs()().map((x)=> x==="true");
            else
