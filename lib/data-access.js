...
export class DataAccess {
    ...
    profileSearch(filters,range,sortField,sortDirection){

        var url = "/rpc/ranked_profiles?order=";
        if(sortField && sortDirection)
            url=url+sortField+"."+sortDirection+",";
        //always add key as secondary order to fix order when primary order fields are equal
        // this is needed for paging to ensure we don't pull in the same record in subsequent
        // pages.
        url = url+"missionary_profile_key.desc&";
        ...
        return this.retry(3,async () =>{
            return this.dbGet(context.url,this.setRange(null,range)).
                        then(function(data,status,xhr){
                            var contentRange = xhr.getResponseHeader("content-range");
                            var num = parseInt(contentRange.replace(/.*\//,""));
                            return {
                                data:data,
                                numResults: num,
                            }
