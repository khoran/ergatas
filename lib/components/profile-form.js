...
export function register(){
   ...
   ko.components.register(NAME, {
       viewModel: function(params) {
            ...
            self.profile_slug = profileObs().profile_slug.extend({rateLimit: 1000});
            self.originalProfileSlug = ko.unwrap(profileObs().profile_slug);
            ...
            self.slugAvailability = ko.observable(null); // null: not checked, true: available, false: taken
            self.slugChecking = ko.observable(false);

            self.checkSlugAvailability = async function(slug) {
                if (!slug || slug.trim() === "") {
                    self.slugAvailability(null);
                    return;
                }
                self.slugChecking(true);
                try {
                    const response = await self.server.postJson("/api/slugExists", {
                        slug: slug.trim(),
                        excludeMissionaryProfileKey: self.missionary_profile_key()
                    });
                    self.slugAvailability(!response.exists);
                } catch (error) {
                    console.error("Failed to check slug availability:", error);
                    self.slugAvailability(null);
                } finally {
                    self.slugChecking(false);
                }
            };

            // Subscribe to slug changes with debouncing
            self.profile_slug.subscribe(function(newSlug) {
                if(self.saving()===true) return;
                self.checkSlugAvailability(newSlug);
            });
            ...
            self.selectedOrganization = ko.computed(() =>{
                ...
                if(org.donation_urls ==  null || org.donation_urls.length == 0){
                    org.donation_urls = [{
                        url: org.website,
                        match_type:"domain"
                    }];
            ...
            self.allowedUrl = function(donation_url){
                const url = new URL(donation_url.url);
                if(donation_url.match_type === "domain"){
                    return self.utils.urlRootDomain(donation_url.url)+" - subdomains okay (eg, subdomain.example.com)";
                }else if(donation_url.match_type === "full_domain"){
                    return url.host+"  - full domain must match";
                }else if(donation_url.match_type === "full_url"){
                    return url.host+url.pathname+" - must match full URL"
                }else{
                    return "Gadzooks! This one wasn't entered correctly, please use the contact page to let us know, thanks!";
                }
            };
            //self.allowedDomains = function(org){
            //    const roots = org.donation_urls.map(u => self.utils.urlRootDomain(u.url));
            //    console.log("allowed domains: ",roots);
            //    if(roots.length === 1)
            //        return "<strong>"+roots[0]+"</strong>";
            //    else
            //        return " one of: <strong>"+roots.join(", ")+"</strong>";
            //};
            self.donationURLRequired = ko.computed(() =>{
                return self.profile.donations_enabled() 
                        && self.selectedOrganization() != null 
                        && (self.selectedOrganization().stripe_account == null
                            || self.selectedOrganization().stripe_account === "");
            });
            ...
            const TAB_STATES = ["org","personal","locations","support","files","ministry"];

            if(profileObs().missionary_profile_key() == null) { // new profile
                self.state("new_profile");
            }else if(location.hash != null
                     && location.hash !== ''
                     && TAB_STATES.includes(location.hash.replaceAll("#",""))){
                self.state(location.hash.replaceAll("#",""));
            }else{
                self.state("org");
            }

            // Keep the URL hash in sync with the active tab so it survives reloads and can be shared.
            self.state.subscribe(function(newState){
                if(TAB_STATES.includes(newState))
            ...
            self.setProfilePicture = function(){
                const bucketPrefix = self.profile.use_mpk_prefix() ? "MPK"+self.missionary_profile_key() : self.userId();
                return this.storage.browseForProfilePicture(self.profile,bucketPrefix,self.server);
            }
            ...
            self.confirmSlugChange = function(){
                return new Promise((resolve) => {
                    alertify.confirm(
                        "Confirm Profile Slug Change",
                        `Changing your profile slug will break any existing links to your profile (such as /worker/${self.originalProfileSlug}). Are you sure you want to continue?`,
                        () => resolve(true), // ok
                        () => resolve(false) // cancel
                    ).set('labels', {ok:'Continue Anyway', cancel:'Restore Old Slug'});
                });
            };
            ...
            self.saveProfile= async function(noAdvance){
                ...
                // Check for slug change on existing profiles
                if (!self.isNew() && self.originalProfileSlug !== ko.unwrap(profileObs().profile_slug)) {
                    const proceed = await self.confirmSlugChange();
                    if (!proceed) {
                        // Restore old slug
                        profileObs().profile_slug(self.originalProfileSlug);
                        // Continue with save
                    }
                }
                ...
                try{
                    ...
                    if(!profileObs().profile_slug() || profileObs().profile_slug() === ""){
                        const baseSlug = (self.profile.first_name() + "-" + self.profile.last_name()).toLowerCase().replace(/[^a-z0-9\-_]/g, '-').replace(/-+/g, '-').replace(/^-|-$/g, '');
                        let slug = baseSlug;
                        let counter = 1;
                        while(true){
                            const response = await self.server.postJson("/api/slugExists", {
                            ...
                            if(!response.exists) break;
                            slug = baseSlug + "-" + counter;
                            counter++;
                        }
                        profileObs().profile_slug(slug);
                    }

                    // Check slug uniqueness
                    if(profileObs().profile_slug() != null && profileObs().profile_slug() !== ""){
                        const response = await self.server.postJson("/api/slugExists", {
                            slug: profileObs().profile_slug(),
                            excludeMissionaryProfileKey: self.missionary_profile_key()
                        });
                        if(response.exists){
                            alertify.error("Profile slug is not unique. Please choose a different one.");
                            return;
                        }
                    }
                    ...
                    if( self.isNew() ){
                        ...
                        const newProfileResponse = await self.server.authPostJson("/api/newProfile",{
                        ...
                        console.log("reloadedProfileResponse: ",reloadedProfile);

                        // only show alerts if this is a manager-created profile (thus having an owner email set)
                        if(self.ownerEmail() != null){
                            if(newProfileResponse.existingProfile === true)
                                alertify.error("The user with email "+self.ownerEmail()+" already has "+
                                    " an Ergatas profile. Please coordinate with them to decide which profile to keep.");
                            else if(newProfileResponse.autoAssigned === true)
                                alertify.success("Since user "+self.ownerEmail()+" has an Ergatas account, we've auto-connected them!");
                            else
                                alertify.success("Invitation to "+self.ownerEmail()+" sent!");
                        }
                    ...
                    // Fire-and-forget: (re)generate this profile's semantic-search
                    // embedding. Must never block or fail the save flow.
                    try{
                        const embedKey = reloadedProfile && reloadedProfile.missionary_profile_key;
                        if(embedKey != null)
                            self.server.authPostJson("/api/updateProfileSimilarity",{missionary_profile_key: embedKey})
                                .catch(err => console.error("failed to update profile similarity embedding",err));
                    }catch(err){ console.error("failed to trigger profile similarity update",err); }

                    ko.mapping.fromJS(reloadedProfile,null,profileObs);
                    self.originalProfileSlug=profileObs().profile_slug();
