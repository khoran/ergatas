...
export function register(){
    ...
    // Per settlement currency (uppercase ISO 4217):
    //   presets  — 3 ascending one-click amounts (~$25/$50/$100 tier)
    //   flatFee  — fixed processing fee, ~ $0.30 USD equivalent, in this currency
    // Currencies absent here fall back to a single "Amount" box + DEFAULT_FLAT_FEE.
    const CURRENCY_CONFIG = {
        USD:{presets:[25,50,100],       flatFee:0.30},
        CAD:{presets:[25,50,100],       flatFee:0.40},
        EUR:{presets:[25,50,100],       flatFee:0.30},
        GBP:{presets:[20,50,100],       flatFee:0.25},
        AUD:{presets:[35,75,150],       flatFee:0.45},
        NZD:{presets:[40,80,150],       flatFee:0.50},
        CHF:{presets:[25,50,100],       flatFee:0.30},
        SGD:{presets:[35,70,140],       flatFee:0.40},
        HKD:{presets:[200,400,800],     flatFee:2.50},
        JPY:{presets:[3000,5000,10000], flatFee:45},
        INR:{presets:[2000,4000,8000],  flatFee:25},
        ZAR:{presets:[400,900,1800],    flatFee:5.50},
        MXN:{presets:[500,1000,2000],   flatFee:6},
        BRL:{presets:[100,250,500],     flatFee:1.50},
        // extend as new settlement currencies appear
    };
    const DEFAULT_FLAT_FEE = 0.30; // fallback for uncurated currencies
    ...
    ko.components.register(name, {
        viewModel: function(params) {
            ...
            // Curated per-currency donation config: preset amounts + flat processing fee.
            self.currencyConfig = ko.pureComputed(() => CURRENCY_CONFIG[self.selectedCurrency()] || null);
            self.presetLevels   = ko.pureComputed(() => { const c = self.currencyConfig(); return c ? c.presets : null; });
            self.showPresets    = ko.pureComputed(() => self.presetLevels() != null);
            self.flatFee        = ko.pureComputed(() => { const c = self.currencyConfig(); return c ? c.flatFee : DEFAULT_FLAT_FEE; });
            // Null-safe accessors for the template (bindings evaluate even when hidden).
            self.presetValue   = i => { const l = self.presetLevels(); return l ? String(l[i]) : ""; };
            self.presetDisplay = i => { const l = self.presetLevels(); return l ? self.moneyFormatWhole(l[i]) : ""; };
            // Keep the selected level valid whenever the available presets change.
            self.syncLevelToCurrency = function(){
                const levels = self.presetLevels();
                if(levels == null){
                    self.level("other");                       // no presets → single Amount box
                } else if(!levels.map(String).includes(self.level())){
                    self.level(String(levels[1]));             // default to the middle tier
                }
            };
            self.presetLevels.subscribe(self.syncLevelToCurrency);
            ...
            if(organization_key != null)
                self.da.getOrganization(organization_key).then(org => {
                    if(org != null){
                        ...
                        // Default to the account's settlement currency, but only if the org can
                        // actually settle it; otherwise fall back to its (single) charge currency.
                        // settlement_currency is sometimes unset while stripe_currencies is not.
                        const charge = self.chargeCurrencies();
                        const settle = self.settlementCurrency();
                        self.selectedCurrency(charge.length > 0 && !charge.includes(settle) ? charge[0] : settle);
                        // ensure the selected level matches the resolved currency's presets
                        self.syncLevelToCurrency();
            ...
            self.fee = ko.computed(()=>{ // stripeFee% + flat fee (~$0.30 USD equiv per currency)
                return self.amount() * stripeFee + self.flatFee();
            })
