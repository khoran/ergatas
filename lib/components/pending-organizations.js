 ...
 *      - navigateFn: root navigate function, used by the "Edit Organization" links
 */
...
export function register(){
   ...
   ko.components.register(name, {
       viewModel: function(params) {
            ...
            // open an org in the org editor without leaving the SPA. Curried like
            // navigateFn so the template can bind it directly to click.
            self.editOrg = (organization_key) =>
                params.navigateFn("dashboard/org-editor?organization_key="+organization_key);
            ...
            self.silentDeny= async function(organization_key){
                try{
                    await params.da.setOrganizationApprovalStatus(organization_key,'denied');
                    await params.da.deleteOrganizationListeners(organization_key);

                    loadPendingOrgs(self.pendingOrganizations,params.da);
                }catch(error){
                    console.error("failed to set approval state for org "+organization_key+". "+error.message,error);
                    alertify.error("Silent deny failed ");
                }
            };
