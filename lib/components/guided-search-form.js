       ...
       viewModel: function(params) {
            ...
            self.doSearch = async function(searchName){
                ...
                if(user_key != null){
                    ...
                    self.appState.router.navigateTo("search/saved/"+encodeURIComponent(searchName));
                }else{
                    //filters are already set in memory by the form above; encode them in
                    //the URL so the search page's applyQueryString reader keeps them
                    //(navigating to bare /search would clear them).
                    var qs = self.search.toQueryString();
                    self.appState.router.navigateTo("search" + (qs ? "?"+qs : ""));
