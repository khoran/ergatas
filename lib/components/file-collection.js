/**
 * INPUT params
 *      - bucketPrefix: prefix for any uploaded files. should not include a trailing '/'
 *      - filenameFilter: array of filenames to not show in file list
 *      - server: object with authPostJson method 
 *      - userId: external user id. 
 *      - title: text for section title. If blank, no title HTMl will be displayed
 *      - readOnly: true for public display, false to allow owner to add/remove files
 *      - useMPKPrefix: if true, use new key format based on profile key. else use userId
 *      - missionary_profile_key
 * 
 */

import * as utils from '../client/client-utils';
//import * as uploaderUtils from '../upload';

async function setFileList(server,fileList,filenameFilter,prefix,getCaption){

    try{
        const titleMaxWidth= 26;
        const args = {};
        const thumbPostfix="-ergatas-thumbnail.png";

        const files = await server.postJson("/api/listUserFiles",{prefix:prefix});

        const filteredFiles= files.filter( (file) =>{
            return filenameFilter.indexOf(file.name) === -1
                    && file.name.indexOf(thumbPostfix) === -1 ;
        });
        console.log("files response: ",filteredFiles);
        const filesWithPreviews = filteredFiles.map((file) =>{
            //see if we can find a file with the thumbnail postfix
            const thumbFile = files.find( (f) =>{
                //remove extension
                const filename = file.name.replace(/\.[^.]+$/, "");
                return f.name === filename+".png"+thumbPostfix;
            });
            var title = file.name.replace(/\.[^.]+$/, "");
            if(title.length >= titleMaxWidth)
                title = title.substring(0,titleMaxWidth-3)+"...";
            var obj = {
                original:file,
                image: file,
                title: title,
                caption: typeof getCaption === 'function' ? (getCaption(file) || '') : '',
                imageFailed: ko.observable(false),
            }
            if(thumbFile != null)
                obj.image = thumbFile;

            return obj;

        });
        console.log("final file list: ",filesWithPreviews);
        //fileList(filteredFiles);
        fileList(filesWithPreviews);
    }catch(error){
        console.error("failed to fetch list of user files",error);
        fileList([]);
    }
}

export function register(){
   ko.components.register('file-collection', {
       viewModel: function(params) {
            var self=this;
            self.title=params.title;
            self.server = params.server;
            console.log("params: ",params);
            const useMPKPrefix = ko.unwrap(params.useMPKPrefix);
            const readOnly = params.readOnly === true;
            const missionary_profile_key = ko.unwrap(params.missionary_profile_key);
            const userId = ko.unwrap(params.userId);
            const bucketPrefix = ko.unwrap(params.bucketPrefix)
                || (useMPKPrefix ? 'MPK' + missionary_profile_key : userId);

            // withUploads controls the delete icon; showUploadButton controls the upload button.
            self.withUploads = ko.observable(!readOnly);
            self.showUploadButton = ko.observable(
                params.showUploadButton !== undefined
                    ? ko.unwrap(params.showUploadButton)
                    : !readOnly
            );
            self.selectedFile= ko.observable();
            self.selectedFileIndex = ko.observable(-1);

            self.fileList = ko.observableArray([]);
            var filenameFilter = ko.unwrap(params.filenameFilter).map(
                name => name.replace(bucketPrefix+"/","")
            );
            console.log("filename filters: ",filenameFilter);

            const getCaption = typeof params.getCaption === 'function' ? params.getCaption : null;

            setFileList(self.server,self.fileList,filenameFilter,bucketPrefix,getCaption);

            self.openViewer = function(file){
                const files = self.fileList();
                const index = files.indexOf(file);
                self.selectedFile(file);
                self.selectedFileIndex(index);
            };

            self.markThumbnailFailed = function(file){
                if(file && ko.isObservable(file.imageFailed))
                    file.imageFailed(true);
            };

            self.closeViewer = function(){
                if(self.swipeHandled){         // swipe just navigated — don't treat the
                    self.swipeHandled = false; // trailing click as a "close" tap
                    return;
                }
                self.selectedFile(undefined);
                self.selectedFileIndex(-1);
            };

            self.showPreviousFile = function(){
                const files = self.fileList();
                if(files.length === 0)
                    return;
                const currentIndex = self.selectedFileIndex();
                const nextIndex = currentIndex <= 0 ? files.length - 1 : currentIndex - 1;
                self.selectedFileIndex(nextIndex);
                self.selectedFile(files[nextIndex]);
            };

            self.showNextFile = function(){
                const files = self.fileList();
                if(files.length === 0)
                    return;
                const currentIndex = self.selectedFileIndex();
                const nextIndex = currentIndex < 0 || currentIndex >= files.length - 1 ? 0 : currentIndex + 1;
                self.selectedFileIndex(nextIndex);
                self.selectedFile(files[nextIndex]);
            };

            // --- mobile swipe navigation ---
            let touchStartX = 0, touchStartY = 0;
            self.swipeHandled = false; // guards the backdrop click so a swipe doesn't also close the viewer

            self.onViewerTouchStart = function(data, event){
                const t = event.touches[0];
                touchStartX = t.clientX;
                touchStartY = t.clientY;
                self.swipeHandled = false;
                return true; // let KO not preventDefault, so pinch/scroll still work
            };

            self.onViewerTouchEnd = function(data, event){
                const t = event.changedTouches[0];
                const dx = t.clientX - touchStartX;
                const dy = t.clientY - touchStartY;
                const SWIPE_THRESHOLD = 50; // px
                if(Math.abs(dx) > SWIPE_THRESHOLD && Math.abs(dx) > Math.abs(dy)){
                    self.swipeHandled = true; // suppress the follow-up backdrop click
                    if(dx < 0)
                        self.showNextFile();
                    else
                        self.showPreviousFile();
                }
                return true;
            };

            self.selectedFileName = ko.pureComputed(function(){
                const selected = self.selectedFile();
                if(selected == null || selected.original == null)
                    return '';
                return selected.original.name;
            });

            self.showUploader = async () =>{

                const uploaderUtils = await import(/* webpackChunkName: "uppy"*/ '../client/upload');
                
                const uppy = uploaderUtils.generalFileUploader(bucketPrefix) ;

                uppy.on('transloadit:complete', (assembly) => {
                    console.log("upload complete: ",assembly.results);

                    uppy.getPlugin('Dashboard').closeModal();
                    setFileList(self.server,self.fileList,filenameFilter,bucketPrefix,getCaption);
                })

                uppy.getPlugin('Dashboard').openModal()
            };
            self.removeFile = (file) => {
                console.log("removing file ",file);
                alertify.confirm("Delete File","Are you sure you want to delete '"+file.name+"'?",
                    async () => {
                        if (params.onDeleteFile) {
                            await params.onDeleteFile(file);
                        } else {
                            await self.server.authPostJson("/api/removeUserFile", {
                                filename: file.name,
                                missionary_profile_key: useMPKPrefix ? missionary_profile_key : undefined,
                            });
                        }
                        if(self.selectedFile() && self.selectedFile().original.name === file.name)
                            self.closeViewer();
                        setFileList(self.server, self.fileList, filenameFilter, bucketPrefix, getCaption);
                    }, () => {});
            };

            if (ko.isObservable(params.refreshTrigger)) {
                params.refreshTrigger.subscribe(() =>
                    setFileList(self.server, self.fileList, filenameFilter, bucketPrefix, getCaption)
                );
            }
        },
        template: require('./file-collection.html'),
    });
}