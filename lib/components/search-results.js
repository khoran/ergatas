   ...
   ko.components.register(NAME, {
       viewModel: function(params) {
            ...
            self.taxReceiptCountryNames = ko.pureComputed(() =>{
                //the filter now holds a single country code (scalar); tolerate a
                //legacy array value from older saved searches by taking the codes present.
                const raw = self.search.filter.taxReceiptCountries.obs()();
                const selected = [].concat(raw == null || raw === "" ? [] : raw);
                const byCode = {};
            ...
            self.filters = [
                ...
                // Hidden pending deployment of the "currencies accepted without exchange"
                // feature. Templates, observables, and currencyCounts computed remain in
                // place; re-add this entry to restore the filter.
                // {
                //     name:"orgCurrencies",
           ...
           if(self.search.searchResultsTemplate() == null)
            ...
            self.selectFilterFn = function(name){
                return  () => {
                    ...
                    jQuery(".cd-panel__content").scrollTop(0);
                };
            };
            ...
            self.panelTitle = ko.pureComputed(function(){
                if(self.page() === "filter_list_template") return "Filters";
                var name = (self.selectedFilter() || "").replace(/_filter$/,"");
                var filter = self.filters.filter(f => f.name === name)[0];
                return filter != null ? filter.display : "Filters";
            });
            ...
            self.toggleFilterPanel = function(state){
                ...
                if(state===true){
                    ...
                    //filter use is a traffic-quality signal for the primary search goal
                    dataLayer.push({event:'search_engagement',action:'open-filters'});
            ...
            //single-country autocomplete for the tax receipt country filter
            self.taxReceiptCountrySelectizeOptions = utils.countrySelectizeOptions(self.appState.countryList.listObs());
