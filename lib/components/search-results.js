   ...
   ko.components.register(NAME, {
       viewModel: function(params) {
            ...
            self.filters = [
                ...
                {
                    name: "missionaryProfiles",
                    display:"Favorite Profiles",
                    showClearBtn: true,
                },
                {
                    name:"organizations",
           ...
           if(self.search.searchResultsTemplate() == null)
            ...
            self.orgSelectizeFilterOptions= {
               ...
               self.downloadPrayerRequestsCSV = async function() {
                   self.loading(true);
                   try {
                       // Get all current search result profiles
                       const profiles = self.search.profiles.peek();
                       if (!profiles || profiles.length === 0) {
                           alertify.error('No profiles in current search results.');
                           self.loading(false);
                           return;
                       }
                       const validProfiles = profiles.filter(p => p && !p.isSeparator && p.missionary_profile_key != null);
                       const resultKeys = (self.search.allResults.peek() || []).map(r => r.missionary_profile_key);
                       const profileKeys = (resultKeys.length > 0 ? resultKeys : validProfiles.map(p => p.missionary_profile_key));
                       const profileByKey = new Map(validProfiles.map(p => [
                           parseInt(p.missionary_profile_key),
                           p,
                       ]));

                       // Fetch all prayer request posts in one query using the profile key list
                       const posts = await self.da.getPublicPrayerPostsByProfileKeys(profileKeys);
                       const allPrayerRequests = (Array.isArray(posts) ? posts : []).map(post => {
                           const profile = profileByKey.get(parseInt(post.missionary_profile_key)) || {};
                           ...
                           const profileUrl = profileSlug
                               ...
                               : '';
                           return {
                               profile_name: ((data.first_name || '') + ' ' + (data.last_name || '')).trim() || ('Profile ' + post.missionary_profile_key),
                               profile_url: profileUrl,
                               post_content: (post.data && post.data.post_content) || '',
                               date_added: post.date_added || '',
                               prayer_count: post.prayer_count || 0,
                           };
                       });
                       if (allPrayerRequests.length === 0) {
                       ...
                       alertify.error('Failed to download prayer requests: ' + (err.message || err));
                   } finally {
                       self.loading(false);
                   }
               };
