import {allCountries, allCurrencies} from '../shared/shared-utils';
import {currencySelectizeOptions} from '../client/client-utils';

/**
 * INPUT
 *   - da: data-access object
 *   - server: server object
 *   - storage: CloudStorage object
 *   - user_key: user_key for currently logged in user
 *   - external_user_id: id for user in auth system
 *   - navigateFn: function that returns a function that changes pages
 *   - adminMode: if true, enable admin only fields
 */


export function register(){
   const NAME="org-application";

   const stateList = require("../data/states.json");
   const countryDataPromise = allCountries();
   const availableCountries = [
       "USA", "CAN", "AUS","GBR",
   ];


   ko.components.register(NAME, {
       viewModel: function(params) {
            var self=this;
            console.log("params: ",params);
            const da=params.da;
            const server = params.server;
            const storage = params.storage;

            self.orgLogoUrl = (x) => storage.orgLogoUrl(x);
            self.navigateFn = params.navigateFn;
            self.orgApplicationStatus= ko.observable("");
            self.organizationApplication = ko.observable();
            self.stateList = ko.observableArray(stateList);
            self.stateFilter = ko.observable();
            self.manualMode = ko.observable(params.adminMode);
            self.countryCodes = ko.observableArray();
            self.currencyData = ko.observable();

            countryDataPromise.then(countryData =>
                self.countryCodes(countryData.filter(x => availableCountries.includes(x.alpha3Code))));

            allCurrencies().then(currencyData => self.currencyData(currencyData));

            self.currencySelectizeOptions = ko.pureComputed(() =>
                currencySelectizeOptions((self.currencyData() && self.currencyData().currencies) || []));

            self.selectedCountryName = ko.pureComputed(() => {
                const application = self.organizationApplication();
                if(application == null || application.country_code == null)
                    return "";
                const country = self.countryCodes().find(c => c.alpha3Code === application.country_code());
                return country != null ? country.name : "";
            });

            // The "will you send your own tax receipt?" checkbox is the inverse of
            // send_receipt (checked = the org sends its own receipt = send_receipt false).
            self.sendsOwnReceipt = ko.pureComputed({
                read: () => {
                    const app = self.organizationApplication();
                    if(app == null || app.donation_settings == null || !ko.isObservable(app.donation_settings.send_receipt))
                        return false;
                    return app.donation_settings.send_receipt() === false;
                },
                write: (checked) => {
                    const app = self.organizationApplication();
                    if(app == null || app.donation_settings == null || !ko.isObservable(app.donation_settings.send_receipt))
                        return;
                    app.donation_settings.send_receipt(!checked);
                },
            });

            self.pendingOrganizationKey = ko.observable(null);
            self.stripeStatus = ko.observable(null);
            self.stripeLoading = ko.observable(false);

            self.loadStripeStatus = async function(){
                try{
                    const status = await server.authPostJson("/api/stripeAccountStatus",
                                        {organization_key: self.pendingOrganizationKey()});
                    self.stripeStatus(status);
                }catch(error){
                    console.error("org-application: failed to load stripe account status", error);
                }
            };
            self.connectStripe = async function(){
                self.stripeLoading(true);
                try{
                    const result = await server.authPostJson("/api/connectStripeAccount",{
                        organization_key: self.pendingOrganizationKey(),
                        return_path: "org-application",
                    });
                    //the return trip may bounce through a sign-in redirect that drops
                    //the query string, so remember the org key here too
                    sessionStorage.setItem("stripe_connect_org",String(self.pendingOrganizationKey()));
                    window.location = result.url;
                }catch(error){
                    console.error("org-application: failed to create stripe onboarding link", error);
                    alertify.error("Oh no, we could not connect to Stripe right now. Please try again later.");
                    self.stripeLoading(false);
                }
            };

            dataLayer.push({event:'user','user-action':"org-application-started",user_id: params.external_user_id()});

            initOrgApp(da,self.organizationApplication);

            // handle returning from a stripe onboarding round-trip. the query params may
            // have been dropped by a sign-in redirect, so fall back to the org key saved
            // in sessionStorage when the redirect to stripe started. the server still
            // verifies this user applied for the given org before doing anything.
            const urlParams = new URLSearchParams(window.location.search);
            const returnedOrgKey = urlParams.get("organization_key") ||
                                   sessionStorage.getItem("stripe_connect_org");
            if(returnedOrgKey != null &&
                    (urlParams.get("stripe_onboarding") != null ||
                     sessionStorage.getItem("stripe_connect_org") != null)){
                self.pendingOrganizationKey(Number(returnedOrgKey));
                // one-shot: clear it once consumed so a leftover key from an
                // earlier "Connect Stripe" click can't force this pending state
                // on every later visit to the page within the same tab
                sessionStorage.removeItem("stripe_connect_org");
                self.orgApplicationStatus("pending");
                self.loadStripeStatus();
                if(urlParams.get("stripe_onboarding") === "refresh")
                    self.connectStripe();
            }

            ko.computed(()=>{
                const application = self.organizationApplication();
                if( application == null || application.country_code == null )
                    return;
                const selectedCountry = application.country_code();

                //self.manualMode(availableCountries.includes(selectedCountry));
                self.manualMode(selectedCountry !== "USA");


            });

            //set the donation currency to the selected country's default. This runs
            // at page load and whenever the country changes, overwriting the current
            // selection (changing country discards a prior currency choice, which is
            // acceptable). It deliberately does NOT read/depend on native_currencies,
            // so once the country is set the user can remove the default and it will
            // not be re-added.
            ko.computed(()=>{
                const application = self.organizationApplication();
                const currencyData = self.currencyData();
                if( application == null || application.country_code == null ||
                        application.donation_settings == null || currencyData == null )
                    return;
                const defaultCurrency = currencyData.countryToCurrency[application.country_code()];
                const selected = application.donation_settings.native_currencies;
                if(selected == null)
                    return;
                selected(defaultCurrency != null ? [defaultCurrency] : []);
            });
            
            self.submitOrgApplication= async function(){
                const user_key = params.user_key();
                const external_user_id= params.external_user_id();
                console.log("submitting org app",self.organizationApplication());
                const country_org_id = self.organizationApplication().country_org_id();
                try{
                    //check to see if org exists and is denied, and inform user
                    var status = await da.unapprovedOrganizationStatus(country_org_id);
                    if(status != null && status.length > 0){
                        if(status[0].status === "denied"){
                            self.orgApplicationStatus("denied");
                            dataLayer.push({event:'user','user-action':"org-application-org-selected",status:"denied",user_id: external_user_id});
                            return;
                        }
                        if(status[0].status === "pending"){
                            await da.insertOrganizationListener(status[0].organization_key,user_key);
                            dataLayer.push({event:'user','user-action':"org-application-org-selected",status:"pending",user_id: external_user_id});
                            self.orgApplicationStatus("pending");
                            self.pendingOrganizationKey(status[0].organization_key);
                            self.loadStripeStatus();
                            return;
                        }
                    }
                    //else, if no status, then org does not yet exist
                    var newOrg = await da.createNonProfitOrganization(
                        ko.mapping.toJS(self.organizationApplication()));
                    console.log("new org: ",newOrg);
                    dataLayer.push({event:'user','user-action':"org-application-org-selected",status:"new-application",user_id: external_user_id});
                    await da.insertOrganizationListener(newOrg.organization_key,user_key);
                    await server.postJson("/api/orgAppNotify",{
                        organization_key:newOrg.organization_key,
                        user_key: user_key,
                        external_user_id:external_user_id});
                    self.orgApplicationStatus("submitted");
                    self.pendingOrganizationKey(newOrg.organization_key);
                    self.loadStripeStatus();
                }catch(error){
                    if(error.status===409)
                        self.orgApplicationStatus("available");
                    else{
                        self.orgApplicationStatus("failed");
                        console.error("Error while submitting organization application: "+error.message,error);
                    }

                }
            };
            self.nonProfitSelectizeOptions = {
                create:false,
                valueField: "ein",
                labelField:"name",
                searchField:["name","displayEIN"],
                onInitialize: function(){
                    var api = this;
                    self.stateFilter.subscribe(function(newValue){
                        console.log("state filter changed ",newValue);
                        api.clearOptions();
                    });

                },
                load: function(query,callback){
                    //console.log("loading options for query "+query);
                    server.postJson("/api/nonProfits",{
                            query:query,
                            state: self.stateFilter(),
                        }).
                        then(function(results){
                            //console.log("results: ",results);
                            callback(results.organizations);
                        }).fail(function(error){
                            console.warn("query failed: "+error.message,error);
                        })
                },
                render: {
                    option: function(data,escape){
                            return "<div>"+data.name+
                                    "</br><span style='font-size:0.7em;'> "+
                                        ( data.displayEIN || data.ein )+", "+data.city+", "+data.state+"</span></div>";
                    },
                },
                onItemAdd: function(value){
                    var data,orgApp;
                    data = this.options[value];
                    orgApp = self.organizationApplication();
                    console.log("onItemAdd: ",data,value);
                    if(data != null && orgApp != null ){
                        dataLayer.push({event:'user','user-action':"org-application-org-selected",user_id: params.external_user_id()});

                        orgApp.name( data.name);
                        orgApp.country_org_id(data.ein);
                        orgApp.city(data.city);
                        orgApp.state(data.state);
                    }
                },
                onDelete: async function(value){
                    console.log("removing item ",value);
                    console.log("orgApp: ",self.organizationApplication());
                    try{
                        await initOrgApp(da,self.organizationApplication,self.organizationApplication().is_shell);
                    }catch(error){
                        console.warn("failed to init org application: ",error);
                    }
                }


            };
            self.browseForLogo = async function(org){
                storage.browseForLogo(org);
            }
        },
        template: require('./'+NAME+'.html'),
    });
}
function initOrgApp(da, obs,is_shell){
    da.newOrganization().
        then((newOrg) => {
            newOrg.is_shell = !! is_shell;
            newOrg.country_code="USA";
            obs(ko.mapping.fromJS(newOrg));
            console.log("org app: ",obs());
            obs().name.extend({rateLimit:300,method: "notifyWhenChangesStop"});
        });


};
