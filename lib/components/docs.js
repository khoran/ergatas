...
export function register(){
   ...
   ko.components.register(name, {
      viewModel: function(params) {
            ...
            // The cloned page template for the selected article (or null for the
            // intro). getPage returns a Promise (article templates live in a lazy
            // chunk), so load into an observable instead of a computed.
            self.currentDocPage = ko.observable(null);
            function loadDoc(n){
                if(!n){
                    self.currentDocPage(null);
                    return;
                }
                Promise.resolve(self.getPage(n))
                    .then( tmpl => self.currentDocPage(tmpl ? tmpl.clone() : null))
                    .catch( e => {
                        console.warn("docs: failed to load article '"+n+"'",e);
                        self.currentDocPage(null);
                    });
            }
            self.docsPage.subscribe(loadDoc);
            loadDoc(self.docsPage());
