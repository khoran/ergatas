...
const countryUrl = "https://restcountries.eu/rest/v2/all";
 ...
 function buildTree(countryCodesObs,readOnlyMode){
    ...
    return jQuery.get(countryUrl).then((result)=>{
        result.forEach((country) =>{
            ...
            countryNode.setState(ko.unwrap(countryCodesObs).indexOf(country.alpha3Code) !== -1);
...
export function register(){
   ...
   ko.components.register(name, {
       viewModel: function(params) {
            ...
            var start = new Date();
            buildTree(self.countryCodes,self.ro).then((tree)=>{
