...
export function register(){
    ...
    ko.components.register(name,{
        viewModel: function(params){
            ...
            // search
            self.searchTerm = ko.observable('').extend({ rateLimit: { timeout: 500 } });

            // paging
            self.pageSize = PAGE_SIZE;
            self.currentPage = ko.observable(1);
            self.searchTerm.subscribe(() => self.currentPage(1));
            self.filteredProfiles = ko.computed(() => {
                const term = self.searchTerm().toLowerCase();
                if (!term) return self.managedProfiles();
                return self.managedProfiles().filter(p => {
                    return Object.values(p).some(val => val && val.toString().toLowerCase().includes(term));
                });
            });
            self.totalPages = ko.computed(() => Math.ceil(self.filteredProfiles().length / PAGE_SIZE));
            self.visibleProfiles = ko.computed(() => {
                const start = (self.currentPage() - 1) * PAGE_SIZE;
                return self.filteredProfiles().slice(start, start + PAGE_SIZE);
            });
