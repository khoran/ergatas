   ...
   ko.components.register(name, {
       viewModel: function(params) {
            ...
            setTimeout(async function(){
                const {default: datatables}  = await import(/* webpackChunkName: "datatables"*/ '../client/datatables');
            },1000);
            ...
            //sends the real welcome email to the logged-in admin so the Mailgun
            //template can be proofed without creating a throwaway profile.
            self.sendingWelcome = ko.observable(false);
            self.sendTestWelcomeEmail = async function(){
                self.sendingWelcome(true);
                try{
                    const result = await params.server.authPostJson("/api/testWelcomeEmail",{});
                    if(result && result.sent === false)
                        alertify.error("Mailgun rejected the welcome email. Check the server log.");
                    else
                        alertify.success("Welcome email sent to "+((result && result.email) || "you"));
                }catch(error){
                    console.error("failed to send test welcome email",error);
                    alertify.error("Failed to send test welcome email");
                }finally{
