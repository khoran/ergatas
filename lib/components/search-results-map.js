    ...
    clusteredDataProvider.addEventListener('tap', async function(event){
        ...
        try{
            ...
            if(Array.isArray(data))
                profiles = await profileControls.getProfiles(data.map((p) => p.missionary_profile_key));
            else
                profiles = await profileControls.getProfile(data.missionary_profile_key);
            bubble.setContent(bubbleContent(profiles,profileControls));
            bubble.open();
        }catch(error){
