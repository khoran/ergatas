...
function main(params) {
   ...
   mapUtils.initMap().then( () =>{
       ...
       // register the cluster click handler once; it reads the current markers
       // off the cluster each time, so it does not need to be re-added per update.
             ...
             if(allEqual){ //they are all on top of each other
                ...
                openInfoWindow(cluster.getCenter(),map,async () =>{
                  ...
                  try{
                     ...
                     console.error("failed to fetch full profile for markers ",markers);
                  }
