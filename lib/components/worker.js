
import { param } from 'jquery';
import * as utils from '../client/client-utils';
import {ensureFields} from '../shared/shared-utils';
/**
  * INPUT
  * -----
  *      - profile:  observable with profile object
  *      - appState: AppState object
  *      - userProfile: obs with logged in user's profile object, if any
  *      - navigateFn: function that returns a function to perform a navigation
  */

export function register(){
   const NAME="worker";


   ko.components.register(NAME, {
        viewModel: function(params) {
            console.log(NAME+" params: ",params);
            var self=this;

            ensureFields(params,["profile","appState","userProfile","navigateFn"]);
            self.profile = params.profile;
            //fire once per worker-profile view; 'source' is the in-app page navigated from
            //(or document.referrer on a direct load) so this can be scoped in GA4/Ads to
            //arrivals from /learn/* or /search — the ad-landing conversion path.
            if(self.profile && self.profile.missionary_profile_key != null)
                dataLayer.push({event:'view_worker_profile',
                            missionary_profile_key:self.profile.missionary_profile_key,
                            source:utils.getNavSource()});
            self.appState = params.appState;
            self.utils = utils;
            self.storage = params.appState.storage;
            self.server = params.appState.server;
            self.da = params.appState.da;


            self.stats = ko.observable();
            self.descExpanded = ko.observable(false);
            self.descOverflow = ko.observable(false);
            self.orgDescriptionExpanded = ko.observable(false);
            self.immediateOpen=ko.observable(location.hash==="#connect");
            self.profilePosts = ko.observableArray([]);
            self.prayedPostKeys = ko.observable({});

            self.userFavorites = utils.getUserFavoritesLazy(params.appState.loggedInUser(), self.da);
            self.isFavorite = utils.isFavorite(self.profile,self.userFavorites);

            self.loadProfilePosts = async function(){
                try{
                    const posts = await self.da.getPublicPostsByProfileKey(self.profile.missionary_profile_key);
                    self.profilePosts(posts.map(post => ({
                        post_key: ko.observable(post.post_key),
                        post_type: ko.observable(post.post_type),
                        date_added: ko.observable(post.date_added),
                        post_content: ko.observable(post.post_content),
                        prayer_count: ko.observable(post.prayer_count),
                        hasOverflow: ko.observable(false),
                        expanded: ko.observable(false),
                        originalHeight: null,
                    })));
                }catch(error){
                    console.error('failed to load profile posts',error);
                    self.profilePosts([]);
                }
            };
            self.loadProfilePosts();

            // "Similar Workers" — nearest profiles by embedding similarity,
            // shown 3-at-a-time and rendered by <profile-collection> so the
            // cards match the search page exactly; arrows page the data.
            self.similarProfiles = ko.observableArray([]);
            self.similarPageSize = 3;
            self.similarPage = ko.observable(0);           // 0-based page index
            self.similarPageCount = ko.pureComputed(function(){
                return Math.max(1, Math.ceil(self.similarProfiles().length / self.similarPageSize));
            });
            self.similarPageProfiles = ko.pureComputed(function(){
                const start = self.similarPage() * self.similarPageSize;
                return self.similarProfiles().slice(start, start + self.similarPageSize);
            });
            self.nextSimilarPage = function(){
                self.similarPage(Math.min(self.similarPageCount() - 1, self.similarPage() + 1));
            };
            self.prevSimilarPage = function(){
                self.similarPage(Math.max(0, self.similarPage() - 1));
            };
            self.loadSimilarProfiles = async function(){
                try{
                    self.similarPage(0);
                    if(self.profile == null || self.profile.missionary_profile_key == null){
                        self.similarProfiles([]);
                        return;
                    }
                    const rows = await self.da.similarProfiles(self.profile.missionary_profile_key, 6);
                    const keys = (rows || []).map(r => r.profile_key).filter(k => k != null);
                    if(keys.length === 0){ self.similarProfiles([]); return; }
                    const profiles = await self.da.getDisplayProfilesByKey(keys);
                    // getDisplayProfilesByKey returns DB order; restore similarity order.
                    const byKey = {};
                    (profiles || []).forEach(p => { byKey[p.missionary_profile_key] = p; });
                    self.similarProfiles(keys.map(k => byKey[k]).filter(Boolean));
                }catch(error){
                    console.error("failed to load similar profiles", error);
                    self.similarProfiles([]);
                }
            };
            self.loadSimilarProfiles();

            self.formatPostDate = function(value){
                const rawValue = ko.unwrap(value);
                if(rawValue == null || rawValue === '')
                    return 'No date';
                const date = new Date(rawValue);
                if(Number.isNaN(date.getTime()))
                    return rawValue;
                return date.toLocaleDateString();
            };

            self.hasPrayedForPost = function(postKey){
                return self.prayedPostKeys()[postKey] === true;
            };

            self.prayForPost = async function(post){
                const postKey = ko.unwrap(post.post_key);
                if(self.hasPrayedForPost(postKey))
                    return;

                try{
                    const updatedPost = await self.server.postJson('/api/profilePostPrayer',{
                        post_key: postKey,
                    });
                    post.prayer_count(parseInt(updatedPost.prayer_count) || (post.prayer_count() + 1));
                    self.prayedPostKeys(Object.assign({}, self.prayedPostKeys(), {[postKey]: true}));
                    dataLayer.push({
                        event: 'prayed',
                        missionary_profile_key: self.profile.missionary_profile_key,
                        post_key: postKey,
                    });
                }catch(error){
                    console.error('failed to increment profile post prayer count',error);
                }
            };
            
            self.peopleGroups = ko.observableArray();
            utils.peopleGroupNames(self.server,self.profile.data.people_id3_codes).
                then( names =>{
                    self.peopleGroups(names);
                });

            self.languages = ko.observableArray();
            utils.languageNames(self.server,self.profile.data.rol3_codes).
                then( names =>{
                    self.languages(names);
                });

            self.donate = function(){
                var ownProfile = self.ownProfile();
                self.utils.donate(self.da,self.profile,'detail',ownProfile);
            }
            self.ownProfile = function(){
                return params.userProfile() && (params.userProfile().missionary_profile_key() === self.profile.missionary_profile_key);
            }

            self.prayClicked = ko.observable(false);
            self.pray = function(){
                if(self.prayClicked() === false){
                    self.prayClicked(true);
                    dataLayer.push({
                        event: "prayed",
                        missionary_profile_key: self.profile.missionary_profile_key
                    });

                    console.log("stats: ",self.stats());
                    if(self.stats() != null && self.stats().prayers != null){
                        var x=self.stats();
                        console.log(" updating prayer count ",x);
                        x.prayers = parseInt(x.prayers) + 1;
                        self.stats(x);
                    }
                        
                }
            }
            self.updateStats = function(){
                utils.pageStats(self.server, self.profile.missionary_profile_key).
                    then(self.stats);
            }

            self.causeNames = ko.observable();
            self.appState.causeList.selectItems(self.profile.data.cause_keys,self.causeNames);

            self.tagNames = ko.observable();
            self.appState.tagList.selectItems(self.profile.data.tag_keys,self.tagNames);

            self.jobCatagoryNames = ko.observable();
            self.appState.jobList.selectItems(self.profile.data.job_catagory_keys,self.jobCatagoryNames);

            self.orgNames = ko.observable();
            self.appState.approvedOrgList.selectItems(self.profile.data.organization_key,self.orgNames);

            self.quickSearchFn = function(filterName,keyField,asArray=false){
               return utils.quickSearchFn(self.appState,filterName,keyField,asArray);
            }

            self.updateStats();
           
            self.toggleDescription= function(data,event){
                self.descExpanded( ! self.descExpanded());
                return false;
            }
            self.toggleOrgDescription = function(){
                self.orgDescriptionExpanded(!self.orgDescriptionExpanded());
                return true;
            }
            self.togglePostContent = function(post, event){
                post.expanded(!post.expanded());
                return false;
            }
            self.lastUpdatedClass = function(){
                var last_update = new Date(self.profile.last_updated_on).getTime();
                const year = 365*24*60*60*1000; // millies in a year
                if((new Date().getTime()) - last_update < year){
                    return  "muted-font";
                }else{
                    return "text-danger"
                }
            }
        },
        template: require('./'+NAME+'.html'),
    });

}
 
