...
export function register(){
    ...
    ko.components.register(name,{
        viewModel: function(params){
            ...
            // ---- donations this user has MADE to other workers (they may be a donor as well
            // as a worker). Loaded here rather than in the donor-donations component so this
            // component can lay both lists out together.
            self.donorDonations = ko.observableArray();
            self.donorSubscriptions = ko.observableArray();
            //years the donor can get an Ergatas-issued tax receipt for (decided server side:
            //gifts to orgs that issue their own receipts don't count)
            self.donorReceiptYears = ko.observableArray();
            self.donorLoading = ko.observable(true);

            self.server.authPostJson("/api/getDonorDonations",{}).then(data =>{
                self.donorDonations((data && data.donations) || []);
                self.donorSubscriptions((data && data.subscriptions) || []);
                self.donorReceiptYears((data && data.receipt_years) || []);
                self.donorLoading(false);
            }).catch(err => {
                console.error("failed to load donor donations: ",err);
                self.donorLoading(false);
            });

            self.hasWorkerDonations = ko.computed(() => (self.donations() || []).length > 0);
            self.hasDonorDonations = ko.computed(() =>
                (self.donorDonations() || []).length > 0 || (self.donorSubscriptions() || []).length > 0);
            // only worth collapsing when there's more than one list to look at
            self.showBoth = ko.computed(() => self.hasWorkerDonations() && self.hasDonorDonations());
            ...
            self.uniqueNames = ko.computed(() => {
                const donations = self.filteredDonations();
