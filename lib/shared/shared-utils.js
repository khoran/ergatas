...
// resolves to {currencies:[{code,name,symbol}], countryToCurrency:{<alpha3>:<code>}}
export function allCurrencies(){
   const currencyUrl= '/currency_data.json';
   return jQuery.get(currencyUrl);
}
...
export function searchParamsFromJson(searchFilter){
    ...
    for (const [key,value] of Object.entries(searchFilter)){
        ...
        } else if(key === "org_currencies" || key === "tax_receipt_countries"){
            //currency and country codes are stored uppercase. tax_receipt_countries is
            //now a single code (scalar), but the RPC param is an array and legacy saved
            //searches may hold an array, so normalize both to an uppercased array.
            params[key] = [].concat(value == null ? [] : value).map( x => String(x).toUpperCase());
        }else
            params[key] = value;
    }
