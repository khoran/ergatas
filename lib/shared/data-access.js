...
export class DataAccess {
    ...
    // is an alterntative to use when 0 or 1 is acceptable. return null for 0 results.
    singleOrNone(result){
        if(result != null && result.length == 1)
            ...
            throw AppError("multiple results returned for when only 1 or 0 where expected");
    }
   ...
   getUsersWithoutEntities(user_key){ //used for user cleanup only
    ...
    primarySearch(params,pageSize,sortField,use_or=null,all_profiles=false,profile_sub_search=false){
        ...
        if(finalParams.missionary_profile_keys.length === 0)
            finalParams.missionary_profile_keys = null // postgres throws a type error for empty arrays
