...
export class DataAccess {
   ...
   getUsersWithoutEntities(user_key){ //used for user cleanup only
    ...
    /// Profiles
    createProfile(data){
        data.data = this.normalizeProfileData(data.data);
        return this.retry(3,async () =>{
    ...
    updateProfileFields(profile){
        ...
        if(Array.isArray(profile)){ // update each element of array
            ...
            profile.data = this.normalizeProfileData(data);
            data = profile.data;
    ...
    updatePostFields(post){
        if(Array.isArray(post))
            return post.map(p => this.normalizePostRecord(p));
        return this.normalizePostRecord(post);
    }

    postPayload(post){
        const normalized = this.normalizePostRecord({
            ...post,
            data: {
    ...
    getOwnedPostsByProfileKey(missionary_profile_key){
        return this.retry(3,async () =>{
            const results = await this.dbAuthGet("/posts_view?missionary_profile_key=eq."+missionary_profile_key+"&order=date_added.desc,post_key.desc");
            return this.updatePostFields(results || []);
        });
    }
    getPublicPostsByProfileKey(missionary_profile_key){
        return this.retry(3,async () =>{
            const results = await this.dbGet("/public_posts_view?missionary_profile_key=eq."+missionary_profile_key+"&order=date_added.desc,post_key.desc");
            return this.updatePostFields(results || []);
        });
    }
    getPublicPrayerPostsByProfileKeys(missionary_profile_keys){
        return this.retry(3,async () =>{
            ...
            const results = await this.dbPost(
    ...
    createPost(post){
        return this.retry(3,async () =>{
            const created = await this.dbAuthPost("/posts_view",this.postPayload(post),this.single(this.representation()));
            return this.updatePostFields(created);
        });
    }
    updatePost(post_key,post){
        return this.retry(3,async () =>{
            const updated = await this.dbAuthPatch("/posts_view?post_key=eq."+post_key,this.postPayload(post),this.single(this.representation()));
            return this.updatePostFields(updated);
        });
    }
    deletePost(post_key){
        return this.retry(3,async () =>{
            return await this.dbAuthDelete("/posts_view?post_key=eq."+post_key,this.single(this.representation()));
        });
    }
    getPostPrayerByKey(post_key){
        return this.retry(3,async () =>{
            return await this.dbAuthGet("/posts_prayer_view?post_key=eq."+post_key,this.single());
        });
    }
    updatePostPrayerCount(post_key,prayer_count){
        return this.retry(3,async () =>{
            return await this.dbAuthPatch(
    ...
    getSavedSearchesByName(user_key,name){
        return this.retry(3,async () =>{
            return await  this.dbGet("/saved_searches_view?limit=1&and=(user_key.eq."+user_key+",data->>name.eq."+name+")",this.single());
