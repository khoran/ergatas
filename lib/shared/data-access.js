...
export class DataAccess {
    ...
    updateUser(user_key,data){
        //these two fields are read-only, so must be removed before an update is attempted
    ...
    getOrganization(organization_key){
        return this.retry(3,async () =>{
            return await this.dbGet("/non_profit_and_organizations_view?organization_key=eq."+organization_key,this.single());
        });
    }
    ...
    getOrganizationBySlug(slug){
        return this.retry(3,async () =>{
            return await this.dbGet("/non_profit_and_organizations_view?slug=eq."+slug,this.single());
        });
    }
    ...
    getAllStripeTransactions(){
        return this.retry(3,async () =>{
            return this.dbAuthGet("/donations_view?stripe_id=neq.");
        });
    }
    getWorkerTransactions(user_key){
        return this.retry(3,async () =>{
            return this.dbAuthGet("/workers_donations?user_key=eq."+user_key);
        });
    }
    // email hashes
    ...
    getUserOrgSearchFilter(userId){
        return this.retry(3,async () =>{
            return await  this.dbAuthGet("/user_org_search_filters?external_user_id=eq."+userId,this.single());
        });
    }
    ...
    insertProfileInvitation(created_by_external_user_id,missionary_profile_key,email){
        return this.retry(3,async () =>{
            return await  this.dbAuthPost("/profile_invitations_view",{
                missionary_profile_key:missionary_profile_key,
                email:email,
    ...
    getProfileInvitations(){
        return this.retry(3,async () =>{
            return await  this.dbAuthGet("/profile_invitations_view",this.single());
        });
    }
