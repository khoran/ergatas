...
// PostgREST's and()/or() composite filters treat "," and "()" as syntax, so a
// free-text value (e.g. a user-chosen saved-search name) containing those
// characters must be wrapped in double quotes to be read as a literal value.
// Plain single-column eq. filters elsewhere in this file aren't grouped this
// way and don't need it.
...
export class SavedSearchesRepo extends BaseRepo {
    ...
    getSavedSearchesByName(user_key,name){
        return this.run(async () =>{
            var quotedName = encodeURIComponent(quotePostgrestValue(name));
            return await  this.client.get("/saved_searches_view?limit=1&and=(user_key.eq."+user_key+",data->>name.eq."+quotedName+")",H.single());
