...
export class TransactionsRepo extends BaseRepo {
    ...
    //used to attribute a stripe charge back to the worker it was given to
    getDonationsByStripeIds(stripe_ids){
        return this.run(async()=>{
            return this.client.authGet("/donations_view?stripe_id=in.("+
                stripe_ids.map(encodeURIComponent).join(",")+")");
        });
    }
    insertDonor(donor){
        return this.run(async()=>{
