...
export class SearchesRepo extends BaseRepo {
    ...
    // Nearest published/current profiles to a given profile, by embedding
    // cosine distance. Anonymous (public) call, like primarySearch.
    // Returns [{profile_key, distance}, ...] ordered most-similar first.
    similarProfiles(missionary_profile_key, maxResults=6){
        return this.run(async () =>{
            return await this.client.post("/rpc/similar_profiles_v1",
                { target_key: missionary_profile_key, max_results: maxResults },
                null, true);
        });
    }
