...
export class OrganizationsRepo extends BaseRepo {
    ...
    nonProfitsWithStripeAccounts(){
        return this.run( async ()=>{
            return await this.client.authGet(
                "/non_profits_view?stripe_account=neq.&select=non_profit_key,stripe_account,donation_settings");
        });
    }
    getNonProfitByStripeAccount(stripe_account){
        return this.run( async ()=>{
            return await this.client.authGet(
                "/non_profits_view?stripe_account=eq."+encodeURIComponent(stripe_account),H.single());
        });
    }
    ...
    insertOrganizationListener(organization_key,user_key){
        return this.run(async () =>{
            return this.client.authPost("/organization_listeners_view?on_conflict=organization_key,user_key",
                    {organization_key, user_key}, H.ignoreDups());
        });
    }
