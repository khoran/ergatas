...
export function normalizeSocialMediaPostRecord(post){
    const normalized = post || {};
    const data = normalized.data || {};
    normalized.data = data;
    normalized.title = data.title || '';
    normalized.description = data.description || '';
    normalized.image_url = data.image_url || '';
    normalized.link_url = data.link_url || '';
    normalized.post_date = normalized.post_date || new Date().toISOString().slice(0,10);
    return normalized;
}

export function updateSocialMediaPostFields(post){
    if(Array.isArray(post))
        return post.map(p => normalizeSocialMediaPostRecord(p));
    return normalizeSocialMediaPostRecord(post);
}

// Build the PostgREST write payload. Optional image/link fields are omitted
// from `data` when empty rather than stored as empty strings.
export function socialMediaPostPayload(post){
    const src = post || {};
    const data = {
        title: src.title || '',
        description: src.description || '',
    };
    if(src.image_url != null && src.image_url !== '')
        data.image_url = src.image_url;
    if(src.link_url != null && src.link_url !== '')
        data.link_url = src.link_url;

    return {
        post_date: src.post_date || new Date().toISOString().slice(0,10),
        data: data,
    };
}
...
export function updateNonProfitFields(nonProfit){
    ...
    if(Array.isArray(nonProfit)){ // update each element of array
        ...
        if(data.native_currencies == null)
            data.native_currencies = [];

        if(data.stripe_currencies == null)
            data.stripe_currencies = [];
