...
const getDB = () =>{
    ...
    if(db == null){
        db = new DataAccess(postgrestBase,axios);
    }
...
const removeAllFiles = (userId,filename) =>{
    listUserFiles(userId).forEach( async (file) =>{
        await file.delete();
    });
}
...
function hashToEmail(hashedEmail){
   ...
   //             }).
   getDB().getEmailHash(hashedEmail).
        then((response) =>{
...
const emailDisableNotice = (userId) =>{
    return emailUpdateNotice(userId,"profile-disabled.html",
        ...
        " To re-activate your profile, got to ergatas.org/profile, make any updates, then click 'Save'"
    );
}
const emailUpdateNotice = async (userId,templateName,textVersion) =>{
    ...
    try{
        ...
        console.log("failed to get email for user id "+userId);
        return;
    }
    try{
        ...
        console.log("failed to get email template "+templateName+" while sending notice to user id "+userId);
        return;
    }
    ...
    try{
        var body = await sendEmail(data);
        console.log("update notice message email sent!",body);
    }catch(error){
        console.error("failed to send update notice email to userId"+userId+", with email "+email,error);
