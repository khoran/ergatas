...
import {randomIntBetween} from './shared-utils.js';
...
export function init(){
    mailingList = new MailingList(process.env.MAILCHIMP_KEY,process.env.MAILCHIMP_SERVER_PREFIX);
}
...
const deleteUser = async (userId,email) =>{
    ...
    try{
        ...
        mailingList.removeUser(listId,email);
    }catch(error){
...
const addUserToMailinglist = async function(email){
    ...
    try{
        await mailingList.ensureUser(listId,email);
        await mailingList.addTags(listId,email,["User"]);
    }catch(error){
        console.warn("failed to add or set tag for new User "+email+": "+error.message,error);
    }
}
const newsletterSignup = async function(firstName,lastName,email,addToPrayerList,recaptchaScore){
    ...
    try{
        await mailingList.ensureUser(listId,email);
        await mailingList.addTags(listId,email,tags);
        await mailingList.setUserName(listId,email,firstName,lastName);
...
export async function newProfile(email,firstName,lastName,missionary_profile_key){
    console.local("setting user name: "+firstName+", "+lastName+", email: "+email);

    const listId= process.env.MAILCHIMP_LIST_ID;
    try{
        await mailingList.ensureUser(listId,email);
        await mailingList.addTags(listId,email,["Missionary"]);
        await mailingList.setUserName(listId,email,firstName,lastName);
    }catch(error){
...
function rankedMatches(query,data,fields){
...
export async function peopleGroupSearch(query,limit,peopleGroupPromise) {
    try{
        var pgNames = await peopleGroupPromise;
        if(pgNames != null)
...
export async function peopleGroupNames(peopleID3Codes,peopleGroupPromise) {
    try{
        var pgNames = await peopleGroupPromise; 
...
export async function downloadPeopleGroups(){

    const urlRoot = process.env.JOSHUA_PROJECT_API_ROOT;
    const key =process.env.JOSHUA_PROJECT_API_KEY;
    var url = urlRoot+`people_groups.json?api_key=${key}&limit=100000`
    var peopleGroups;
    var nameData={};
    var fieldsToKeep = ["PeopleID3","PeopNameInCountry","PeopleCluster","PeopNameAcrossCountries"];
    try{
        console.local("starting people group download...");
        peopleGroups = (await axios({
            method: "GET",
            url: url
        })).data;
        ...
        peopleGroups.forEach( group => {
            ...
            fieldsToKeep.forEach(fieldName => 
                data[fieldName] = group[fieldName]);
            nameData[data.PeopleID3] = data;
        });
        return nameData;
...
export async function languageSearch(query,limit,languagePromise) {
    try{
        var names = await languagePromise;
        if(names != null)
...
export async function languageNames(rol3Codes,languagePromise) {
    try{
        var names = await languagePromise; 
...
export async function downloadLanguages(){
    ...
    const urlRoot = process.env.JOSHUA_PROJECT_API_ROOT;
    const key =process.env.JOSHUA_PROJECT_API_KEY;
    var url = urlRoot+`languages.json?api_key=${key}&limit=100000`
    var languages;
    var nameData={};
    ...
    try{
        ...
        languages.forEach( lang => {
            var data = {};
            fieldsToKeep.forEach(fieldName => 
                data[fieldName] = lang[fieldName]);
            nameData[data.ROL3] = data;
        });
        return nameData;
