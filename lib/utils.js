...
const nonProfitSearch = async (query,state) =>{
    var url = "https://projects.propublica.org/nonprofits/api/v2/search.json?c_code[id]=3&q="+query;
...
const buildIndex = async function(pageName,info,url){
    ...
    if(pageName==="profile-detail" && url != null){
        try{
            ...
            missionary_profile_key = url.match(/\/(\d+)(?:\?.*)?$/)[1];
...
/* not used
export async function writeWorldShapes(){
    const countryUrl = "https://restcountries.eu/rest/v2/all";
    var countryCodes ;
    var allShapes
    var countryData = (await axios.get(countryUrl)).data;
    const __dirname = path.resolve();

    countryCodes = countryData.map((country) => country.alpha3Code );
    allShapes = await getAllCountryShapes(countryCodes);

    //write allShapes to data file

    console.local("writing "+Object.entries(allShapes).length+" shapes to file");
    //console.local("USA ",allShapes.USA);
    //console.local(JSON.stringify(allShapes.USA));
    const index= fs.writeFileSync(`${__dirname}/lib/data/country-shapes.json`,JSON.stringify(allShapes));
}
async function getCountryShapes(countryCodes){
    const mapAPIKey = process.env.HERE_MAP_API_KEY;
    const urlBase = "https://geocoder.ls.hereapi.com/6.2/geocode.json?apiKey="+mapAPIKey+
            "&additionaldata=IncludeShapeLevel,country";

    var result= (await axios.get(urlBase+"&country="+countryCodes.join(","))).data;

    //console.local("country shape result (js): ",result);
    ...
    if(views.length > 0){
        ...
        if(innerResults.length > 0){
            innerResults.forEach((inner)=>{
                if(! inner.Location.Shape){
                    console.warn("missing data: ",inner);
                }else
...
export async function getAllCountryShapes(countryCodes){
    ...
    for(i=0;i < length; i=i+batchSize){
        batch = countryCodes.slice(i,i+batchSize);
        jobs.push(getCountryShapes(batch).then((shapes)=>{
            var entries = Object.entries(shapes);
            console.log("appending "+entries.length+" shapes to final result");
            entries.forEach((entry) =>{
                allShapes[entry[0]] = entry[1];
            });
        }));
    }
    ...
    console.log("got "+Object.entries(allShapes).length+" shapes");
    if(Object.entries(allShapes).length !== countryCodes.length){
        console.warn("failed to fetch all shapes, "+countryCodes.length+" given, but only got "+Object.entries(allShapes).length);
    }
    return allShapes;
}
*/
