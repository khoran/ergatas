...
export async function peopleGroupSearch(query,limit,peopleGroupPromise) {
    try{
        ...
        if(pgNames != null)
            return  Object.values(pgNames).filter(group =>{
                var queryRE = new RegExp(query,"i");
                return queryRE.test(group.PeopNameInCountry) 
                        || queryRE.test(group.PeopleCluster)
                        || queryRE.test(group.PeopNameAcrossCountries);
            }).slice(0,limit);
        else{
...
export async function peopleGroupNames(peopleID3Codes,peopleGroupPromise) {
    try{
        ...
        //return  pgNames.filter(group => peopleID3Codes.includes(group.PeopleID3));
    }catch(error){
...
export async function downloadPeopleGroups(){
    ...
    try{
        ...
        console.local("done. got "+peopleGroups.length+" records",peopleGroups[0]);
        ...
        console.local("nameData first el: ",nameData[0]);
        return nameData;
...
export async function languageSearch(query,limit,languagePromise) {
    try{
        ...
        if(names != null)
            return  Object.values(names).filter(group =>{
                var queryRE = new RegExp(query,"i");
                return queryRE.test(group.Language) ;
            }).slice(0,limit);
        else{
...
export async function downloadLanguages(){
    ...
    try{
        ...
        console.local("done. got "+languages.length+" records",languages[0]);
