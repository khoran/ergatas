...
import { jwtPayload, getServerDB, sendTemplatedEmail, userIdToEmail, deleteBlobByPath } from './utils.js';
...
async function getOwnedProfile(sdb, userId, missionary_profile_key) {
    ...
    const profile = await sdb.dbAuthGet(
...
export async function recordWorkerDocument(token, { missionary_profile_key, document_type, storage_path, original_filename, submission_period }) {
    ...
    try {
        ...
        if (err.status === 409) {
            ...
            try { await deleteBlobByPath(storage_path); }
...
export async function listWorkerDocuments(token, missionary_profile_key) {
    ...
    const [docs, settings] = await Promise.all([
        sdb.dbAuthGet(`/worker_documents_view?missionary_profile_key=eq.${missionary_profile_key}&order=submission_period.desc`),
...
export async function deleteWorkerDocument(token, worker_document_key) {
    ...
    const profile = await sdb.dbAuthGet(
        `/all_profile_search?missionary_profile_key=eq.${doc.missionary_profile_key}`,
        sdb.single()
    );
    ...
    await deleteBlobByPath(doc.storage_path);
}
...
export async function getOrgCompliance(token, submission_period) {
    ...
    let reminderFilter = `/worker_document_reminders_view?missionary_profile_key=in.(${profileKeys.join(',')})&resolved_on=is.null`;
...
export async function sendTestReminderEmail(token, params) {
    ...
    const variables = {
        ...
        uploadUrl: 'https://' + process.env.DOMAIN + '/profile',
        ...
        for (const profile of profiles) {
            ...
            for (const ctx of docTypeContexts) {
                ...
                const docs = await sdb.dbAuthGet(
                    `/worker_documents_view?missionary_profile_key=eq.${profile.missionary_profile_key}&document_type=eq.${docType}&submission_period=eq.${periodStartStr}`
                );

                if (docs && docs.length > 0) {
                    console.log('worker-documents: skip', who, docType, periodStartStr, '— document already submitted');
                    await sdb.dbAuthPatch(
                        `/worker_document_reminders_view?missionary_profile_key=eq.${profile.missionary_profile_key}&document_type=eq.${docType}&submission_period=eq.${periodStartStr}&resolved_on=is.null`,
                ...
                const reminders = await sdb.dbAuthGet(
                    `/worker_document_reminders_view?missionary_profile_key=eq.${profile.missionary_profile_key}&document_type=eq.${docType}&submission_period=eq.${periodStartStr}`
                );
            ...
            for (const p of pending) {
                if (p.reminder) {
                    await sdb.dbAuthPatch(
                        `/worker_document_reminders_view?worker_document_reminder_key=eq.${p.reminder.worker_document_reminder_key}`,
                        {
