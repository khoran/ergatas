...
export class AppState{
    constructor(da,search,storage,server) {
        var self=this;
        self.da=da;
        self.search=search;
        self.storage=storage;
        self.router = undefined;   // set later via setRouter() once the router exists
        self.server = server;
        ...
        self.currencyCountsList = new CachedList(
            () => self.da.currencyCounts(),
            (keys,data) => utils.selectedKeysAsArray(keys,data,"currency","currency",true));

        self.loggedInUser = ko.observable();
        // Set by session.handleDonationResult when a donor returns from a redirect-based payment
        // method with an unfinished ("open") checkout. The matching direct-donation-popup consumes
        // it to re-open and re-mount the embedded checkout so the donor can complete or retry.
        // Payload: {missionary_profile_key, client_secret, stripeAccount}.
        self.pendingDonationResume = ko.observable(null);
    ...
    selectProfile(data){
        ...
        const url = data && data.profile_slug ? 'worker/' + data.profile_slug : 'profile-detail/'+data.missionary_profile_key;
