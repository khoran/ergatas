...
export async function getDonor(customer_id=null, stripe_id=null) {
...
async function sendNoReceiptNotice(checkoutSession){
    ...
    try{
        ...
        await utils.mailingList.sendTemplatedEmail("donation_notice_no_receipt",email,data);
...
async function recordDonation(amount,donation_type,stripe_id,missionary_profile_key){
    ...
    try{
        ...
        if(ptx == null){
            ...
            let donor = await getDonor(stripe_id=stripe_id);
...
export async function populateDonorKeys(){
    ...
    for(const tx of transactions){
        try{
            const donor_key = await getDonor(stripe_id= tx.stripe_id);
            if(donor_key){
...
export async function customerDetails(customerIds,organization_key) {
    const org = await utils.getDB().getOrganization(organization_key);
    let accountHeader = undefined;
    if(org && org.stripe_account )
         accountHeader = {stripeAccount: org.stripe_account};
