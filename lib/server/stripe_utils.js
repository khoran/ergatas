...
async function recordDonation(amount,donation_type,stripe_id,missionary_profile_key,stripeAccount=null){
    ...
    try{
        ...
        if(ptx == null){
            ...
            let donor = await getDonor(stripe_id, null, stripeAccount);
...
export async function syncStripeCurrencies(non_profit_key, stripe_account, donation_settings = null){
    try{
        ...
        if(JSON.stringify(existing) === JSON.stringify(currencies)){
            console.info(`stripe currencies unchanged for non_profit ${non_profit_key}: ${currencies}`);
            return currencies;
        }
        donation_settings = Object.assign({},donation_settings,{stripe_currencies: currencies});
