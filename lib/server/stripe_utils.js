                ...
                sessionData.payment_method_options= {
                    acss_debit:{
                        ...
                        mandate_options:{
                            ...
                            //interval_description: "monthly",
                            default_for:["invoice","subscription"],
                            transaction_type:"personal",
                        }
                    }
                };
            //sessionData.payment_method_options.acss_debit.currency = accountInfo.currency;
            sessionData.currency = accountInfo.currency;
            //sessionData.payment_method_options.acss_debit.mandate_options.payment_schedule="interval";
            //sessionData.payment_method_options.acss_debit.mandate_options.interval_description = "monthly";
...
async function connectedAccountInfo(accountHeader){
    ...
    const activePaymentMethods = Object.entries(account.capabilities).
                ...
                map(cap =>cap[0].replace("_payments","") ).  //get name by removeing '_payments'
   ...
   // // Step 2: Get country spec
   // const countrySpec = await getStripe().countrySpecs.retrieve(country, {
   //     stripeAccount: accountId
   // });
   // console.log("country spec: ",countrySpec);
   // console.log(countrySpec.supported_payment_currencies); 

   // return countrySpec.supported_payment_currencies[0];
...
async function finishSubscriptionSetup(obj){
    ...
    //await getStripe().invoices.update(subscription.latest_invoice.id,{
    //    payment_settings:{
    //        default_mandate: setupIntent.mandate
    //    }
    //},accountHeader);

    //const paymentIntent = await getStripe().paymentIntents.retrieve(subscription.latest_invoice.payment_intent,accountHeader);
    //console.log("payment intent: "+JSON.stringify(paymentIntent,null,"  "));
    //const newPI = await getStripe().paymentIntents.update(subscription.latest_invoice.payment_intent,{
    //    payment_method_options:{
    //        acss_debit:null
    //        //{ mandate_options: null }
    //    }
    //},accountHeader);
    //console.log("payment intent updated: "+JSON.stringify(newPI,null,"  "));
   // //if(paymentIntent.payment_method_options.acss_debit && paymentIntent.payment_method_options.acss_debit.mandate_options){
   // //    delete paymentIntent.payment_method_options.acss_debit.mandate_options;
   // //    console.log("new pm options: ",paymentIntent.payment_method_options);
   // //}

    //const confirmedPI = await getStripe().paymentIntents.confirm(subscription.latest_invoice.payment_intent, {
...
async function sendSubscriptionNotice(checkoutSession,stripeAccount){
    ...
    try{
        ...
        await sendCustomerEmail(checkoutSession.email || checkoutSession.customer_email || checkoutSession.customer_details.email, data,receipt);
