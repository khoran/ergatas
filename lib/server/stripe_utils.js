...
export async function makeDonation(email,worker_name,missionary_profile_key,amount,donation_type,return_url){
...
function buildFailureNotice(data){
    ...
    const textVersion = `We were unable to process your Ergatas donation. \n\n`+
...
async function sendFailureNotice(checkoutSession){
    ...
    try{
        ...
        const missionary_profile_key = checkoutSession.metadata.missionary_profile_key;
        const worker_name= checkoutSession.metadata.worker_name;
        var amount, created, paymentMethod,message;
...
async function getOrCreateProduct(worker_name, missionary_profile_key){
    try{
        ...
        if(searchResults.data.length > 0){
            ...
            var profile = (await utils.getDB().getProfileByKey(missionary_profile_key)).data;
            var images = [];
