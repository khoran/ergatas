...
export async function makeDonation(email,worker_name,missionary_profile_key,amount,donation_type){
    ...
    try{
        ...
        var  sessionData = {
            ...
            customer: customer==null ? undefined: customer.id,
            customer_email: customer==null? email:undefined,
            success_url: `${domain}/worker-donation.html`,
            cancel_url: `${domain}/worker-donation.html`,
            metadata:{
...
export async function handleStripeEvent(body,sig){
    ...
    console.local("handling a stripe event");
    try {
    ...
    switch (event.type) {
        case 'payment_intent.succeeded':
            ...
            console.local("payment_intent.succeeded");
            break;
        case "invoice.created":
            console.log("create invoice. Lines: ",event.data.object.lines);
            break;
        case 'checkout.session.completed': //maybe where we send the receipt. check that payment_status = paid
            ...
            break;
        //TODO: may also need to listen for checkout.session.{async_payment_succeeded, async_payment_failed}
        default:
...
async function getStripeCustomer(email){
    //var results = await stripe.customers.search({
        //query:"email:"+email,
...
async function recordDonation(amount,donation_type,stripe_id,missionary_profile_key){
    ...
    try{
        ...
        const ptx = (await utils.getDB().insertPossibleTransaction(
                        missionary_profile_key,amount,donation_type,stripe_id,confirmed)).data;
...
//TODO: somehow get the workers profile image
    ...
    try{
        ...
        if(searchResults.data.length > 0){
            ...
            return  await stripe.products.create({
                ...
                images:[
                    //not displaying, possibly a CORS issue
...
async function sendCustomerEmail(email,data,reciept){
    try{
        const mailDomain= process.env.MAIL_DOMAIN;
        const settings={
                    to: email,
