...
export async function allCurrencies(){
    const specs = await getStripe().countrySpecs.retrieve("US");

    if(specs != null && specs.supported_payment_currencies != null)
        return  specs.supported_payment_currencies;
    else {
        console.error("failed to get list of currencies, reverting to just USD",specs);
        return ["usd"];
    }
}
    ...
    try{
        await sendCustomerEmail(checkoutSession.email || checkoutSession.customer_details.email,
                                ...
                                { text: "\n\nThank you for your donation to an Ergatas worker! Your payment is being processed. "+
                                        "You will receive a reciept as soon processing is done. For bank transfers, this can take up to 4 business days.\n\n"+
                                        " - The Ergatas Team\n"
                                });
    }catch(error){
