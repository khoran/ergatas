...
export async function buildIndex(pageName,info,url){
    ...
    }else if(pageName === "wiki_page"){

        if(info.page_key != null){
            page = (await getDB().getPageByKey(info.page_key));
            if(page != null){
                info.title = page.title;
                info.description = page.description;
                info.sharing_image = page.sharing_image;
            }
        }else
            console.error("wiki_page requested but no page_key given in info",info);
    }
