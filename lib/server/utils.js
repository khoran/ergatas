...
// dist/asset-manifest.json listing them in load order. Cached for the process
// lifetime — the server restarts on deploy.
var assetManifest;
function getAssetManifest(){
    if(assetManifest == null){
        const __dirname = path.resolve();
        try{
            assetManifest = JSON.parse(fs.readFileSync(`${__dirname}/dist/asset-manifest.json`,'utf-8'));
        }catch(error){
