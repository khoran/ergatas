...
export async function numWorkersForPeopleGroup(peopleID3Code){

    var results = (await getDB().primarySearch({people_id3_codes:[peopleID3Code]},1,"rank,desc")).data;
    //console.local("num people group workers results: ",results);
    if(results != null && results.all_results != null && results.all_results.length != null)
       return results.all_results.length
    else
      return 0;
