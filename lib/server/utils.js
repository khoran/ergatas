...
const forwardMessage = async (email) =>{
    ...
    if(email.sender == null || email.recipient == null){
       console.log("in forwardMessage: missing sender or recipient in email: ",email);
       return;
    }
...
const newsletterSignup = async function(firstName,lastName,email,addToPrayerList,recaptchaScore,dailyPrayer){
    ...
    try{
        ...
        if(dailyPrayer){
           await mailingList.addToGroup(listId,email,"Pray for Missionary of the Day");
        }
...
export async function emailUsersWithDonationClicks(){
   ...
   //for debugging only !!!!!!!!!!!!!!
   start.setMonth(start.getMonth() +1); //set back to this month
   end.setMonth(start.getMonth()); //set back to this month
   end.setDate(30);
