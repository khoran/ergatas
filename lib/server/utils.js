...
export async function pageSlugCache(){
    const pages = (await getDB().getPageSlugs());
    const cache = {};
    ...
    if(pageName==="worker" && url != null){
        try{
            ...
            if(url.startsWith("/worker/")){
                ...
                if(!await getDB().profileSlugExists(slug)){
                    throw "not-found";
                }
                profile = await getDB().getProfileBySlug(slug);
            }else{
                matches = url.match(/\/(\d+)(?:\?.*)?$/);
                if(matches && matches.length >= 2){
                    missionary_profile_key = matches[1];

                    if(!await getDB().profileExists(missionary_profile_key)){
        ...
        try{
            ...
            let page = (await getDB().getPageByKey(info.page_key));
...
export function sitemapUrlsFn(pageInfo){
    ...
    return async function(){
        ...
        try{
            const profiles = (await getDB().getAllProfileSummaries());
