...
import {VRMailingList} from './vr-mailing-list.js';
    ...
    const db = new DataAccess(postgrestBase, async (requestData) =>{
        try{
            ...
            if(response.data != null && response.status >= 200 && response.status < 300)
                ...
                throw AppError("DB request failed with status "+response.status+", "+response.statusText);
        }catch(error){
...
export async function checkProfileUpdates(){
    ...
    //disable accounts
    var profilesToDisable = (await getDB().profilesByLastUpdate(disableDate.toISOString())).data
...
export async function inviteProfileOwner(ownerName,ownerEmail,adminEmail,profile_key,adminUserId,token){
    try{
        ...
        if(owner != null){
            //then auto accept
            setProfileOwner(token,owner.id,profile_key);
            return;
        }
        

        const adminName = user.fullName || `Admin';`
        const userPerms = (await getDB().getUserOrgSearchFilter(adminUserId)).data;
        //console.log("got perms: ",userPerms);

        const org = (await getDB().getOrganization(userPerms.organization_key)).data;
...
If you’d like to be able to manage this profile yourself in the future, please create a free account on ergatas.org using the email address ${ownerEmail}. Then visit your dashboard to automatically claim your profile:
...
export async function claimProfile(token){
    const payload = utils.jwtPayload(token);
    const invitation = (await getDB(token).getProfileInvitations()).data;
    if(invitation != null && invitation.email === payload.email){
        console.log("claiming invite ",invitation);
        setProfileOwner(token,payload.sub,invitation.missionary_profile_key);
    }
}
export async function setProfileOwner(token,userId,profile_key){
    console.log("setting owner of profile "+profile_key+" to "+userId);
    const profile = (await getDB(token).getProfileByKey(profile_key)).data;
    console.log("got existing profile: ",profile);
    const user = (await getDB().getUserInfoByUserId(userId)).data;
...
export async function assignNewProfilePermissions(userId,missionary_profile_key){
    ...
    const filter = (await getDB().getUserOrgSearchFilter(userId)).data;
    ...
    //await getDB().updateUserOrgSearchFilter(userId,filter);
...
export async function getMissionaryProfile(missionary_profile_key){
    return (await getDB().getDisplayProfileByKey(missionary_profile_key)).data;
}
...
export async function orgSlugCache(){
    const orgs = (await getDB().organizationList()).data;
    const cache = {};
...
//    //TODO: Cache slugs here. reload on a miss
//    const org = (await getDB().getOrganizationBySlug(slug)).data;
//    console.log("org by slug? ",org);
//
//    return org;
//}
    ...
    if(pageName==="profile-detail" && url != null){
        try{
            ...
            if(matches.length >= 2){
                ...
                profile = (await getDB().getDisplayProfileByKey(missionary_profile_key)).data;
...
export async function cleanUpOldUsers(dryRun=true,count=1){
    ...
    const tenantId = process.env.AUTH_TENANT_ID;
    const usersWithoutEntities = (await getDB().getUsersWithoutEntities()).data;
...
export async function pickPrayerCardDrawingWinner(){
    try{
        ...
        var user= (await getDB().getUserForPrayerCardDrawing()).data;
        console.log("prayer card winner: ",user);

        //const user = (await getDB().getUserInfoByKey(user_key)).data;
        const email = await userIdToEmail(user.external_user_id);
        const body = `
${user.first_name},
...
export async function sendDonationNotice(possible_transaction_key,external_user_id){
    ...
    try{
        ...
        const tx = (await getDB().getPossibleTransaction(possible_transaction_key)).data;
        console.log("tx: ",tx);

        const profile = (await getDB().getDisplayProfileByKey(tx.missionary_profile_key)).data;
        ...
        const data = {
            ...
            to: "information@"+mailDomain, //TODO: REMOVE THIS FOR PRODUCTION
            //to:  email,
...
export async function claimOrg(organization_key, church_name, church_website,user_key,read_only,email, userId,adminName){
    ...
    const mailDomain= process.env.MAIL_DOMAIN;
    const org = (await getDB().getOrganization(organization_key)).data;
...
export async function getManagedProfiles(external_user_id){
    ...
    const filter = (await getDB().getUserOrgSearchFilter(external_user_id)).data;
    ...
    if(user_key == null){
        ...
        if( ! filter.read_only)
            await getDB().cacheUserPermissions(user_key,Object.keys(allProfiles));
    }
...
export async function grantUserOrgPermission(user_key, organization_key, read_only){
  ...
  try{
    ...
    const org = (await getDB().getPlainOrganization(organization_key)).data;
