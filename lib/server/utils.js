...
export async function cleanUpOldUsers(dryRun=true,count=1){
    ...
    console.log("found "+usersWithoutEntities.length+" profileless users");

    console.log("cleaning up users. dryRun? "+dryRun+", count="+count);
    ...
    for( let user of usersWithoutEntities.slice(0,count)){
        ...
        console.log((dryRun?"DRY RUN":"LIVE")+" ----- considering user ------------",user);

        //skip any user created in the last 30 days
        if(new Date(user.user_created).getTime() > minAge.getTime()){
            console.log("    skipping user less than 30 days old",user.user_created,minAge);
            continue;
        }
        ...
        try{
            ...
            if(email != null){
                ...
                if(mailingList.userExists(user.email)){
                    console.log("    found user in mailing list",user.first_name, user.last_name, user.user_key)
    ...
    console.log("mailing list emails: ",mailingListEmails);

    for(let email of mailingListEmails){
        //get user from authdb
        email = email.email_address.toLowerCase();

        console.log((dryRun?"DRY RUN":"LIVE")+" ----- considering email: ",email);
        try{
            const authUser= await fusionRequest("GET","/api/user?email="+email,null,
                                        {"X-FusionAuth-TenantId":tenantId});
            console.log("    found mailing list user in auth db",email,authUser.id);
