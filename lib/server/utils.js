import axios from 'axios';
import path from 'path';
import Mailgun from 'mailgun.js';
import FormData from 'form-data';
import { SignJWT, jwtVerify, errors } from 'jose';
import { AppError,WebError } from './app-error.js';
import crypto from 'crypto';
import { DataAccess } from '../shared/data-access.js';
import fs from 'fs';
import {ListMonk} from './listmonk.js';
import cheerio from "cheerio";
import serveStatic from 'serve-static';
import { BlobServiceClient } from '@azure/storage-blob';
import {getProfilesForFilter,hydrateWikiPage} from '../shared/shared-utils.js';
import webPush  from 'web-push';
import {BetaAnalyticsDataClient} from '@google-analytics/data';
import * as stripeUtils from './stripe_utils.js';
import * as embeddings from './embeddings.js';

var mailgun;
export var mailingList;

export const messagingAdminEmail = "messaging-admin@ergatas.org";

export async function init(){
    //mailingList = new VRMailingList(process.env.VR_ACCESS_TOKEN,process.env.VR_SERVER_URL);
    mailingList = new ListMonk(process.env.LISTMONK_USERNAME,
                               process.env.LISTMONK_PASSWORD,
                               process.env.LISTMONK_SERVER_URL );
    await mailingList.loadTemplates();

}


function getMailgun(){
    if(mailgun == null){
        const api_key = process.env.MAILGUN_API_KEY;
        const domain = process.env.MAILGUN_DOMAIN;
        if(api_key == null || api_key === '')
            throw new AppError("no api_key defined for Mailgun");
        if(domain == null || domain === '')
            throw new AppError("no domain defined for Mailgun");

        const MG= new Mailgun(FormData);
        mailgun = MG.client( {
            username: 'api',
            key: api_key,
        });
          //  apiKey: api_key, domain: domain});
    }
    return mailgun;
}
export async function sendEmail(data){
    try{
        await getMailgun().messages.create(process.env.MAILGUN_DOMAIN, data);
    }catch(error){
        console.error("failed to send email to "+data.to+". "+error.message,error);
        throw new AppError("failed to send email to "+data.to+". "+error.message);
    }
}
export async function sendTemplatedEmail(templateName,toEmail,variables){
    const domain= process.env.DOMAIN;
    const data = {
        from: "information@"+domain,
        to: toEmail,
        bcc: "info@ergatas.org",
        template: templateName,
        'h:X-Mailgun-Variables': JSON.stringify(variables),
    };
    return await sendEmail(data);
}
function fusionRequest(type,path,data,headers){
    const fusionUrl= process.env.FUSION_BASE_URL;
    const key = process.env.FUSION_USER_INFO_KEY;

    if(headers == null)
        headers = {};

    headers["Authorization"] = key;

    return axios({
        method: type,
        url: fusionUrl+path,
        data: data,
        headers: headers,
    });

}
export async function loginDataFromToken(token){
    
    const payload = await jwtPayload(token);
    //console.log("token payload: ",payload);

    return {
        access_token: token,
        roles: payload.roles,
        email: payload.email,
        userId: payload.sub,
        expires: payload.exp,
    };
}
export async function jwtPayload(token){
    const secret = process.env.JWT_SECRET;
    //console.local("token:",token);
    if(secret == null || secret==="")
        throw new AppError("no secret given, cannot verify JWT");

    const { payload } = await jwtVerify(token, new TextEncoder().encode(secret));
    return payload;

}
export async function getJWT(code){

    const authTokenUrl= process.env.AUTH_TOKEN_URL_BASE;
    const redirectUrl = process.env.REDIRECT_URL || "localhost";
    const authClientId= process.env.AUTH_CLIENT_ID;

    if(code == null || code === ""){
        throw new AppError("no code given");
    }

    return axios.post(authTokenUrl,
        "grant_type=authorization_code&"+
        "client_id="+authClientId+"&"+
        "code="+code+"&"+
        "redirect_uri="+redirectUrl
    ,{
        headers: {
            "Content-Type":"application/x-www-form-urlencoded",
        }
    }).then(async (response)=>{
        //console.local("login data: ",response.data);
        return {
            refresh_token: response.data.refresh_token,
            clientData: await loginDataFromToken(response.data.access_token)
        };
    }).catch((error) =>{
        console.error("caught error: ",error);
        throw error;
    });

};
export async function refreshJWT(refresh_token){
    const authTokenUrl= process.env.AUTH_TOKEN_URL_BASE;
    const authClientId= process.env.AUTH_CLIENT_ID;

    return axios.post(authTokenUrl,
        "grant_type=refresh_token&"+
        "client_id="+authClientId+"&"+
        "refresh_token="+refresh_token
    ,{
        headers: {
            "Content-Type":"application/x-www-form-urlencoded",
        }
    }).then(async (response)=>{
        return await loginDataFromToken(response.data.access_token);
    });

}
export async function requireRole(req,role){

    const payload = await jwtPayload(req.body.token);
    if(payload == null || payload.roles == null || ! payload.roles.includes(role))
      throw new AppError("Not authorized");

}
export function getDB(token) {
    const postgrestBase= process.env.POSTGREST_SERVER_URL_BASE;
    
    //const db = new DataAccess(postgrestBase,axios);
    const db = new DataAccess(postgrestBase, async (requestData) =>{
        try{
            const response = await axios(requestData)
            //console.log("DB response: ",response);
            if(response.data != null && response.status >= 200 && response.status < 300)
                return response.data;
            else
                //console.log("DB ERROR: ",response);
                throw new WebError("DB request failed with status "+response.status+", "+response.statusText,response.status);
        }catch(error){
            //console.log("DB ERROR: ",error);
            var message;
            if(error.response){
                const r=error.response;
                if(r.status === 401) //auth token bad or expired
                    throw new errors.JWTExpired('jwt expired')
                message = `status: ${r.status}, ${r.statusText}. ${r.config.url} data: ${JSON.stringify(r.data,' ')}`;
            }else
                message = error;
            throw new WebError("DB request failed: "+message,error.response ? error.response.status : 500);
        }
    });
    db.setToken(token);
    return db;
}
export async function getServerDB() {
    const secret = process.env.JWT_SECRET;
    const token = await new SignJWT({ role: 'ergatas_server', roles: ['ergatas_server'] })
        .setProtectedHeader({ alg: 'HS256' })
        .setExpirationTime('60s')
        .sign(new TextEncoder().encode(secret));
    return getDB(token);
}

// Generate/refresh a profile's semantic-search embedding and store it in
// web.profile_embeddings. Runs server-side (ergatas_server role) so it works
// regardless of who triggered the save. Returns true if an embedding was
// written, false if there was no text to embed.
export async function updateProfileEmbedding(missionary_profile_key){
    const db = await getServerDB();
    let text = await db.getProfileEmbeddingText(missionary_profile_key);
    // PostgREST returns a scalar text function as a plain string; guard anyway.
    if(text != null && typeof text !== 'string'){
        if(Array.isArray(text)) text = text[0];
        if(text != null && typeof text === 'object') text = Object.values(text)[0];
    }
    if(text == null || String(text).trim() === ""){
        console.warn("no embedding text for profile "+missionary_profile_key+", skipping");
        return false;
    }
    const vector = await embeddings.embedText(String(text), "document");
    await db.upsertProfileEmbedding(missionary_profile_key,
                                    embeddings.toVectorLiteral(vector));
    return true;
}


export async function fixBlobMimeTypes() {

    //const uploadBucketName = process.env.UPLOAD_BUCKET ;
    const uploadBucketName = "ergatas-public-content";
    const connectionString = process.env.AZURE_STORAGE_CONNECTION_STRING;
    const blobServiceClient = BlobServiceClient.fromConnectionString(connectionString);
    const bucket = blobServiceClient.getContainerClient(uploadBucketName);

    
    var i=0;
    for await (const blob of bucket.listBlobsFlat()) {
        i=i+1;
        //console.log(`name: ${blob.name}, content-type: `,blob.properties.contentType);
        if(blob.properties.contentType==="application/octet-stream"){

            //if( ! blob.name.endsWith("Butlers-in-Europe-2017.jpg"))
                //continue;

            console.log("fixing "+blob.name);

            var blockBlob = bucket.getBlockBlobClient(blob.name);
            var httpOptions = {
                blobContentMD5: blob.properties.contentMD5,
            };

            if(blob.name.match(/\.(jpg|jpeg)$/i)){
                console.log("found jpg");
                httpOptions.blobContentType= 'image/jpeg';
            }else if(blob.name.match(/\.png$/i)){
                console.log("found png");
                httpOptions.blobContentType= 'image/png';
            }else{
                continue;
            }

            //console.log("options: ",httpOptions);
            blockBlob.setHTTPHeaders(httpOptions);

        }
    }
    console.log("found "+i+" blobs");

}



function checkProfileBucketPermissions(userId,missionary_profile_key){
    console.log("checking profile bucket permissions ",userId, missionary_profile_key);
    if(missionary_profile_key == null){
        console.log("using userId");
        return userId;
    }else if(userHasPermissionOnProfile(userId, missionary_profile_key)){ // use MPK prefix
        console.log("using mpk prefix");
        return "MPK"+missionary_profile_key;
    }else{  //not permitted
        console.error("file not deleted for user "+userId+", either no profile key given, or user not authorized");
        return null;
    }
}
const THUMBNAIL_POSTFIX = "-ergatas-thumbnail.png";

// The upload assembly stores a file's thumbnail next to the original, named
// "<basename>.png-ergatas-thumbnail.png" (matches the lookup in file-collection.js).
// Returns null when no thumbnail could exist for this blob.
function thumbnailBlobPath(blobPath) {
    const slash = blobPath.lastIndexOf("/");
    const dir  = blobPath.slice(0, slash + 1);
    const name = blobPath.slice(slash + 1);
    const baseNoExt = name.replace(/\.[^.]+$/, "");
    return dir + baseNoExt + ".png" + THUMBNAIL_POSTFIX;
}
export async function deleteBlobByPath(blobPath) {
    return getUploadBucket().deleteBlob(blobPath);
}
// Deletes a blob along with its thumbnail (if one was generated). deleteIfExists
// tolerates files that never had a thumbnail (e.g. .ods, which isn't convertible).
export async function deleteBlobAndThumbnailByPath(blobPath) {
    const bucket = getUploadBucket();
    try {
        await bucket.getBlockBlobClient(thumbnailBlobPath(blobPath)).deleteIfExists();
    } catch (err) {
        console.error("failed to delete thumbnail for " + blobPath, err.message);
    }
    return bucket.deleteBlob(blobPath);
}
export async function removeFile(userId,filename,missionary_profile_key=null){
    const prefix = checkProfileBucketPermissions(userId,missionary_profile_key);
    return deleteBlobAndThumbnailByPath(prefix+"/"+filename);
}
export async function removeAllFiles(userId,missionary_profile_key=null){
    const bucket = getUploadBucket();
    const prefix = checkProfileBucketPermissions(userId,missionary_profile_key);
    var files = await getUserBucketFiles(prefix);
    return Promise.all(files.map( (file) =>{
        return bucket.deleteBlob(file.name);
    }));
}
export async function getUserBucketFiles(prefix,bucket) {
    const userBucket = bucket || getUploadBucket();
    var blobs = [];

    for await (const blob of userBucket.listBlobsFlat({prefix:prefix+"/"})) {
        blobs.push(blob);
    }

    return blobs;
}
function getUploadBucket(){
    const uploadBucketName = process.env.UPLOAD_BUCKET ;
    const connectionString = process.env.AZURE_STORAGE_CONNECTION_STRING;
    const blobServiceClient = BlobServiceClient.fromConnectionString(connectionString);
    return blobServiceClient.getContainerClient(uploadBucketName);
}
export async function userFileLinks(prefix){
    const bucket = getUploadBucket();
    var files = await getUserBucketFiles(prefix);
    var filenames = files.map(file => 
        {
            return {
                name: file.name.replace(prefix+'/',"" ),
                link: bucket.url+"/"+file.name,
                date: (file.properties 
                        && file.properties.lastModified 
                        && new Date(file.properties.lastModified)) 
                       || new Date("2020-01-01"),
            };
        });
    //reverse sort by date
    filenames.sort((a, b) => b.date - a.date);

    //console.log("filenames: ",filenames);
    return filenames;
}
export async function changeToMPKPrefix(missionary_profile_key){
    //get profile, then user_id from user 
    // get list of files
    // copy to new prefix
    // set use_mpk_prefix to true and save
    // delete old prefix files
    
    try{
        const secret = process.env.JWT_SECRET;
        const token = await new SignJWT({
                //role: 'ergatas_web',
                role: 'ergatas_org_admin',
                sub:"a1da7bec-d229-4da7-8b74-0d5a2448a41a"
            })
            .setProtectedHeader({ alg: 'HS256' })
            .setExpirationTime('60s')
            .sign(new TextEncoder().encode(secret))




        const profile = await getDB(token).getProfileByKey(missionary_profile_key);
        //console.log("profile: ",profile);
        if(profile == null) throw AppError("Failed to get profile with key "+missionary_profile_key);

        if(profile.data.use_mpk_prefix === true){
            console.log("profile "+missionary_profile_key+" already set to use mpk prefix");
            return;
        }

        const _sdb = await getServerDB();
        const user = await _sdb.getUserInfoByKey(profile.user_key);
        //console.log("user: ",user);
        if(user == null) throw AppError("failed to get user with key "+profile.user_key);

        const oldPrefix = user.external_user_id;
        const newPrefix = "MPK"+missionary_profile_key;
        console.log(`Old prefix: ${oldPrefix}, new prefix: ${newPrefix}`)
        const bucket = getUploadBucket();
        var files = await getUserBucketFiles(oldPrefix,bucket);
        var blobClient;
        for(const file of files){
            const name =  file.name.replace(oldPrefix+'/',"");
            const sourceURL=  bucket.url+"/"+file.name;
            console.log(`copying from ${sourceURL} to ${newPrefix}/${name}`);
            blobClient = bucket.getBlobClient(newPrefix+"/"+name);
            await blobClient.syncCopyFromURL(sourceURL);
        }
        profile.data.use_mpk_prefix = true;
        profile.data.picture_url = profile.data.picture_url.replace(oldPrefix,newPrefix);
        await getDB(token).updateProfile(missionary_profile_key,profile);

        for(const file of files){
            console.log("deleting file "+file.name);
            await bucket.deleteBlob(file.name);
        }

    }catch(error){
        console.error("failed to copy files to new prefix for profile "+missionary_profile_key,error);
        throw error;
    }

}
export async function nonProfitSearch(query,state){
    var ein,url;

    if(/[-0-9]+/.test(query)){
        // query is in format of EIN, either all numbers or xx-xxxxxx
        ein = parseInt(query.replace(/-/g,""));
        url = "https://projects.propublica.org/nonprofits/api/v2/organizations/"+ein+".json";
    }else{
        url = "https://projects.propublica.org/nonprofits/api/v2/search.json?c_code[id]=3&q="+query;

        if(state){
            url = url + "&state%5Bid%5D="+state;
        }
    }

    return axios.get(url).
        then(function(response){
            //if ein query type
            if(response.data.organization != null){
                if(ein != null)
                    response.data.organization.displayEIN = query; // so it matches what the user typed in
                return {
                    organizations: [ response.data.organization ]
                };
            }else
                return response.data;
        }).catch( (error) =>{
            console.warn(" nonprofit query failed for "+query);
            //return empty result
            return {
                organizations: []
            };
        });

};
export async function notifyOrgApplication(user_key,external_user_id, organization_key){
    const orgNotifyEmail = process.env.ORG_APP_NOTIFY_EMAIL;
    const baseUrl = process.env.REDIRECT_URL;
    var user,email,data,body;
    const mailDomain= process.env.MAIL_DOMAIN;

    try{
        email = await userIdToEmail(external_user_id);
        console.log("email: "+email);
    }catch(error){
        console.warn("failed to fetch email for user id "+external_user_id,error);
        email="email not found";
    }
    try{
        data = {
            from: "information@"+mailDomain,
            to: orgNotifyEmail,
            subject: "New Organization Application. org key: "+organization_key,
            text: "User "+user_key+", "+email+", has submitted a  new organization for approval.\n "+
                    baseUrl+"organization-review/"+organization_key,
        };
        body = await sendEmail(data);
        console.log("org application notification email sent!");
    }catch(error){
        console.error("org application notification email failed: ",error);
        throw new AppError("failed to send org application notification message");
    }

};
export async function updateJPWorkerCache(joshuaProject){

    var peopleGroups = (await (await getServerDB()).peopleGroupsWithWorkers()).map(x=>x.code);
    var countries = (await (await getServerDB()).countriesWithWorkers()).
                              map(x=>x.code.toLowerCase());
    console.log("fetched JP worker data: ",peopleGroups,countries);
    joshuaProject.updateWorkerCache(peopleGroups,countries);

};
export async function notifyOrgUpdate(token,organization_key,message){
    //fetch list of external_user_ids from organization_listeners_to_notify
    // send either approved or denied email
    
    const org = (await getDB(token).getOrganization(organization_key));
    const mailDomain= process.env.MAIL_DOMAIN;
    //console.log("notifyOrgUpdate: org ",org);
    var emailHtml;
    var emailText;
    //console.log("org status: "+org.status);
    if(org.status === "approved"){
        emailHtml= fs.readFileSync(process.cwd()+"/lib/snippet-templates/org-approved.html","utf8");
        emailText = "Your organization application has been approved! "+
                " To create your profile go to ergatas.org/profile .";
    }else if(org.status === "denied"){
        emailHtml= fs.readFileSync(process.cwd()+"/lib/snippet-templates/org-denied.html","utf8");

        const $ = cheerio.load(emailHtml);

        emailText =  message || "Unfortunately, we don't feel that this organization sufficiently agrees with our statement of faith, and "+
                  "is therefore not eligible to be included on Ergatas. If you feel this is an error, feel free to reply to "+
                  "this email explaining why. You can also review our statement of faith at ergatas.org/sof .";
        $("#message_block").append(emailText);
        emailHtml = $.html();

    }else{
        throw new AppError("organization "+organization_key+" has not been marked as approved or denied yet");
    }

    const emailData = {
        from: "information@"+mailDomain,
        subject: "Ergatas Application for "+org.name,
        html: emailHtml,
        text: emailText
    };
    const users = (await getDB(token).selectOrganizationListeners(organization_key));
    for(var i in users){
        var user = users[i];
        console.info("sending org status update email to "+user.external_user_id);
        var email = await userIdToEmail(user.external_user_id);
        emailData.to = email;
        await sendEmail(emailData);
       // console.log("done sending to "+user.external_user_id);
    }
    //delete listeners
    await getDB(token).deleteOrganizationListeners(organization_key);

}

export function recordLokiLog(indexName,logs){
    const lokiURL = process.env.LOKI_URL

    if(lokiURL == null)
        return;
        
    var data = {
        streams:logs.map(log => {
                return {
                    stream:{
                        app:indexName,
                        severity:log.level,
                    },
                    values: [[String(log.timestamp*1000000),JSON.stringify(log)]],
                };
            })
    };
    //console.local("loki log: ",JSON.stringify(data));
    return axios.post(lokiURL+"/loki/api/v1/push",data,
        {
            headers: {
                "Content-Type":"application/json",
            }
        }).then((response)=>{
            if(response.error != null && response.error.reason != null){
                throw new AppError("Failed to record batch of log messages: "+response.error.reason);
            }
        }).catch((error) =>{
            console.error("Failed to record batch of log messages to "+lokiURL+", ",error.message);
            //throw error;
        });

}
export async function validateRecaptcha(token,action,remoteIp){

    const verifyUrl = process.env.GOOGLE_RECAPTCHA_VERIFY_URL;
    const secret = process.env.GOOGLE_RECAPTCHA_SECRET_KEY;
    return axios.post(verifyUrl,
        "secret="+secret+"&"+
        "response="+token+"&"+
        "remoteip="+remoteIp
    ).then((response)=>{
        //console.log("recaptcha response: ",response.data);
        if(response.data == null)
            throw new AppError("request to verify recaptcha token failed. no data returned");
        if(response.data.success === false)
            throw new AppError("request to verify recaptcha token failed. success was false");
        if(response.data.action != null && response.data.action !== action)
            throw new AppError("request to verify recaptcha token failed. returned action did not match given action: "+
                                response.data.action+" != "+action);
        return response.data.score;
    });
    
}

async function getFusionUser(userId){
    const response=await fusionRequest("GET","/api/user/"+userId);
    return response.data.user;
}
async function getFusionUserByEmail(email){
    const tenantId = process.env.AUTH_TENANT_ID;
    try{

        return (await fusionRequest("GET","/api/user?email="+email,null,
                                        {"X-FusionAuth-TenantId":tenantId})).data.user;
    }catch(error){
        return null;
    }
}
async function updateFusionUser(userId,email,user){
    try{
        user.email=email; //required field
        const response = await fusionRequest("PATCH","/api/user/"+userId,{user:user});
    }catch(error){
        console.error("failed to update fusion user: ",error);
    }
}
export async function userIdToEmail(userId){
    console.local("fetching email for user id "+userId);
    try{
        const user = await getFusionUser(userId);
        return user.email;
            //then((response)=>{
                ////console.local("user information response: ",response.data,response.data.user);
                //return response.data.user.email;
            //});
        //return x;
    }catch(error){
        //console.local("LOCAL failed to fetch email for userId "+userId,error);
        console.error("failed to fetch email for userId "+userId,error && error.response && error.response.data);
        throw error;
    }
}
export async function getUserEmails(userIds){
    //console.local("fetching emails for users: ",userIds);
    return Promise.all(userIds.map(async (userId) =>{
        var email="";
        try{
            email = await userIdToEmail(userId);
        }catch(error){
            console.warn("failed to translate email for exernal_user_id "+userId,error.message);
        }
        return {
            external_user_id: userId,
            email:email,
        }
    }));
        
}
export async function isUserVerified(userId){
    const user = await getFusionUser(userId);
    return {verified:  user.verified};
   // return fusionRequest("GET","/api/user/"+userId).
   //     then((response)=>{
   //         return {verified:  response.data.user.verified};
   //     });
}
export function resendVerifyEmail(email){
    const tenantId = process.env.AUTH_TENANT_ID;
    return fusionRequest("PUT","/api/user/verify-email?email="+email,null,{
        "X-FusionAuth-TenantId":tenantId
    });
}

function emailToHash(emailAddress){
    const mailDomain= process.env.MAIL_DOMAIN;
    return "user_"+crypto.createHash("md5").update(emailAddress).digest("hex")+"@"+mailDomain;
}
async function saveEmailMapping(emailAddress,hash){
    console.info("saving map from "+emailAddress+" to "+hash);

   await (await getServerDB()).insertEmailHashMapping(emailAddress,hash);

}
async function hashToEmail(hashedEmail){
    console.info("fetching real email for hash "+hashedEmail);
    try{

        var hashes = await (await getServerDB()).getEmailHash(hashedEmail)
        if(hashes != null && hashes.email_address != null){
            console.info("found real email: "+hashes.email_address);
            return hashes.email_address;
        }
    }catch(error){
        console.error("true email not found for hashed email "+hashedEmail,error);
        throw new AppError("true email not found for hashed email "+hashedEmail+"\n"+JSON.stringify(error));
    }
    
}
export async function contact(fromEmail, fromName, message,toEmail){


    const fromHash=emailToHash(fromEmail);
    const toHash=emailToHash(toEmail);

    await saveEmailMapping(fromEmail,fromHash);
    await saveEmailMapping(toEmail,toHash);

    const data = {
        from: "Ergatas - "+fromName+" <"+fromHash+">",
        to: toEmail,
        bcc: messagingAdminEmail,
        subject: fromName+" sent you a message on Ergatas",
        text: message,
        html: message.replace(/\n/g,"<p>")
    };
    console.info("first contact mailgun data: ",data);
    try{
        var body = await sendEmail(data);
        console.info("message email sent!");
    }catch(error){
        console.error("failed to send message from user to missionary. "+error.message,error);
        throw new AppError("failed to send message");
    }
}

export async function forwardMessage(email){
    console.info("forwarding message from "+email.sender+" to "+email.recipient);
    if(email.sender == null || email.recipient == null){
       console.error("in forwardMessage: missing sender or recipient in email: ",email);
       console.error("email data: "+JSON.stringify(email));
       return;
    }
    // lookup hashed 'to' address in database to get true email address
    // also rewrite 'from' to be hashed
    const realToEmail = await hashToEmail(email.recipient);
    const hashedFrom = emailToHash(email.sender);
    //make sure we save this mapping so when they reply we know who they are
    await saveEmailMapping(email.sender,hashedFrom);

    const fromName = email.From.replace(/(.*?) <.*>$/,"$1") || "";
    console.info("From : "+email.From+", extracted name: "+fromName);
    if(email['attachment-count'] != null){
        console.info("attachments found: "+email['attachment-count'])
    }
    const data = {
        //from: hashedFrom,
        from: "Ergatas - "+fromName+" <"+hashedFrom+">",
        to: realToEmail,
        bcc: messagingAdminEmail,
        subject: email.subject,
        text: email['body-plain'],
        html: email['body-html'],
    };
    console.info("forward mailgun data: ",data);

    try{
        var body = await sendEmail(data);
        console.info("message email sent!");
    }catch(error){
        console.error("failed to send message from user to missionary. "+error.message,error);
        throw new AppError("failed to send message");
    }

}
// removes files in azure, mailing list entry, and authdb entry
export async function deleteUser(userId,email,token,missionary_profile_key){
    console.info("DELETING user "+userId,missionary_profile_key);

    //delete from database
    try{
        //see what profiles are still owned by user
        const perms = await (await getServerDB()).getUserOrgSearchFilter(userId);

        //perms will be null if user is not a profile manager permission on something
        if(perms != null && perms.owned_profile_keys != null && perms.owned_profile_keys.length > 0){
            return {error:true,
                    message: "We cannot remove your account while you still own some profiles."+
                        " Please either delete each profile first, or invite the worker to take over their profile.",
                    owned_profile_keys: perms.owned_profile_keys};
        }else if(missionary_profile_key != null){ //normal user, delete their profile first
            await getDB(token).deleteProfile(missionary_profile_key);
        }

        await getDB(token).deleteUser(userId);


        await removeAllFiles(userId);

        fusionRequest("DELETE","/api/user/"+userId+"?hardDelete=true");
    }catch(error){
        console.error("failed to delete user "+userId+" from DB: ",error);
        return {
            error:true,
            message: "Sorry, an error occurred while removing your account. Please contact us.",
        }
    }


    //delete from MailChimp
    try{
        console.log("removing user "+userId+" from mailchimp");

        await mailingList.removeUser(email);
    }catch(error){
        console.warn("failed to remove user from mailing list: "+(error.message || error),error);
    }

    return {};
}
async function deleteUserFromDB(user_key){

    try{
        await (await getServerDB()).deleteUser(user_key);
    }catch(error){
        console.error("failed to delete user_key "+user_key+" from db: ",error);
    }
}
export async function checkProfileUpdates(){
    const disableDays = 2*365; // 2 years
    const warning1Days = disableDays - 60;
    const warning2Days = disableDays - 30;
    const days = 24*60*60*1000

    const now= new Date();
    const warning1Date = new Date(now - warning1Days*days);
    const warning2Date = new Date(now - warning2Days*days); 
    const disableDate = new Date(now - disableDays*days);
    const stats = {
        disabled: {success: 0, failed:0},
        warning1Sent: {success: 0, failed:0},
        warning2Sent: {success: 0, failed:0},
    };
    console.info("now: "+now.toISOString());
    console.info("warning 1 date: "+warning1Date.toISOString());
    console.info("warning 2 date: "+warning2Date.toISOString());
    console.info("disable date: "+disableDate.toISOString());


    var processProfile = ( newState,emailNoticeFn,stat)=>{
        console.log("creating function for state "+newState);
        return async (profile) =>{
            try{
                console.info("======= processing profile for targetState "+newState,profile);
                await Promise.all([
                    emailNoticeFn(profile.external_user_id),
                    (await getServerDB()).updateProfileState(profile.missionary_profile_key,newState),
                ]);
                stat.success = stat.success + 1;
            }catch(error){
                console.error("failed to move profile to state "+newState+", userId: "+profile.external_user_id+
                                ". "+error.message);
                stat.failed = stat.failed + 1;
            }
        };
    }


    //disable accounts
    var profilesToDisable = (await (await getServerDB()).profilesByLastUpdate(disableDate.toISOString()))

    //console.log("profiles to disable: ",profilesToDisable);
    if(profilesToDisable != null)
        await Promise.all(profilesToDisable.
                    map(processProfile("disabled",emailDisableNotice,stats.disabled)));


    //warning 2 accounts
    var warning2Profiles= (await (await getServerDB()).profilesByLastUpdate(warning2Date.toISOString()))
    //console.log("profiles in warning2: ",warning2Profiles);
    if(warning2Profiles != null)
        await Promise.all(warning2Profiles.
                    filter(profile => profile.state === "warning1").
                    map(processProfile("warning2",emailWarning2Notice,stats.warning2Sent)));


    //warning 1 accounts
    var warning1Profiles= (await (await getServerDB()).profilesByLastUpdate(warning1Date.toISOString()))
    //console.log("profiles in warning1: ",warning1Profiles);
    if(warning1Profiles != null)
        await Promise.all(warning1Profiles.
                    filter(profile => profile.state === "current").
                    map(processProfile("warning1",emailWarning1Notice,stats.warning1Sent)));

    console.info("stats: ",stats);
    return stats;

}
function emailWarning1Notice(userId){
    return emailUpdateNotice(userId,"update-reminder-1.html",
        "It looks like it's been over a year since your last profile update! "+
        " For best results, it's important to keep your profile up to day, particularly your current support level. "+
        " Go to ergatas.org/profile to make your updates. If nothing has changed, just click the 'Save' button to indicate that."+
        " If no updates are made for 2 years, your profile will no longer be shown in search results. "
    );
}
function emailWarning2Notice(userId){
    return emailUpdateNotice(userId,"update-reminder-2.html",
        "It looks like it's been over a year since your last profile update! "+
        " For best results, it's important to keep your profile up to day, particularly your current support level. "+
        " Go to ergatas.org/profile to make your updates. If nothing has changed, just click the 'Save' button to indicate that."+
        " If no updates are made for 2 years, your profile will no longer be shown in search results. "
    );
}
function emailDisableNotice(userId){
    return emailUpdateNotice(userId,"profile-disabled.html",
        "Oh no! It looks like it's been over 2 years since your last profile update! "+
        " Your profile has now been disabled and will not be shown in search results. "+
        " To re-activate your profile, go to ergatas.org/profile, make any updates, then click 'Save'"
    );
}
async function emailUpdateNotice(userId,templateName,textVersion){
    var email ;
    var emailTemplate;
    const mailDomain= process.env.MAIL_DOMAIN;
    
    try{
        email = await userIdToEmail(userId);
        console.log("got user email: "+email);
    }catch(error){
        console.error("failed to get email for user id "+userId+". "+error.message);
        return;
    }
    try{
        //console.log("snippet path: "+process.cwd()+"/lib/snippet-templates/"+templateName);
        emailTemplate  = fs.readFileSync(process.cwd()+"/lib/snippet-templates/"+templateName,"utf8");
    }catch(error){
        console.error("failed to get email template "+templateName+" while sending notice to user id "+userId+
            ", "+error.message);
        return;
    }


    const data = {
        from: "information@"+mailDomain,
        to: email,
        bcc: "update-reminders@"+mailDomain,
        subject: "Action Needed - Ergatas Profile Update Reminder",
        text: textVersion,
        html: emailTemplate,
    };
    //console.log("update notice mailgun data: ",data);

    try{

        if(process.env.NODE_ENV === "development")
            console.local("development mode, otherwise would have sent email to : ",data.to);
        else
            await sendEmail(data);
        console.info("update notice message email sent to "+userId);
    }catch(error){
        console.error("failed to send update notice email to userId"+userId+", with email "+email+
            ". "+error.message,error);
        throw new AppError("failed to send update notice message");
    }

}

export async function addUserToMailinglist(email){
    console.log("adding new user to mailing list ",email);

    try{
        await mailingList.ensureUser(email);
        await mailingList.addTags(email,["User"]);
    }catch(error){
        console.warn("failed to add or set tag for new User "+email+": "+error.message,error);
    }
}
export async function newsletterSignup(firstName,lastName,email,addToPrayerList,recaptchaScore,dailyPrayer,mobilizer){
    console.info("doing newsletter signup. score: "+recaptchaScore+". ",firstName, lastName, email,addToPrayerList,mobilizer);

    const tags = ["Newsletter"];

    if(addToPrayerList){
        tags.push("Prayer List");
    }
    if(dailyPrayer){
       tags.push("Missionary of the Day");
    }
    if(mobilizer){
        tags.push("Mobilizer");
    }
    try{
        await mailingList.ensureUser(email);
        await mailingList.addTags(email,tags);
        await mailingList.setUserName(email,firstName,lastName);
    }catch(error){
        console.error("failed to add user to newsletter "+email+": "+error.message,error);
        throw error;
    }
}
export async function newProfile(email,firstName,lastName,missionary_profile_key,sendWelcome){
    console.local("setting user name: "+firstName+", "+lastName+", email: "+email);

    try{
        await mailingList.ensureUser(email);
        await mailingList.addTags(email,["Missionary"]);
        await mailingList.setUserName(email,firstName,lastName);

    }catch(error){
        console.warn("failed to set user name: "+firstName+", "+lastName+", email: "+email);
    }

    //profiles created by an org admin on someone else's behalf get an invitation
    //instead; the welcome email only goes to workers who made their own profile.
    if(sendWelcome !== false)
        await sendWelcomeEmail(email,firstName,missionary_profile_key);
}
// Welcome email for a worker who just created their own profile. Never let a mail
// failure break profile creation - the profile is already saved by this point -
// so failures are logged and reported with a false return rather than thrown.
export async function sendWelcomeEmail(email,firstName,missionary_profile_key){
    const domain = process.env.DOMAIN;
    try{
        let workerUrl = null;
        if(missionary_profile_key != null){
            try{
                const profile = await (await getServerDB()).getProfileByKey(missionary_profile_key);
                if(profile != null && profile.profile_slug)
                    workerUrl = `https://${domain}/worker/${profile.profile_slug}`;
            }catch(error){
                console.warn("failed to look up profile "+missionary_profile_key+
                    " for welcome email: "+error.message);
            }
        }
        const variables = {
            firstName: firstName ?? '',
            workerUrl: workerUrl,
            profileUrl: missionary_profile_key != null
                ? `https://${domain}/profile/edit/${missionary_profile_key}`
                : `https://${domain}/profile`,
            dashboardUrl:    `https://${domain}/dashboard`,
            displayPagesUrl: `https://${domain}/learn/display-pages`,
            givingUrl:       `https://${domain}/learn/donation-methods`,
            claimOrgUrl:     `https://${domain}/claim-org`,
            contactUrl:      `https://${domain}/contact`,
        };
        console.info("sending welcome email to "+email,variables);
        await sendTemplatedEmail('welcome',email,variables);
        return true;
    }catch(error){
        console.error("failed to send welcome email to "+email+": "+error.message,error);
        return false;
    }
}
export async function deleteProfile(missionary_profile_key,token,unlinkOnly){
    console.info("DELETING Profile "+missionary_profile_key);

    const payload = await jwtPayload(token);
    const userId = payload.sub;

    if(unlinkOnly === false)
        await getDB(token).deleteProfile(missionary_profile_key);

    //update search_filter
    const userPerms = await (await getServerDB()).getUserOrgSearchFilter(userId);
    const search_filter = userPerms && userPerms.search_filter;
    let doUpdate = false;

    for(var list of ["missionary_profile_keys","ro_profile_keys"]){
        if(search_filter != null && search_filter[list] != null){
            //remove missionary_profile_key from it
            search_filter[list]= 
                search_filter[list].filter(key => key != missionary_profile_key);
            doUpdate = true
        }
    }
    if(doUpdate){
        console.log("saving new search filter for org "+userPerms.organization_key,search_filter);
        await (await getServerDB()).updateOrganization(
            userPerms.organization_key,{ search_filter: userPerms.search_filter, });
    }

}
export async function inviteProfileOwner(ownerName,ownerEmail,adminEmail,profile_key,adminUserId,token){
    try{
        //send new profile message
        const user = await getFusionUser(adminUserId);
        //console.log("got user: ",user);
        const adminName = user.fullName || 'Admin';

        const userPerms = (await (await getServerDB()).getUserOrgSearchFilter(adminUserId));
        //console.log("got perms: ",userPerms);
        const profile = await getDB(token).getProfileByKey(profile_key)
        const org = await (await getServerDB()).getOrganization(profile.data.organization_key);
        const orgName = org.display_name;

        //see if owner already has an ergatas account with given email
        const owner = await getFusionUserByEmail(ownerEmail);
        //console.log("owner: ",owner);
        if(owner != null){

            const user = (await (await getServerDB()).getUserInfoByUserId(owner.id));
            console.log("owner user: ",user);
            if(user.has_profile === true){
                console.log("user "+user.user_key+" already has profile(s) "+user.missionary_profile_keys);
                return {existingProfile: true,
                        missionary_profile_keys: user.missionary_profile_keys};
            }
            //try{
                //const userProfile = await (await getServerDB()).getProfileByUser(user.user_key);
            //}catch(e){
                //// no existing profile, so we're ok.
            //}

            //then auto accept
            await setProfileOwner(token,owner.id,profile_key);
            await sendEmail({
                from: "web@"+process.env.MAIL_DOMAIN,
                to: ownerEmail,
                bcc: messagingAdminEmail,
                subject: "New  profile created for you on Ergatas.org",
                text:`
${ownerName},

${adminName} has created a Kingdom Worker profile representing you and your ministry with "${orgName}" on Ergatas.org. 

You can edit this profile by logging into your Ergatas account.

If you don’t want such a profile created on your behalf, we encourage you to contact ${adminName} at ${adminEmail} and let them know. If that fails, you can contact us at info@ergatas.org.

Thank you!

Ergatas Team `
            });
            return {existingProfile: false,autoAssigned:true};
        }


        await (await getServerDB()).insertProfileInvitation(adminUserId,profile_key,ownerEmail);

        await sendEmail({
            from: "web@"+process.env.MAIL_DOMAIN,
            to: ownerEmail,
            bcc: messagingAdminEmail,
            subject: "New  profile created for you on Ergatas.org",
            text:`
${ownerName},

${adminName} has created a Kingdom Worker profile representing you and your ministry with "${orgName}" on Ergatas.org. Ergatas is a free web app that tries to help workers raise their personal support by making their ministry visible to more people.

If you’d like to be able to manage this profile yourself in the future, please create a free account on https://ergatas.org using the email address ${ownerEmail}. Then visit your dashboard to automatically claim your profile:

https://ergatas.org/dashboard

If you don’t want such a profile created on your behalf, we encourage you to contact ${adminName} at ${adminEmail} and let them know. If that fails, you can contact us at info@ergatas.org.

Thank you!

Ergatas Team `
        })
        return {existingProfile: false};
    }catch(error){
        console.error("failed to send email to profile owner: ",error);
        if(error.name==="JWTExpired")
            throw error;
        throw new AppError("failed to invite profile owner for profile key "+profile_key);
    }
}
export async function claimProfile(token){
    const payload = await jwtPayload(token);
    const invitation = (await getDB(token).getProfileInvitations());
    if(invitation != null && invitation.email === payload.email){
        console.log("claiming invite ",invitation);
        await setProfileOwner(token,payload.sub,invitation.missionary_profile_key);
        await  (await getServerDB()).deleteProfileInvitation(invitation.profile_invitation_key)
    }
}
export async function setProfileOwner(token,userId,profile_key){
    console.log("setting owner of profile "+profile_key+" to "+userId);
    const profile = (await getDB(token).getProfileByKey(profile_key));
    console.log("got existing profile: ",profile);
    const user = (await (await getServerDB()).getUserInfoByUserId(userId));
    console.log("got user to set owner to: ",user);
    profile.user_key = user.user_key;

    await getDB(token).updateProfile(profile.missionary_profile_key,profile);

}
export async function assignNewProfilePermissions(userId,missionary_profile_key){

    const filter = (await (await getServerDB()).getUserOrgSearchFilter(userId));
    //console.log("new profile. current filter: ",filter);
    if(filter.search_filter == null)
        filter.search_filter = {};

    if(filter.search_filter.missionary_profile_keys == null)
        filter.search_filter.missionary_profile_keys=[];

    if( ! filter.search_filter.missionary_profile_keys.includes(missionary_profile_key))
        filter.search_filter.missionary_profile_keys.push(missionary_profile_key);

    //console.log("filter with new profile: ",filter);
    await (await getServerDB()).updateOrganization(filter.organization_key,{search_filter:filter.search_filter});

    //update permissions cache
    await getManagedProfiles(userId)
    
}
export async function getMissionaryProfile(missionary_profile_key){
    return (await (await getServerDB()).getDisplayProfileByKey(missionary_profile_key));
}
export async function incrementProfilePostPrayer(post_key){
    const db = (await getServerDB());
    const post = await db.getPostPrayerByKey(post_key);
    if(post == null)
        throw new AppError("Post not found");

    return await db.updatePostPrayerCount(post_key,(parseInt(post.prayer_count) || 0) + 1);
}
export function sendMessage(name,email,message){
    const missingFields = [];

    if(name == null || String(name).trim() === "")
        missingFields.push("name");

    if(email == null || String(email).trim() === "")
        missingFields.push("email");

    if(message == null || String(message).trim() === "")
        missingFields.push("message");

    if(missingFields.length > 0)
        throw new AppError("Missing required field(s): " + missingFields.join(", "));

    const mailDomain= process.env.MAIL_DOMAIN;
    return sendEmail({
            from: "web@"+mailDomain,
            "h:Reply-To":email,
            to: "information@"+mailDomain,
            subject: "Contact Form Message",
            text: "Message from: "+name+", "+email+"\n\n"+message,
        });
}
export const sendBulkMessage= function(emails,subject,message){
    const mailDomain= process.env.MAIL_DOMAIN;
    const allEmailAddresses = emails.join(", ") + ", information@ergatas.org";
    return sendEmail({
            from: "Ergatas <web@"+mailDomain+">",
            to: "Ergatas <web@"+mailDomain+">",
            bcc: allEmailAddresses,
            subject: subject,
            html: message
        });
}
export async function orgSlugCache(){
    const orgs = (await (await getServerDB()).organizationList());
    const cache = {};
    for(const org of orgs){
        if(org.slug != null && org.slug !== "")
            cache[org.slug] = org;
    }
    return cache;
}

// Cache page slugs for quick lookup (slug -> page_key)
export async function pageSlugCache(){
    const pages = (await (await getServerDB()).getPageSlugs());
    const cache = {};
    if(!pages) return cache;
    for(const p of pages){
        if(p.slug != null && p.slug !== ""){
            cache[p.slug] = p;
        }
    }
    return cache;
}
export function bucketUrl(image_url){
    const bucketName = process.env.UPLOAD_BUCKET;
    const bucketBaseUrl= process.env.BUCKET_BASE_URL;
    const bucketBase=bucketBaseUrl+"/"+bucketName+"/";
    return bucketBase+image_url;
}
function addImageHeaders($,full_url){
    $("meta[property='twitter:image']").attr("content",full_url);
    $("meta[property='og:image']").attr("content",full_url);
    $("meta[property='og:image:width']").attr("content","400");
    $("meta[property='og:image:height']").attr("content","400");
}
function escapeHtml(text){
    return String(text)
        .replace(/&/g,"&amp;")
        .replace(/</g,"&lt;")
        .replace(/>/g,"&gt;")
        .replace(/"/g,"&quot;");
}
//reduce user-authored profile description HTML to plain-text paragraphs.
//We never inject the raw HTML server-side; crawlers only need the text.
function descriptionParagraphs(html){
    if(html == null || String(html).trim() === "") return [];
    try{
        const $$ = cheerio.load(String(html));
        var blocks = $$("p, li, h1, h2, h3, h4")
            .map((i,el) => $$(el).text().replace(/\s+/g," ").trim()).get()
            .filter( t => t !== "");
        if(blocks.length === 0){
            const text = $$.root().text().replace(/\s+/g," ").trim();
            if(text !== "") blocks = [text];
        }
        return blocks;
    }catch(error){
        console.error("failed to extract text from profile description",error);
        return [];
    }
}
//truncate plain text at a word boundary for meta descriptions / schema fields
function truncateText(text,maxLength){
    if(text == null) return "";
    text = String(text).trim();
    if(text.length <= maxLength) return text;
    var cut = text.slice(0,maxLength);
    const lastSpace = cut.lastIndexOf(" ");
    if(lastSpace > maxLength/2) cut = cut.slice(0,lastSpace);
    return cut+"…";
}
//serialize a schema.org object into a JSON-LD <script> string, escaping `<` so a
//stray "</script>" inside any value can't break out of the script element.
function jsonLdScript(obj){
    const json = JSON.stringify(obj).replace(/</g,"\\u003c");
    return `<script type="application/ld+json">${json}</script>`;
}
//extract Q/A pairs from a prerendered Bootstrap FAQ accordion under `selector` so the
//FAQPage structured data always matches the visible questions and answers. Used by the
//about page (#faq) and any learn page carrying a `.learn-faq` block.
function buildFaqQuestions($,selector){
    const questions = [];
    $(selector).find(".accordion .card").each( (i,card) => {
        const question = $(card).find(".faq-card-title").first().text().replace(/\s+/g," ").trim();
        const answer = $(card).find(".card-body").first().text().replace(/\s+/g," ").trim();
        //skip cards without a text answer (e.g. the audio-only pronunciation card)
        if(question !== "" && answer !== ""){
            questions.push({
                "@type":"Question",
                "name":question,
                "acceptedAnswer":{
                    "@type":"Answer",
                    "text":answer
                }
            });
        }
    });
    return questions;
}
//inject schema.org JSON-LD appropriate to the page being built.
function addStructuredData($,pageName,info,canonicalUrl,profile,domain,org){
    try{
        //never emit structured data for pages we don't want indexed
        if(info && info.indexed === false) return;

        const orgPublisher = {
            "@type":"Organization",
            "name":"Ergatas",
            "url":`https://${domain}/`,
            "logo":`https://${domain}/img/ergatas2-light-green.png`
        };
        const image = $("meta[property='og:image']").attr("content");

        //site-wide identity + sitelinks search box, on the home/root page only
        if(pageName === "" || pageName === "home" || pageName === "index"){
            $("head").append(jsonLdScript({
                "@context":"https://schema.org",
                "@type":"Organization",
                "name":"Ergatas",
                "description":"A free website connecting Christian supporters with missionaries raising financial support. "+
                    "Search for missionaries across many sending organizations by location, people group, vocation, "+
                    "support level and more, then pray for, follow, or donate to them.",
                "url":`https://${domain}/`,
                "logo":`https://${domain}/img/ergatas2-light-green.png`,
                "nonprofitStatus":"Nonprofit501c3",
                "sameAs":[
                    "https://www.facebook.com/ergatas",
                    "https://x.com/Ergatas",
                    "https://www.instagram.com/ergatas777/"
                ]
            }));
            $("head").append(jsonLdScript({
                "@context":"https://schema.org",
                "@type":"WebSite",
                "name":"Ergatas",
                "url":`https://${domain}/`,
                "potentialAction":{
                    "@type":"SearchAction",
                    "target":{
                        "@type":"EntryPoint",
                        "urlTemplate":`https://${domain}/search/{search_term_string}`
                    },
                    "query-input":"required name=search_term_string"
                }
            }));
        }else if(pageName === "worker" && profile != null && profile.missionary_name){
            const person = {
                "@type":"Person",
                "name":profile.missionary_name,
                "url":canonicalUrl
            };
            if(image) person.image = image;
            const description = descriptionParagraphs(profile.data && profile.data.description).join(" ");
            if(description) person.description = truncateText(description,300);
            if(org && org.display_name)
                person.worksFor = {"@type":"Organization","name":org.display_name};
            if(profile.data && profile.data.location)
                person.homeLocation = {"@type":"Place","name":profile.data.location};
            $("head").append(jsonLdScript({
                "@context":"https://schema.org",
                "@type":"ProfilePage",
                "mainEntity":person
            }));
            $("head").append(jsonLdScript({
                "@context":"https://schema.org",
                "@type":"BreadcrumbList",
                "itemListElement":[
                    {"@type":"ListItem","position":1,"name":"Home","item":`https://${domain}/`},
                    {"@type":"ListItem","position":2,"name":"Search","item":`https://${domain}/search`},
                    {"@type":"ListItem","position":3,"name":profile.missionary_name,"item":canonicalUrl}
                ]
            }));
        }else if(pageName === "about"){
            //build FAQPage schema from the prerendered FAQ accordion so the
            //structured data always matches the visible questions and answers
            const questions = buildFaqQuestions($,"#faq");
            if(questions.length > 0){
                $("head").append(jsonLdScript({
                    "@context":"https://schema.org",
                    "@type":"FAQPage",
                    "mainEntity":questions
                }));
            }
        }else if(pageName === "wiki-page" && info && info.title){
            //wiki page title/description were hydrated from the DB in buildIndex
            const article = {
                "@context":"https://schema.org",
                "@type":"Article",
                "headline":(info.title || "").replace(/\s*[-|]\s*Ergatas\s*$/,""),
                "url":canonicalUrl,
                "publisher":orgPublisher
            };
            if(info.description) article.description = info.description;
            if(image) article.image = image;
            $("head").append(jsonLdScript(article));
        }else if(info && (info.path === "learn/" || info.path === "docs/")){
            const sectionName = info.path === "docs/" ? "Documentation" : "Learn";
            const sectionUrl = info.path === "docs/" ? `https://${domain}/docs` : `https://${domain}/learn`;
            //drop the " - Ergatas" brand suffix so the title reads cleanly as a headline/crumb
            const cleanTitle = (info.title || "").replace(/\s*[-|]\s*Ergatas\s*$/,"");
            const article = {
                "@context":"https://schema.org",
                "@type":"Article",
                "headline":cleanTitle,
                "description":info.description,
                "url":canonicalUrl,
                "publisher":orgPublisher
            };
            if(image) article.image = image;
            $("head").append(jsonLdScript(article));
            $("head").append(jsonLdScript({
                "@context":"https://schema.org",
                "@type":"BreadcrumbList",
                "itemListElement":[
                    {"@type":"ListItem","position":1,"name":"Home","item":`https://${domain}/`},
                    {"@type":"ListItem","position":2,"name":sectionName,"item":sectionUrl},
                    {"@type":"ListItem","position":3,"name":cleanTitle,"item":canonicalUrl}
                ]
            }));
            //learn pages carrying a prerendered FAQ accordion (.learn-faq) also emit
            //FAQPage schema built from the visible questions (spec §2.3, §2.4).
            const faqQuestions = buildFaqQuestions($,".learn-faq");
            if(faqQuestions.length > 0){
                $("head").append(jsonLdScript({
                    "@context":"https://schema.org",
                    "@type":"FAQPage",
                    "mainEntity":faqQuestions
                }));
            }
        }
    }catch(error){
        console.error("failed to add structured data for page "+pageName,error);
    }
}
// Entry js/css filenames are content-hashed by webpack; the build emits
// dist/asset-manifest.json listing them in load order. Cached by mtime so a
// rebuild is picked up without a server restart.
var assetManifest;
var assetManifestMtime;
function getAssetManifest(){
    const __dirname = path.resolve();
    const manifestPath = `${__dirname}/dist/asset-manifest.json`;
    try{
        const mtime = fs.statSync(manifestPath).mtimeMs;
        if(assetManifest == null || mtime !== assetManifestMtime){
            assetManifest = JSON.parse(fs.readFileSync(manifestPath,'utf-8'));
            assetManifestMtime = mtime;
        }
    }catch(error){
        if(assetManifest == null){
            console.error("FAILED to read dist/asset-manifest.json — has the client been built? "+
                "Falling back to unhashed asset names",error);
            assetManifest = {js:["/ergatas.js"],css:["/ergatas.css"]};
        }
    }
    return assetManifest;
}
export async function buildIndex(pageName,info,url){

    //console.local("building index for page "+pageName,info,url);
    const __dirname = path.resolve();
    const index= fs.readFileSync(`${__dirname}/lib/page-templates/index.html`,'utf-8');
    const subdir = info.path || "";

    var missionary_profile_key,profile,org;
    var canonicalUrl;
    const domain = process.env.DOMAIN;

    const $ = cheerio.load(index);

    //expand the asset markers into script/link tags for the (content-hashed)
    // build output. Order matters: defer scripts execute in document order.
    const assets = getAssetManifest();
    const jsMarker = $("script[data-app-js]");
    assets.js.forEach( src => jsMarker.before(`<script defer src="${src}"></script>`));
    jsMarker.remove();
    const cssMarker = $("link[data-app-css]");
    // onerror -> FOUC failsafe (see the inline script in index.html): if this
    // render-blocking stylesheet 404s (e.g. stale SW-cached HTML pointing at a
    // content-hashed CSS name a later deploy removed) force one reload rather
    // than leave the page painted unstyled.
    assets.css.forEach( href => cssMarker.before(`<link rel="stylesheet" href="${href}" onerror="window.__ergatasCssFailed()">`));
    cssMarker.remove();


    if(info && info.prerender !== false){
        const page_template = fs.readFileSync( `${__dirname}/lib/page-templates/${subdir}${pageName}.html` ,'utf-8');
        $("#page_content").append(page_template);
    }


    if(pageName === "home" || pageName ==="index" || pageName==="coming-soon")
        pageName=""; //set to root page

    try{
        if(info.path != null){
            canonicalUrl=`https://${domain}/${info.path}${pageName}`;
        }else if(["search","organization"].includes(pageName) 
                   && ! url.match(/^\/search\/saved\//)){
            canonicalUrl=`https://${domain}${url}`;
        }else
            canonicalUrl=`https://${domain}/${pageName}`;
    }catch(error){
        console.error("error while setting canonical URL ",error);
        canonicalUrl=`https://${domain}/${pageName}`;
    }

    //console.local("canonicalURL: "+canonicalUrl);

    if(pageName==="worker" && url != null){
        try{
            //set title to worker name and picture to profile picture
            var matches;
            if(url.startsWith("/worker/")){
                //strip any query string / hash so it isn't treated as part of the slug — e.g. the
                //?session_id=…&mpk=… appended to donation redirect-return URLs.
                const slug = url.split("/")[2].split(/[?#]/)[0];
                if(!await (await getServerDB()).profileSlugExists(slug)){
                    throw "not-found";
                }
                profile = await (await getServerDB()).getProfileBySlug(slug);
            }else{
                matches = url.match(/\/(\d+)(?:\?.*)?$/);
                if(matches && matches.length >= 2){
                    missionary_profile_key = matches[1];

                    if(!await (await getServerDB()).profileExists(missionary_profile_key)){
                        //console.warn("profile-detail page requested for non-existing profile "+missionary_profile_key);
                        throw "not-found";
                    }
                    profile = (await (await getServerDB()).getDisplayProfileByKey(missionary_profile_key));
                }else{
                    console.warn("failed to parse missionary_profile_key out of url "+url);
                    throw "not-found";
                }
            }
            //console.local("profile: ",profile);

            //if profile has a slug make that the canonical url, otherwise use the profile key
            if(profile.profile_slug){
                canonicalUrl = `https://${domain}/worker/${profile.profile_slug}`;
            }else{
                canonicalUrl = `https://${domain}/profile-detail/${profile.missionary_profile_key}`;
            }

            info.title = "Ergatas Profile - "+profile.missionary_name;
            if(profile.data.picture_url){
                info.sharing_image = bucketUrl(profile.data.picture_url);
            }

            info.indexed = ! profile.data.limit_social_media;

            try{
                org = await (await getServerDB()).getOrganization(profile.data.organization_key);
            }catch(orgError){
                console.error("failed to load organization for profile meta tags on "+url,orgError);
            }

            const paragraphs = descriptionParagraphs(profile.data.description);

            //give each profile its own content-bearing snippet instead of the
            //generic page_info description
            if(paragraphs.length > 0)
                info.description = truncateText(paragraphs.join(" "),160);

            //worker pages aren't prerendered from a template (the profile is
            //rendered client-side from the DB), so crawlers that don't run JS
            //would see an empty page. Inject a plain-text summary the client
            //router replaces on hydration (see doNavigate in lib/client/main.js).
            //Skipped for profiles that limit social media exposure (noindexed above).
            if(info.indexed !== false){
                const name = escapeHtml(profile.missionary_name);
                const subtitleParts = [];
                if(org && org.display_name) subtitleParts.push(escapeHtml(org.display_name));
                if(profile.data.location) subtitleParts.push(escapeHtml(profile.data.location));
                //#seo-prerender: JS-capable browsers hide this before first paint
                //(inline style in index.html) so humans don't see this plain-text
                //flash before hydration; it stays in the raw HTML for non-JS
                //crawlers / answer engines.
                $("#page_content").append(
                    `<div id="seo-prerender" class="container mt-4">`+
                        `<h1>${name}</h1>`+
                        (subtitleParts.length > 0 ? `<p class="text-muted">${subtitleParts.join(" — ")}</p>` : "")+
                        paragraphs.map( p => `<p>${escapeHtml(p)}</p>`).join("")+
                        `<p><a href="${escapeHtml(canonicalUrl)}">View the full profile of ${name} on Ergatas to pray for, follow, or financially support their ministry.</a></p>`+
                    `</div>`);

                //inline the profile the server already loaded so the client can
                //render the formatted page without re-fetching it (consumed by the
                //worker route handler in lib/client/router-setup.js). Escape < so a
                //"</script>" in user content can't break out; escape U+2028/2029
                //which are valid JSON but historically illegal in JS string literals.
                if(url.startsWith("/worker/")){
                    const slug = url.split("/")[2].split(/[?#]/)[0];
                    const prefetch = JSON.stringify({slug, profile})
                        .replace(/</g,"\\u003c").replace(/\u2028/g,"\\u2028").replace(/\u2029/g,"\\u2029");
                    $("body").append(`<script>window.__ergatas_prefetch=${prefetch};</script>`);
                }
            }

            //shown only while JS hydrates (styled in index.html); the client empties
            //#page_content when the real page mounts, removing it. Injected for every
            //worker page, including noindexed ones that have no #seo-prerender summary.
            $("#page_content").append(
                `<div id="hydration-loader" aria-hidden="true">`+
                    `<img width="150" src="/img/block-spinner2.svg" alt="Loading…"/>`+
                `</div>`);
        }catch(error){
            if(error === "not-found"){
               throw error;
            }
            console.error(`failed to add worker specific info to meta tags on ${url} page: ${error.message}`);
            if(info && info.title)
                $("title").html(info.title);
        }
    }else if(pageName === "organization" ){
        try{
            //remove nav
            $("nav").remove();
            //remove 'install as app?' banner
            $("#install_banner").remove();
            //replace footer
            $("footer").html(`
                <div class="container"><div class="row"><div class="col text-center">
                    <a href="https://${domain}" target="_blank">
                        <img class="mt-4" alt="Ergatas logo" id="logo-bottom" width="110" src="/img/powered_by.png"></a>
                </div></div></div>
            `)
             //indicate that this is a search-enabled paged
             $("body").append(`<div id="is_search_page"></div>`);
            
        }catch(error){
            console.error(`failed to add organization specific info to meta tags on organization (/${url}) error: ${error.message}`);
        }
    }else if(pageName === "wiki-page"){
        try{

            let page = (await (await getServerDB()).getPageByKey(info.page_key));
            //console.local("wiki page: ",page);
            if(page != null && page.data != null){
                info.title = page.data.title || info.title;
                info.description = page.data.description || info.description;
                info.sharing_image = page.data.sharing_image || info.sharing_image;
            }
            hydrateWikiPage($,page.data);
        }catch(error){
            console.error(`failed to add wiki page specific info to meta tags on wiki-page error: ${error}`);
        }
    }


    $("head").append(`<link rel="canonical" href='${canonicalUrl}' />`);

    //keep og:url aligned with the canonical URL for this page
    $("meta[property='og:url']").attr("content",canonicalUrl);

    if(info){

        if(info.title){
            $("title").html(info.title);
            $("meta[property='og:title']").attr("content",info.title);
            $("meta[property='twitter:title']").attr("content",info.title);
        }

        if(info.sharing_image){
            //if info.sharing_image is a full URL, use it directly, otherwise use as a path under domain
            if(info.sharing_image.startsWith("http://") || info.sharing_image.startsWith("https://")){
                addImageHeaders($,info.sharing_image);
            }else
                addImageHeaders($,`https://${domain}${info.sharing_image}`);
            $("meta[property='og:image:width']").remove();
            $("meta[property='og:image:height']").remove();
        }

        if(info.description){
            $("meta[name=description]").attr("content",info.description);
            $("meta[property='og:description']").attr("content",info.description);
            $("meta[property='twitter:description']").attr("content",info.description);
        }

        if(info.indexed===false){
            //console.local("adding noindex meta tag for page "+pageName);
            $("head").append("<meta name='robots' content='noindex'/>");
        }
    }

    addStructuredData($,pageName,info,canonicalUrl,profile,domain,org);

    return $.html();

}
export function setCustomCacheControl(res, path) {
    // express-static-gzip hands us the pre-compressed variant's path when the
    // client accepts br/gz (e.g. logo.svg.br). Strip that encoding suffix before
    // inspecting the real file type, or mime.lookup sees ".br" and misses that
    // it's an image — which would drop the Cross-Origin-Resource-Policy header
    // below and let helmet's same-origin default block cross-origin loads (the
    // FusionAuth login page embeds images from this origin).
    const realPath = path.replace(/\.(br|gz)$/, '');
    if (/\.[0-9a-f]{12,}\.(js|css)$/.test(realPath)
            || /[\\/]fonts[\\/]/.test(realPath)
            || /-[0-9]+(\.[0-9]+)*(\.min)?\.js$/.test(realPath)) {
        //never change in place: content-hashed build output, self-hosted text
        //fonts, and version-numbered vendor libs (e.g. jquery-3.5.1.min.js)
        res.setHeader('Cache-Control', 'public, max-age=31536000, immutable');
    } else if (/[\\/]webfonts[\\/]/.test(realPath)) {
        //FontAwesome subsets (scripts/subset-fa.cjs) change content without
        //changing filename when icons are added, so they must expire
        res.setHeader('Cache-Control', 'public, max-age='+(7*24*60*60));
    } else if (/(service-worker\.js|asset-manifest\.json)$/.test(realPath)) {
        //must revalidate every time or clients could be stuck on an old build
        res.setHeader('Cache-Control', 'no-cache');
    } else if ((serveStatic.mime.lookup(realPath) || '').startsWith('image/')) {
        //site images rarely change; 30 days
        res.setHeader('Cache-Control', 'public, max-age='+(30*24*60*60));
        res.setHeader('Cross-Origin-Resource-Policy', 'cross-origin');
    }
}
export function validOrigin(req){

    const validOrigins = process.env.CLIENT_ORIGINS.split(";");
    const origin = req.headers.origin;
    return validOrigins.indexOf(origin) !== -1;
}
export function sitemapUrlsFn(pageInfo){

    return async function(){
        var urls = ["/"];
        const pages = Object.keys(pageInfo).filter( page => page !== "profile-detail");
        pages.forEach( page => {
            const info = pageInfo[page];
            if(info.indexed !== false){
                if(info.path != null){
                    urls.push(`/${info.path}${page}`);
                }else
                    urls.push(`/${page}`);
            }
        })

        try{
            const profiles = (await (await getServerDB()).getAllProfileSummaries());
            profiles.forEach( profile => {
                if(!profile) return;
                const url = profile.profile_slug
                    ? `/worker/${profile.profile_slug}`
                    : `/profile-detail/${profile.missionary_profile_key}`;
                //include lastMod (date-only, W3C-datetime compliant) so crawlers
                //re-index changed profiles sooner
                const ts = profile.last_updated_timestamp || profile.created_on;
                const lastMod = typeof ts === "string" ? ts.slice(0,10) : ts;
                if(lastMod)
                    urls.push({url, lastMod});
                else
                    urls.push(url);
            });
        }catch(error){
            console.error("failed to get profile urls from DB",error);
        }

        return urls;

    };

}
//build the /llms.txt document (https://llmstxt.org) — a curated markdown index
//of the site's key content for AI engines. Derived from page_info.json (the same
//source as the sitemap) so it stays in sync; cached since page_info is static per deploy.
var llmsTxtCache;
export function llmsTxt(pageInfo){
    if(llmsTxtCache != null) return llmsTxtCache;

    const domain = process.env.DOMAIN || "ergatas.org";
    const base = `https://${domain}`;

    function entry(page){
        const info = pageInfo[page];
        const title = (info.title || page).replace(/\s*[-|]\s*Ergatas\s*$/,"").trim();
        const url = info.path != null ? `${base}/${info.path}${page}` : `${base}/${page}`;
        const description = (info.description || "").trim();
        return `- [${title}](${url})`+(description ? `: ${description}` : "");
    }
    function listable(page){
        const info = pageInfo[page];
        return info.indexed !== false && !info.virtual && !info.alias_for;
    }

    const learnPages = Object.keys(pageInfo).filter( p => listable(p) && pageInfo[p].path === "learn/");
    const docsPages = Object.keys(pageInfo).filter( p => listable(p) && pageInfo[p].path === "docs/");
    const keyPages = ["about","search","daily-prayer","sof","donate","contact"].filter( p => pageInfo[p] != null && listable(p));

    llmsTxtCache = [
        "# Ergatas",
        "",
        "> Ergatas is a free website that connects Christian supporters with vetted missionaries "+
        "(\"Kingdom Workers\") who are raising financial support. Supporters can search for "+
        "missionaries across many sending organizations by location, people group, vocation/skill, "+
        "cause, support level, language spoken, cultural distance, where the workers work is making an impact, or family structure, "+
        "then pray for, follow, or donate to them. Ergatas is free for "+
        "missionaries and donors, takes no portion of donations, and donations are sent to the "+
        "missionary's own sending organization. All missionaries are vetted and confiremd to be a member of a sending agency " +
        "providing oversite and accountability",
        "",
        "## Key pages",
        "",
        `- [Home](${base}/): Search for Christian missionaries across organizations and find someone whose ministry you're passionate about supporting`,
        ...keyPages.map(entry),
        "",
        "## Learn about missions and missionary support",
        "",
        ...learnPages.map(entry),
        "",
        "## How-to guides",
        "",
        ...docsPages.map(entry),
        "",
        "## Optional",
        "",
        `- [Sitemap](${base}/sitemap.xml): all pages, including every public missionary profile`,
        `- [Missionary of the Day feed](${base}/feeds/missionaryOfTheDay): RSS feed featuring a different missionary each day`,
        `- [New Missionaries feed](${base}/feeds/newMissionaries): RSS feed of missionaries recently added to the site`,
        ""
    ].join("\n");

    return llmsTxtCache;
}
export async function randomMissionary(){
    const profile = (await (await getServerDB()).randomSharableProfile());
    //console.local("got random profile ",profile);
    return profile;
}

export async function getPushSubscription(push_subscription_key){
    try{
       return (await (await getServerDB()).getPushSubscription(push_subscription_key)).subscription;
    }catch(error){
       console.error("failed to get subscription for key "+push_subscription_key,error);
    }
}
export async function unsubscribePushNotifications(subscription){
   await (await getServerDB()).deletePushSubscription(subscription);
}

export async function savePushSubscription(subscription,lists){
    try{
       await (await getServerDB()).insertPushSubscription(subscription,lists);
    }catch(error){
       console.error("failed to insert new subscription ",error);
    }
}
export async function sendMODNotification(push_subscription_key,feeds){
   var sub = await getPushSubscription(push_subscription_key);
   console.local("sub for key "+push_subscription_key,sub);
   sendMODNotifications(feeds,[sub]);
}
export async function sendMODNotifications(feeds,subscriptions){
   const profile = feeds.headItem("missionaryOfTheDay");
   //console.local("MOD profile: ",profile);

   if(profile == null)
      return;

   const payload = {
      title: "Ergatas Notice",
      body: "Pray for "+profile.author,
      url: profile.url,
   };

   if(profile.enclosure && profile.enclosure.url){
      payload.image = profile.enclosure.url;
   }
   if(subscriptions == null) { //use all subscriptions
      subscriptions = (await (await getServerDB()).getPushSubscriptions(["daily_prayer_list"]))
                           .map(row => row.subscription);
   }
   //console.local("subscriptions: ",subscriptions);
   console.info("sending notification to "+subscriptions.length+" subscriptions");
   subscriptions.forEach( sub => sendNotification(sub,payload,"daily_prayer_list"));

}
export async function sendNotification(subscription,payload,topic){

	const options={ };

   if(topic != null && topic !== ""){
      options.headers = {
         topic: topic
      };
   }

   webPush.setVapidDetails(
        'https://ergatas.org/',
        process.env.VAPID_PUBLIC_KEY,
        process.env.VAPID_PRIVATE_KEY
   );

	console.local("sending push notification",subscription, payload, options);
   try{
      await webPush.sendNotification(subscription, JSON.stringify(payload),options)
   }catch(error){
      console.error("notification failed: ",error);
   }

}

export async function sendMODEmails(feeds, listName = "Missionary of the Day", profileItem = null){
   var content;
   const profile = profileItem || feeds.headItem("missionaryOfTheDay");
   if(profile == null || profile.url == null){
      console.error("no missionary of the day found");

      return;
   }
   //console.local("profile: ",profile);
   const emailHtml= fs.readFileSync(process.cwd()+"/lib/snippet-templates/mod-email.html","utf8");
   const settings={
                subject_line: "Ergatas Missionary of the Week",
                preview_text: `Pray for the ministry of ${profile.author}`,
                from_name: "Ergatas",
                reply_to: "information@ergatas.org",
                title: "MoD "+(new Date()).toLocaleString('en-US',{
                                                 year:"numeric",
                                                 month:"long",
                                                 day:"numeric"}),
             };

   const $ = cheerio.load(emailHtml);

   $("#name").html(profile.author);
   $(".profile_link").attr("href",profile.url);
   $(".contact_link").attr("href",profile.url+"#connect");
  


   if(profile.enclosure && profile.enclosure.url){
      $("#image").attr("src",profile.enclosure.url);
      $("#image").attr("alt",profile.author);
   }else{
      $("#image_row").html("");
   }

   try{
       if(profile.guid != null){
           const fullProfile =await getMissionaryProfile(profile.guid);
           console.local("full profile: ",fullProfile);
           $(".description").html(fullProfile.data.description);

       }
   }catch(error){
       console.warn("failed to fetch full profile for MOD email: ",error);
   }

   // Add a "Prayer Requests" section just before the description if the profile has any.
   try{
       if(profile.guid != null){
           const posts = await (await getServerDB()).getPublicPostsByProfileKey(profile.guid);
           const prayerRequests = (posts || []).filter(p => p.post_type === 'prayer request');
           if(prayerRequests.length > 0){
               const itemsHtml = prayerRequests
                   .map(() => '<div class="prayer-request-item" style="white-space: pre-line; padding-bottom:10px;"></div>')
                   .join("");
               const sectionHtml = '<div class="prayer-requests" style="padding-bottom:20px;">'+
                   '<h3 style="margin:0 0 10px 0;">Prayer Requests</h3>'+
                   itemsHtml+
                   '</div>';
               $(".description").before(sectionHtml);
               // Label the description as "Ministry" so it's distinct from the prayer requests above it.
               $(".description").before('<h3 class="ministry-heading" style="margin:0 0 10px 0;">Ministry</h3>');
               // Set content as text (not html) to escape it, matching how posts render on the profile page.
               $(".prayer-request-item").each((i, el) => $(el).text(prayerRequests[i].post_content || ""));
           }
       }
   }catch(error){
       console.warn("failed to add prayer requests to MOD email: ",error);
   }


   content = $.html();
   console.log("listname: " + listName + ", settings: ", settings);
   mailingList.sendCampaign(listName,content,settings);
}
/* not used anymore
export async function emailUsersWithDonationClicks(monthsPrev=1,test=false){

   console.local("emailing users with donation clicks");
   var start = new Date();
   var end = new Date();
   var indexedUsers = {};
   const dayInMillies = 24*60*60*1000;
   //set to first of last month
   start.setDate(1); 
   start.setMonth(start.getMonth() - monthsPrev);

   end.setMonth(start.getMonth() + 1);
   end.setDate(1);
   end.setTime(end.getTime() - dayInMillies); //rollback one day to get last day of previous month

   console.local(`date range:[${start},${end}]`);

   try{
      const users = (await (await getServerDB()).getUsersWithDonation(start,end));
      console.local("found "+users.length+" users in range");
      users.forEach(user => {indexedUsers[user.external_user_id] = user });
      (await getUserEmails(users.map( user => user.external_user_id)))
         .forEach( info =>
            indexedUsers[info.external_user_id].email= info.email);

      Promise.all(
         users.filter(user => user.email != null && user.email !== "").
            map(async user =>{
               console.local(user.first_name,user.email,user.last_possible_tx_date,user.tx_dates);
               var dates = new Set(user.tx_dates.split(",").
                              map( d => new Date(d)).
                              filter(d => d.getMonth() === start.getMonth()).
                              map(d => d.toLocaleString('en-US',{year:'numeric',month:'long',day:'numeric'})));
               console.local("dates: ",dates);
               return await sendDonationClickEmail(user.first_name,user.email,dates,test);
      }));

   }catch(error){
      console.error("error while emailing users with donation clicks: ",error);
   }

}
*/
async function sendDonationClickEmail(name,email,dates,test){

   const mailDomain= process.env.MAIL_DOMAIN;
   console.local("sending donation click email to "+email);
   const snippetName="end-of-month-donation-clicks.html";
   const __dirname = path.resolve();
   const template = fs.readFileSync(`${__dirname}/lib/snippet-templates/${snippetName}`,'utf-8');

   const $ = cheerio.load(template);
   var dateStr="";
   dates.forEach( d => dateStr = dateStr+d+"<br/>");

   console.local("name: "+name+", email: "+email+", dateStr:"+dateStr);
   $("#name").html(name);
   $("#dates").html(dateStr);

   try{
      if(test)
         email = "kevin.horan@ergatas.org";

      await sendEmail( {
            from: "information@"+mailDomain,
            to: email,
            bcc: messagingAdminEmail,
            subject: "Possible Donation on Ergatas",
            html: $.html(),
      });
   }catch(error){
      console.error("failed to send donation click email: ",error);
   }
}

export  function cleanupFiles(files){
	console.info("cleaning up files: ",files);	
	Object.values(files).forEach( file => deleteFile(file.path));
}
function deleteFile(filename){
	console.info("attempting to delete filename "+filename);
	fs.unlink(filename, function(err) {
		 if(err && err.code == 'ENOENT') {
			  // file doens't exist
			  console.info("File doesn't exist, won't remove it.");
		 } else if (err) {
			  // other errors, e.g. maybe we don't have enough permission
			  console.error("Error occurred while trying to remove file: "+err.code,err);
		 } else {
			  console.info(`removed file ${filename}`);
		 }
	});
}
export async function queueMessage(userId,fromEmail,name, message){

    try{ //send notice of new message, but don't die if it fails
       sendEmail({
           from: "Ergatas App <information@ergatas.org>",
           to: "kevin.horan@ergatas.org",
           subject: "NEW MESSAGE to review",
           html: "<a href='https://ergatas.org/message-moderation'>Messages</a><br>"+message,
       });
    }catch(error){
    }
    try{
       await (await getServerDB()).insertMessage(userId,fromEmail,name,message);
    }catch(error){
       console.error(`failed to save in-app message. from ${name} <${fromEmail}> to ${userId}`,message);
    }
}
export  async function sendQueuedMessage(messageQueueKey){
   var messageInfo = (await (await getServerDB()).getMessage(messageQueueKey));
   //console.local("message info for queue key "+messageQueueKey+": ",messageInfo);

   const toEmail = await userIdToEmail(messageInfo.external_user_id);
   console.log("email for user id "+messageInfo.external_user_idd+": "+toEmail);
   const result =await  contact(
                           messageInfo.from_email,
                           messageInfo.from_name,
                           messageInfo.message,toEmail);
   console.log("contact result: ",result);
}
export async function deleteQueuedMessage(messageQueueKey){
    await (await getServerDB()).deleteMessage(messageQueueKey);
}
export async function getAllQueuedMessages(){
   const db = await getServerDB();
   const messages = await db.getAllMessages();

   if(messages == null || messages.length === 0)
      return messages;

   // attach the worker profile each message was written to, so moderators can
   // open it while reviewing
   const userIds = [...new Set(messages.map(info => info.external_user_id).filter(id => id != null))];
   if(userIds.length === 0)
      return messages;

   const profiles = await db.getProfilePreviewsByUserIds(userIds);
   const profilesByUserId = new Map();
   (profiles ?? []).forEach(profile => {
      if(!profilesByUserId.has(profile.external_user_id))
         profilesByUserId.set(profile.external_user_id,profile);
   });

   messages.forEach(info => {
      const profile = profilesByUserId.get(info.external_user_id);
      if(profile == null)
         return;
      info.missionary_profile_key = profile.missionary_profile_key;
      info.missionary_name = profile.missionary_name;
      info.profile_url = profile.profile_slug
                            ? "/worker/"+profile.profile_slug
                            : "/profile-detail/"+profile.missionary_profile_key;
   });

   return messages;
}

async function getAccessToken(){

    const refreshToken=process.env.GOOGLE_ANALYTICS_REFRESH_TOKEN;
    const clientId=process.env.GOOGLE_ANALYTICS_CLIENT_ID;
    const clientSecret=process.env.GOOGLE_ANALYTICS_CLIENT_SECRET;

    try{
        var response = await axios({
            method: "POST",
            url: "https://www.googleapis.com/oauth2/v4/token",
            data: {
                client_id: clientId,
                client_secret: clientSecret,
                grant_type: "refresh_token",
                refresh_token:refreshToken,
            }
        });
        if(response.data){
           return response.data.access_token;
        }else
            throw new AppError("no data field in response while getting access token for GA");
    }catch(error){
        console.local("failed to get analytics refresh token",error && error.message);
    }

}
async function analyticsQuery(params,accessToken){
    const viewId =process.env.GOOGLE_ANALYTICS_VIEW_ID;
    try{
        params.ids="ga:"+viewId;
        params['start-date']="2020-01-01"; // start of data collection
        params['end-date']="2023-01-01";
        
        var response = await axios({
            method: "GET",
            url: "https://www.googleapis.com/analytics/v3/data/ga",
            params: params,
            headers:{
                Authorization: "Bearer "+accessToken,
            }
        });

        if(response.data){
           //console.log("got stats: ",response.data);
           return response.data;
        }else
            throw new AppError("no data field in response while getting stats from GA");
    }catch(error){
        console.local("failed to get analytics data");
    }
}

// GA4 records a profile visit under whichever path the visitor actually landed
// on. /profile-detail/<key> redirects to the canonical /worker/<slug> (see the
// profile-detail route in lib/client/router-setup.js), so nearly every real
// view is recorded under the slug path: matching only /profile-detail/<key>
// made these numbers read as near-zero for any profile that has a slug (all of
// them). Match both paths, and filter on pagePath rather than
// pagePathPlusQueryString so a visit arriving with utm_* (or any other query
// string) still counts.
function ga4ProfilePathFilter(missionary_profile_key, profileSlug){
    if(missionary_profile_key == null){
        //summary mode: every worker profile page on the site
        return {
            orGroup:{
                expressions: ["/profile-detail/","/worker/"].map( prefix => ({
                    filter:{
                        fieldName: "pagePath",
                        stringFilter:{ matchType: "BEGINS_WITH", value: prefix },
                    }
                }))
            }
        };
    }

    const paths = ["/profile-detail/"+missionary_profile_key];
    if(profileSlug != null && profileSlug !== "")
        paths.push("/worker/"+profileSlug);

    return {
        filter:{
            fieldName: "pagePath",
            inListFilter:{ values: paths },
        }
    };
}
function emptyProfileStats(){
    return {
        donationClicks: 0,
        pageViews: 0,
        prayers: 0,
    };
}
export const PROFILE_STAT_NAMES = ["pageViews","donationClicks","prayers"];

//one GA4 event-count report for a profile's pages. `byDate` adds the date
//dimension (as dimensionValues[1]) so the rows can be split into time buckets.
async function runGA4ProfileReport({missionary_profile_key, profileSlug, startDate, endDate, byDate}){
    const propertyId = process.env.GOOGLE_ANALYTICS_GA4_PROPERTY_ID;
    const analyticsDataClient = new BetaAnalyticsDataClient();
    const dimensions = [{ name: "eventName" }];

    if(byDate)
        dimensions.push({ name: "date" });

    const [response] = await analyticsDataClient.runReport({
        property: `properties/${propertyId}`,
        dateRanges: [{ startDate: startDate, endDate: endDate }],
        dimensions: dimensions,
        metrics: [{ name: "eventCount" }],
        dimensionFilter: ga4ProfilePathFilter(missionary_profile_key, profileSlug),
        limit: 10000,
    });
    return response;
}
//totals the three numbers the dashboard shows out of a report's rows. rowFilter
//picks which rows count, which is how one two-window report is split into the
//current and previous periods.
function ga4StatsFromRows(rows, rowFilter){
    const sums = {};
    (rows || []).forEach( row =>{
        if(rowFilter != null && ! rowFilter(row))
            return;
        const name = row.dimensionValues[0].value;
        sums[name] = (sums[name] || 0) + parseInt(row.metricValues[0].value);
    });
    const eventValue = name => sums[name] || 0;

    return {
        donationClicks: eventValue("donate-level-1") + eventValue("begin-checkout"),
        pageViews: eventValue("page_view"),
        prayers: eventValue("prayed"),
    };
}
export async function getGA4ProfileStats(missionary_profile_key, profileSlug){
    console.log("GA4 stats for profile "+missionary_profile_key);
    try{
        const response = await runGA4ProfileReport({
            missionary_profile_key: missionary_profile_key,
            profileSlug: profileSlug,
            startDate: "2023-01-01", // start of GA4 data collection
            endDate: "today",
        });
        return ga4StatsFromRows(response.rows);
    }catch(error){
        console.error("failed to get GA4 page stats for profile key "+missionary_profile_key+
                        ": "+(error && error.message));
        return emptyProfileStats();
    }
}
async function profileSlugForKey(missionary_profile_key){
    if(missionary_profile_key == null)
        return null;
    try{
        const profile = (await (await getServerDB()).getDisplayProfileByKey(missionary_profile_key));
        return profile == null ? null : profile.profile_slug;
    }catch(error){
        //no slug just means we fall back to matching the /profile-detail path
        console.warn("could not look up slug for profile "+missionary_profile_key+
                        ": "+(error && error.message));
        return null;
    }
}
//all-time totals: GA4 (2023 onward) plus what Universal Analytics recorded
//before it. Pass profileSlug when the caller already knows it, to save a lookup.
export async function getProfileStats(missionary_profile_key, profileSlug){
    if(profileSlug === undefined)
        profileSlug = await profileSlugForKey(missionary_profile_key);

    const [ga4Stats,oldStats] = await Promise.all([
        getGA4ProfileStats(missionary_profile_key,profileSlug),
        getUniversalProfileStats(missionary_profile_key),
    ]);
    console.log("new, old stats: ",ga4Stats,oldStats);
    return {
        donationClicks: ga4Stats.donationClicks + oldStats.donationClicks,
        pageViews: ga4Stats.pageViews + oldStats.pageViews,
        prayers: ga4Stats.prayers + oldStats.prayers,
    }

}
//The GA4 date keys (YYYYMMDD) for the `days` days ending today, newest first.
//Built by stepping UTC dates rather than subtracting milliseconds, so a DST
//change can't drop or repeat a day.
export function ga4DateKeys(days,today = new Date()){
    const cursor = new Date(Date.UTC(today.getFullYear(),today.getMonth(),today.getDate()));
    const keys = [];
    for(let i=0;i<days;i++){
        keys.push(""+cursor.getUTCFullYear()+
                  String(cursor.getUTCMonth()+1).padStart(2,"0")+
                  String(cursor.getUTCDate()).padStart(2,"0"));
        cursor.setUTCDate(cursor.getUTCDate()-1);
    }
    return keys;
}
//Splits the rows of a 2*days report into the most recent `days` (current), the
//`days` before that (previous), and a per-day series over the current window.
//Exported for testing.
export function bucketProfileTrend(rows,days,today = new Date()){
    const currentKeys = ga4DateKeys(days,today);
    const current = new Set(currentKeys);
    const previous = new Set(ga4DateKeys(2*days,today).slice(days));
    const dateOf = row => row.dimensionValues[1].value;

    const rowsByDate = new Map();
    (rows || []).forEach( row =>{
        const key = dateOf(row);
        if( ! current.has(key))
            return;
        if( ! rowsByDate.has(key))
            rowsByDate.set(key,[]);
        rowsByDate.get(key).push(row);
    });

    //oldest first, with an entry for every day in the window so the sparkline
    //keeps a point for days that saw no activity at all
    const dates = currentKeys.slice().reverse();
    const dailyStats = dates.map( key => ga4StatsFromRows(rowsByDate.get(key) || []));
    const series = {};
    PROFILE_STAT_NAMES.forEach( metric =>{
        series[metric] = dailyStats.map( stats => stats[metric]);
    });

    return {
        current: ga4StatsFromRows(rows, row => current.has(dateOf(row))),
        previous: ga4StatsFromRows(rows, row => previous.has(dateOf(row))),
        series: series,
        dates: dates,
    };
}
function emptyProfileTrend(days){
    const series = {};
    PROFILE_STAT_NAMES.forEach( metric =>{
        series[metric] = new Array(days).fill(0);
    });
    return {
        current: emptyProfileStats(),
        previous: emptyProfileStats(),
        series: series,
        dates: [],
    };
}
//What the dashboard's stats panel needs: the last `days` days, the same length
//window before it to compare against, a daily series to draw, and the all-time
//totals. The trend is GA4-only - the window never reaches back into the
//pre-2023 Universal Analytics data, which only the all-time totals include.
export async function getProfileStatsTrend(missionary_profile_key,days){
    days = Math.min(Math.max(parseInt(days) || 30, 7), 90);
    const profileSlug = await profileSlugForKey(missionary_profile_key);
    var trend;

    try{
        const response = await runGA4ProfileReport({
            missionary_profile_key: missionary_profile_key,
            profileSlug: profileSlug,
            //today plus the 2*days-1 days before it: two windows of `days` days
            startDate: (2*days - 1)+"daysAgo",
            endDate: "today",
            byDate: true,
        });
        trend = bucketProfileTrend(response.rows,days);
    }catch(error){
        console.error("failed to get GA4 stats trend for profile key "+missionary_profile_key+
                        ": "+(error && error.message));
        trend = emptyProfileTrend(days);
    }

    return Object.assign({
        days: days,
        allTime: await getProfileStats(missionary_profile_key,profileSlug),
    },trend);
}
export async function getUniversalProfileStats(missionary_profile_key){
    // THESE ARE THE OLD STATS
    /*
    sample query: 
    https://www.googleapis.com/analytics/v3/data/ga?
        ids=ga%3A230421808&
        access_token=ya29.A0ARrdaM-vyzG-QBK2YgdXp_DPtbhaI1Mbeyq1m8u2ZktHnxH1mk-IPAT7IdMoPD6HPWeInFyPB-wICy_QmDtc5ILfJVkWnj1GsLvuqbwCX2uDnAy0EwgBI2C6UESGVZzsP87mqSEYUG48V1L9lBKHBe4SSvQs&
        dimensions=ga%3ApagePath&
        metrics=ga%3Agoal2Starts%2Cga%3Agoal2Completions%2Cga%3Apageviews&
        filters=ga%3ApagePath%3D%40profile-detail;ga:pagePath!@code&
        start-date=2020-01-01&
        end-date=today

    donation stats
        metrics=ga%3AuniqueEvents&
        filters=ga%3AeventCategory%3D%3Ddonations+level+1%3Bga%3AeventLabel%3D%3D254&

    profile page views:
        metrics=ga%3Apageviews&
        filters=ga%3ApagePath%3D%40%2Fprofile-detail%2F254%3Bga%3ApagePath%21%40code

    */

    const accessToken = await getAccessToken();
    const stats ={
        donationClicks:0,
        pageViews:0,
        prayers:0,
    }
    try{

        //var x =   "ga:eventCategory==donations level 1"+(missionary_profile_key == null ? "":(";ga:eventLabel=="+missionary_profile_key));
        //console.log(" missionary key: "+missionary_profile_key+", donation filter: "+x);

        const donationStats = await analyticsQuery({
            metrics:"ga:uniqueEvents",
            filters:"ga:eventCategory==donations level 1"+(missionary_profile_key == null ? "":(";ga:eventLabel=="+missionary_profile_key)),
        },accessToken);
        if(donationStats && donationStats.rows && donationStats.rows.length==1)
            stats.donationClicks = parseInt(donationStats.rows[0][0]);

        const pageStats = await analyticsQuery({
            metrics:"ga:pageviews",
            filters:"ga:pagePath!@code"+(missionary_profile_key == null ? "":(";ga:pagePath=@/profile-detail/"+missionary_profile_key)),
        },accessToken);
        if(pageStats && pageStats.rows && pageStats.rows.length==1)
            stats.pageViews = parseInt(pageStats.rows[0][0]);

        const prayerStats = await analyticsQuery({
            metrics:"ga:uniqueEvents",
            filters:"ga:eventAction==prayed;ga:pagePath!@code"+(missionary_profile_key == null ? "":(";ga:pagePath=@/profile-detail/"+missionary_profile_key)),
        },accessToken);
        if(prayerStats && prayerStats.rows && prayerStats.rows.length==1)
            stats.prayers= parseInt(prayerStats.rows[0][0]);

    }catch(error){
        console.error("failed to get page stats for profile key "+missionary_profile_key,error);
    }

    //console.log("stats: ",stats);
    return stats;
    
}

export function verifyMailgunRequest(signature){

    console.local("verifying mailgun request");
    const signingKey = process.env.MAILGUN_API_KEY;
    const encodedToken = crypto
        .createHmac('sha256', signingKey)
        .update(signature.timestamp.concat(signature.token))
        .digest('hex')

    return (encodedToken === signature.signature)
}
export async function handleMailgunEvent(eventData){
    console.local("handling mailgun event ",eventData);

    if(eventData.event == null)
        return;
    
    if(eventData.event == "failed")
        console.error("mailgun failure event: ",eventData);

}

export async function cleanUpOldUsers(dryRun=true,count=1){
    // find users who are not in auth and don't have a profile or saved search
    const tenantId = process.env.AUTH_TENANT_ID;
    const usersWithoutEntities = (await (await getServerDB()).getUsersWithoutEntities());
    console.info("found "+usersWithoutEntities.length+" profileless users");

    console.info("cleaning up users. dryRun? "+dryRun+", count="+count);

    const minAge =new Date(new Date().getTime() -  30*24*60*60*1000); //30 days ago
    var i;

    //see if they exist in authdb
    for( let user of usersWithoutEntities.slice(0,count)){

        console.info((dryRun?"DRY RUN":"LIVE")+" ----- considering user ------------",user);

        //skip any user created in the last 30 days
        if(new Date(user.user_created).getTime() > minAge.getTime()){
            console.info("    skipping user less than 30 days old",user.user_created,minAge);
            continue;
        }

        try{
            const email = await userIdToEmail(user.external_user_id);
            if(email != null){
                user.email = email;
                if(mailingList.userExists(user.email)){
                    console.info("    found user in mailing list",user.first_name, user.last_name, user.user_key)
                    console.info("    deleting user from all ",user.user_key, user.external_user_id);
                    if( ! dryRun){
                        deleteUser(user.external_user_id); // from mailinglist,authdb
                        //deleteUserFromDB(user.user_key);
                    }
                }
            }
        }catch(error){
            console.info("    user not found in auth db, removing from PG");
            //if( ! dryRun)
                //deleteUserFromDB(user.user_key);
        }
        

    };
    console.info("-----------------------------------");
    console.info("fetching mailing list members...");

    //fetch all entries from mailing list and see which ones are in authdb. Delete any not found
    const mailingListEmails = (await mailingList.getAllMemberEmails(count)).members;
    console.info("mailing list emails: ",mailingListEmails);

    for(let email of mailingListEmails){
        //get user from authdb
        email = email.email_address.toLowerCase();

        console.info((dryRun?"DRY RUN":"LIVE")+" ----- considering email: ",email);
        try{
            const authUser= await fusionRequest("GET","/api/user?email="+email,null,
                                        {"X-FusionAuth-TenantId":tenantId});
            console.info("    found mailing list user in auth db",email,authUser.id);

        }catch(error){
            console.info("    failed to find mailing list member "+email+" in auth db");
            if(error.response != null && error.response.status === 404){
                console.info("    removing user from mailing list",email);
                //remove from mailing list
                if( ! dryRun)
                    await mailingList.removeUser(email);
            }else{
                console.error("    error contacting authdb: ",error);
            }
        }
    }
    console.info("-----------------------------------");
}

// Picks a random prayer-card-drawing entrant and emails them the winner notice.
// Pass overrideEmail to send the notice to a specific address instead (for testing);
// the winner is still drawn at random to populate the firstName greeting.
export async function pickPrayerCardDrawingWinner(overrideEmail){
    try{
        console.log("picking prayer card winner");
        var user= (await (await getServerDB()).getUserForPrayerCardDrawing());
        console.log("prayer card winner: ",user);

        const email = overrideEmail || await userIdToEmail(user.external_user_id);
        await sendTemplatedEmail('prayer-card-winner', email, { firstName: user?.first_name ?? '' });
    }catch(error){
        console.error("failed to pick prayer card winner: ",error);
        throw new AppError("failed to pick prayer card winner");
    }

}
export async function sendDonationNotice(possible_transaction_key,external_user_id){

    const mailDomain= process.env.MAIL_DOMAIN;
    try{
        console.log("sendDonationNotice ",possible_transaction_key,external_user_id);
        const email = await userIdToEmail(external_user_id);

        const tx = (await (await getServerDB()).getPossibleTransaction(possible_transaction_key));
        console.log("tx: ",tx);

        const profile = (await (await getServerDB()).getDisplayProfileByKey(tx.missionary_profile_key));
        console.log("profile: ",profile);

        const stripeAccount = await stripeUtils.getStripeAccountForProfile(tx.missionary_profile_key);
        const customer = await stripeUtils.getCustomerFromStripeId(tx.stripe_id, stripeAccount);
        var message=`${profile.data.first_name},`;

        if(tx.donation_type==="one-time"){
            message += `\n\nYou've received a donation on Ergatas for $${tx.amount}! The money has been transferred to your agency. `

        }else if(tx.donation_type==="recurring"){
            message += `\n\nYou've received a recurring donation on Ergatas for $${tx.amount}/month! The money has been transferred to your agency. `
        }else{
            console.log("unknown donation type: "+tx.donation_type+", donation notice not sent");
            return;
        }
        if(customer != null)
            message += `\n\nYour donor's name is ${customer.name}, email: ${customer.email}`;

        message += `\n\nWould you mind letting us know if you already knew this donor, or if this is a new contact for you? That helps us know how effective we're being at finding workers new contacts. Thanks!\n`;

        //console.log("message: ",message);

        const data = {
            from: "information@"+mailDomain,
            bcc: "information@"+mailDomain,
            to:  email,
            subject: "New Donation on Ergatas!",
            text:  message,
        }
        await sendEmail(data);
    }catch(error){
        console.error("failed to send donation notice: ",error,possible_transaction_key);
        await sendEmail({
            from: "information@"+mailDomain,
            to: "information@"+mailDomain, 
            subject: "Donation Notice Failure",
            text:  `Failed to send donation notice for tx key ${possible_transaction_key} for user ${external_user_id}`,
        });
        throw error;
    }
}
export async function claimOrg(organization_key, church_name, church_website,user_key,read_only,email, userId,adminName){
    console.log("claim submitted for org",organization_key,church_name, church_website,read_only);
    const mailDomain= process.env.MAIL_DOMAIN;
    const org = (await (await getServerDB()).getOrganization(organization_key));

    //update name data in fusionauth
    await updateFusionUser(userId,email,{fullName:adminName});


    await sendEmail({
        from: "information@"+mailDomain,
        to: "information@"+mailDomain,
        subject: "Organization claim",
        text:` Organization claim

        Add user ${userId} to ProfileManager group.

         org key: ${organization_key}
         org name: ${org.display_name}
         church_name: ${church_name}
         church_website: ${church_website}
         read_only: ${read_only}
         admin name: ${adminName}
         user email: ${email}
         user_key: ${user_key}
         external_user_id: ${userId}`
    });
}
export async function userHasPermissionOnProfile(external_user_id,missionary_profile_key){
    const allowedProfiles = await getManagedProfiles(external_user_id);
    console.log("does user "+external_user_id+" have permission on "+missionary_profile_key,allowedProfiles);
    const match = allowedProfiles.find(p => p.missionary_profile_key === missionary_profile_key);
    console.log("found match: ",match);
    return match != null;
}
export async function getManagedProfiles(external_user_id){
    console.log("getting managed profiles for "+external_user_id);

    const filter = (await (await getServerDB()).getUserOrgSearchFilter(external_user_id));
    if(filter == null)
        return []; // no permissions to manage — return empty array, not undefined.
                   // res.send(undefined) yields an empty body that the client's
                   // jQuery JSON parser rejects (parsererror), which crashes the
                   // profile route for brand-new users.

    const ro_profile_keys = filter.search_filter.ro_profile_keys || [];
    // remove ro_profile_keys from search_filter so it does not break query later
    delete filter.search_filter.ro_profile_keys;

    const allProfiles = {};
    const user_key = filter.user_key;


    //console.log("running filter: ",filter);
    if(filter != null && filter.search_filter != null){
        // add owned profiles into missionary_profile_keys array
        if(filter.search_filter.missionary_profile_keys == null)
            filter.search_filter.missionary_profile_keys = [];

        filter.search_filter.missionary_profile_keys = 
                filter.search_filter.missionary_profile_keys.
                    concat(filter.owned_profile_keys || []).
                    concat(ro_profile_keys);

        console.debug("managed profiles search params: ",filter.search_filter);
        // run filter, add profile_keys into set, insert into cached_user_permissions
        var profiles =(await getProfilesForFilter((await getServerDB()),filter.search_filter));
        //console.log("profiles:",profiles);

        if(profiles != null)
            for(const profile of profiles){
                allProfiles[profile.missionary_profile_key] = {
                    missionary_name: profile.missionary_name,
                    state: profile.state,
                    published: profile.data.published,
                    missionary_profile_key: profile.missionary_profile_key,
                    organization_display_name: profile.organization_display_name,
                    picture_url: profile.data.picture_url,
                    ro: ro_profile_keys.includes(profile.missionary_profile_key),
                    owned: profile.user_key === user_key,
                }
            }
    }

    if(user_key == null){
        console.warn("in updatePermissions, no filters found");
    }else{
        console.log("updating permission cache for user "+user_key,allProfiles);
        // update cached_user_permissions
        // delete all entries for user_key
        await (await getServerDB()).clearUserPermissionCache(user_key);

        // insert new entires for each profileKeys, if not read_only
        const rwKeys = Object.values(allProfiles).filter(p => ! p.ro).map(p => p.missionary_profile_key);
        await (await getServerDB()).cacheUserPermissions(user_key,rwKeys);
    }
    return Object.values(allProfiles);
    
}
export async function grantUserOrgPermission(user_key, organization_key, read_only){

  try{
    await (await getServerDB()).insertUserProfilePermissions(
                  user_key, organization_key, read_only);
    //also add org to orgs search_filter
    const org = (await (await getServerDB()).getPlainOrganization(organization_key));
    if(org.search_filter == null)
        org.search_filter={};

    if(org.search_filter.organization_keys == null)
        org.search_filter.organization_keys=[];
    
    if( ! org.search_filter.organization_keys.includes(String(organization_key))){
        org.search_filter.organization_keys.push(String(organization_key));
        await (await getServerDB()).updateOrganization(organization_key,org);
    }



  }catch(error){
    console.error("grantUserOrgPerm error: ",error);
    throw new AppError(error.message || error);
  }
}
export function subdomainRedirect(res,hostname,page){

    const domain = process.env.DOMAIN;
    let subdomain = hostname.replace(domain,"").replace(".","");
    //console.log(` ================ page: ${page}, subdomain: ${subdomain}`);
    if(subdomain !== ""){
        if(page == null || page === "")
            page = subdomain;
        console.log(`REDIRECTING from subdomain ${subdomain} to https://${domain}/${page}`)
        res.redirect(`https://${domain}/org/${page}`);
        return true;
    }
    return false;
}
export async function addROProfile(userId, missionary_profile_key){

    const perms = await (await getServerDB()).getUserOrgSearchFilter(userId);
    const organization_key = perms.organization_key;
    const org = await (await getServerDB()).getPlainOrganization(organization_key);

    if(org.search_filter == null)
        org.search_filter={};

    // see if we already have permission on this profile
    if((org.search_filter.missionary_profile_keys != null
            && org.search_filter.missionary_profile_keys.includes(missionary_profile_key) )
        ||(perms.owned_profile_keys != null && perms.owned_profile_keys.includes(missionary_profile_key)) )
        return false;

    if(org.search_filter.ro_profile_keys == null)
        org.search_filter.ro_profile_keys = [];

    console.log("ro keys: ",org.search_filter.ro_profile_keys,missionary_profile_key);
    if( ! org.search_filter.ro_profile_keys.includes(missionary_profile_key)){
        console.log("adding new key to RO keys and saving");
        org.search_filter.ro_profile_keys.push(missionary_profile_key);
        await (await getServerDB()).updateOrganization(organization_key,org);
        return true;
    }
    return false;

}

export async function txDetails(token,possible_transaction_key){

    const allowedTx= (await getDB(token).getWorkerTransaction(possible_transaction_key));
    if(allowedTx == null || allowedTx.length == 0)
        return {} //Either tx didn't exist, or we don't have permisison on it

    //now get the full tx info, including stripe_id
    const tx = (await (await getServerDB()).getPossibleTransaction(possible_transaction_key));
    console.log("found tx ",tx);
    if(tx.stripe_id == null || tx.stripe_id === "")
        return {}; //tx not made with stripe

    const stripeAccount = await stripeUtils.getStripeAccountForProfile(tx.missionary_profile_key);
    const customer = await stripeUtils.getCustomerFromStripeId(tx.stripe_id, stripeAccount);

    return {
        name: customer.name,
        email: customer.email,
    };
}

export async function getWorkerDonations(userId,token){

    let stripeAccount = undefined;
    try{
        //get organization_key from user_org_search_filters
        let org_key =  (await getDB(token).getUserOrgSearchFilter(userId)).organization_key;
        //then get stripe_account from org info
        let org = (await getDB(token).getOrganization(org_key));
        stripeAccount = org.stripe_account;
    }catch(e){
        //ok if no org info, just means we won't use a connected stripe account
    }


    const allowedTxs= (await getDB(token).getWorkerTransactions());
    if(allowedTxs == null || allowedTxs.length == 0)
        return {} //Either tx didn't exist, or we don't have permisison on it

    //console.log("got txs: ",allowedTxs);

    //get unique list of customer ids
    const customerIds = allowedTxs.reduce((acc,tx) => {
        if(tx.stripe_customer_id && !acc.includes(tx.stripe_customer_id))
            acc.push(tx.stripe_customer_id);
        return acc;
    },[]);
    //console.log("customer ids: ",customerIds);


    const customerDetails = await stripeUtils.customerDetails(customerIds, stripeAccount);

    // add customer details to each tx
    allowedTxs.forEach(tx => {
        let details = customerDetails[tx.stripe_customer_id];
        if(details != null){
            delete tx.stripe_customer_id;
            delete tx.donor_key;
            tx.donor_name = details.name;
            tx.email = details.email;
            tx.phone = details.phone;
            if(details.address != null){
                tx.city = details.address.city;
                tx.country= details.address.country;
                tx.line1= details.address.line1;
                tx.line2= details.address.line2;
                tx.postal= details.address.postal;
                tx.state= details.address.state;
            }
        }
    });

    return allowedTxs;
}



//Donations the logged in user has *made* (as opposed to received as a worker), found by
//matching their login email against stripe customers on the platform and every connected
//account. The transactions themselves come from our own records (which carry the worker
//name); stripe is also asked for any live subscriptions so the donor can be offered a
//billing-portal link to manage them.
export async function getDonorDonations(email){
    const empty = {is_donor: false, donations: [], subscriptions: [], receipt_years: []};
    if(email == null || email === "") return empty;

    const customers = await stripeUtils.findCustomersByEmail(email);
    if(customers.length === 0) return empty;

    //amounts and dates come from stripe's charges (our possible_transactions table holds one
    //row per subscription, not per monthly payment, so it can't be used for money totals);
    //our own records only supply the attribution - which worker each charge was given to.
    const charges = await stripeUtils.donorCharges(customers);
    const donations = await attributeCharges(charges);

    //years the donor can claim an Ergatas-issued receipt for: gifts to orgs that issue their
    //own receipts aren't ours to receipt
    const receipt_years = Array.from(new Set(
            donations.filter( d => d.provides_tax_receipt !== true)
                     .map( d => receiptYearOf(d.created_on))))
        .sort( (a,b) => b - a);

    const subscriptions = await stripeUtils.donorSubscriptions(customers);

    return {
        is_donor: true,
        donor_name: (customers.find( c => c.name) || {}).name || null,
        donations,
        subscriptions,
        receipt_years,
    };
}

//Turns raw stripe charges into donation records, naming the worker and organization each gift
//went to. Charges we can't tie back to a recorded transaction are dropped: without knowing the
//receiving organization we can't say who is responsible for receipting the gift.
async function attributeCharges(charges){
    const stripeIds = Array.from(new Set(charges.map( c => c.stripe_id).filter( id => id != null)));
    const byStripeId = {};

    if(stripeIds.length > 0){
        try{
            const txs = await (await getServerDB()).getDonationsByStripeIds(stripeIds);
            (txs || []).forEach( tx => byStripeId[tx.stripe_id] = tx);
        }catch(error){
            console.error("failed to look up transactions for donor charges: ",error);
        }
    }

    const donations = charges.map( charge =>{
        const tx = byStripeId[charge.stripe_id];
        if(tx == null){
            console.warn("donor charge "+charge.id+" has no recorded transaction ("+charge.stripe_id+")");
            return null;
        }
        return {
            charge_id: charge.id,
            missionary_profile_key: tx.missionary_profile_key,
            name: tx.name,
            amount: charge.amount,
            currency: charge.currency,
            donation_type: charge.donation_type,
            created_on: (new Date(charge.created * 1000)).toISOString(),
            receipt_url: charge.receipt_url,
        };
    }).filter( d => d != null);

    await addOrganizationInfo(donations);

    return donations;
}

//Annotates donations in place with the receiving worker's organization, which the receipt needs
//(and which decides whether Ergatas or the organization issues it).
async function addOrganizationInfo(donations){
    const profileKeys = Array.from(new Set(donations.map( d => d.missionary_profile_key).filter( k => k != null)));
    if(profileKeys.length === 0) return;

    try{
        const profiles = await (await getServerDB()).getDisplayProfilesByKey(profileKeys);
        const byKey = {};
        (profiles || []).forEach( p => byKey[p.missionary_profile_key] = p);
        donations.forEach( d =>{
            const profile = byKey[d.missionary_profile_key];
            if(profile != null){
                d.organization_name = profile.organization_display_name;
                //true => the organization issues its own receipt (donation_settings.send_receipt
                //is false); false => the gift is receipted by Ergatas
                d.provides_tax_receipt = profile.provides_tax_receipt;
            }
        });
    }catch(error){
        //without it we can't tell whose receipt a gift belongs on, so treat it as not ours
        console.error("failed to load organizations for donor donations: ",error);
        donations.forEach( d => d.provides_tax_receipt = true);
    }
}

//Contact and registration details for the receipt. Set these in the environment; without them
//the receipt still renders but says so plainly rather than quietly omitting required details.
//Which calendar year a gift falls in decides which receipt it lands on, so a charge just either
//side of midnight on new year's must not shift with the server's timezone. Everything on the
//receipt - the year list, the year filter and the printed dates - is resolved in this one zone.
function receiptTimeZone(){
    return process.env.RECEIPT_TIMEZONE || "America/New_York";
}
export function receiptYearOf(dateStr){
    return parseInt((new Intl.DateTimeFormat('en-US',
        {timeZone: receiptTimeZone(), year:'numeric'})).format(new Date(dateStr)));
}
function receiptDayOf(dateStr){
    return (new Intl.DateTimeFormat('en-US',
        {timeZone: receiptTimeZone(), year:'numeric', month:'short', day:'numeric'})).format(new Date(dateStr));
}

function receiptIssuer(){
    return {
        name: process.env.RECEIPT_LEGAL_NAME || "Ergatas",
        ein: process.env.RECEIPT_EIN || "",
        address: process.env.RECEIPT_ADDRESS || "",
    };
}

//the receipt is written into an about:blank window by the browser, so relative image paths
//would not resolve. Embed the logo as a data URI instead, cached after the first read.
let receiptLogoDataUri;
function receiptLogo(){
    if(receiptLogoDataUri === undefined){
        try{
            const svg = fs.readFileSync(path.join(process.cwd(),"public","img","ergatas-logo-green.svg"));
            receiptLogoDataUri = "data:image/svg+xml;base64,"+svg.toString("base64");
        }catch(error){
            console.error("failed to read ergatas logo for donation receipt: ",error);
            receiptLogoDataUri = null;
        }
    }
    return receiptLogoDataUri;
}

//A printable year-end tax receipt for everything the donor gave through Ergatas in the given
//year that Ergatas itself receipts. Gifts to organizations that issue their own receipts
//(provides_tax_receipt) are deliberately excluded so the donor is never receipted twice for
//the same gift. Amounts come from stripe charges, so recurring gifts appear once per payment.
export async function donationReceipt(email,year){
    const requestedYear = parseInt(year);
    if(!Number.isInteger(requestedYear))
        throw new AppError("a valid year is required");

    const {donations,donor_name} = await getDonorDonations(email);

    return renderDonationReceipt(email,donor_name,requestedYear,donations);
}

//split out from donationReceipt so the document can be rendered (and checked) without
//reaching stripe
export function renderDonationReceipt(email,donor_name,requestedYear,donations){
    const forYear = (donations || [])
        .filter( d => d.provides_tax_receipt !== true)
        .filter( d => receiptYearOf(d.created_on) === requestedYear)
        .sort( (a,b) => new Date(a.created_on) - new Date(b.created_on));

    //gifts can settle in more than one currency, and they can't be summed across currencies
    const totals = {};
    forYear.forEach( d =>{
        const currency = (d.currency || "usd").toUpperCase();
        totals[currency] = (totals[currency] || 0) + (parseFloat(d.amount) || 0);
    });

    const issuer = receiptIssuer();
    const logo = receiptLogo();
    const money = (amount,currency) =>{
        try{
            return (new Intl.NumberFormat('en-US',
                {style:'currency',currency:(currency || 'usd').toUpperCase()})).format(amount);
        }catch(error){
            return amount+" "+(currency || "").toUpperCase();
        }
    };

    const rows = forYear.map( d => `<tr>`+
            `<td>${escapeHtml(receiptDayOf(d.created_on))}</td>`+
            `<td>${escapeHtml(d.name || "")}</td>`+
            `<td>${escapeHtml(d.organization_name || "")}</td>`+
            `<td>${escapeHtml(d.donation_type === "recurring" ? "Monthly" : "One-time")}</td>`+
            `<td class="amount">${escapeHtml(money(d.amount,d.currency))}</td>`+
        `</tr>`).join("\n");

    const totalRows = Object.keys(totals).sort().map( currency =>
        `<tr><td colspan="4">Total contributions ${escapeHtml(String(requestedYear))}</td>`+
        `<td class="amount">${escapeHtml(money(totals[currency],currency))}</td></tr>`).join("\n");

    const issuerLines = [
        issuer.ein ? `EIN ${escapeHtml(issuer.ein)}` : `<span class="missing">EIN not configured</span>`,
        issuer.address ? escapeHtml(issuer.address) : `<span class="missing">Address not configured</span>`,
    ].join("<br/>");

    return `<!doctype html>
<html lang="en">
<head>
<meta charset="utf-8"/>
<title>${escapeHtml(issuer.name)} Tax Receipt ${requestedYear}</title>
<style>
  body{font-family: Helvetica, Arial, sans-serif; color:#222; margin: 2rem auto; max-width: 50rem; padding: 0 1rem;}
  h1{font-size:1.5rem; margin-bottom:.25rem;}
  .logo{height:3.5rem; width:auto; margin-bottom:1rem;}
  .issuer{font-size:.9rem; color:#444; line-height:1.5; margin-bottom:1.5rem;}
  .missing{color:#b00; font-style:italic;}
  .meta{margin:1.5rem 0; font-size:.95rem; line-height:1.6;}
  .meta strong{display:inline-block; min-width:8rem;}
  table{width:100%; border-collapse:collapse; margin-top:1rem;}
  th,td{text-align:left; padding:.5rem .4rem; border-bottom:1px solid #ddd; font-size:.95rem;}
  th{border-bottom:2px solid #888;}
  .amount{text-align:right; white-space:nowrap;}
  tfoot td{font-weight:bold; border-bottom:none; border-top:2px solid #888;}
  .statement{margin-top:2rem; font-size:.9rem; color:#333; line-height:1.6;}
  .usa{margin-top:1.5rem; padding:.75rem 1rem; border:1px solid #888; font-size:.85rem; line-height:1.5;}
  .noprint{margin-top:2rem;}
  @media print{ .noprint{display:none;} body{margin:0;} }
</style>
</head>
<body>
${logo ? `<img class="logo" src="${logo}" alt="${escapeHtml(issuer.name)}"/>` : ''}
<h1>Contribution Receipt &mdash; ${requestedYear}</h1>
<div class="issuer">${escapeHtml(issuer.name)}<br/>${issuerLines}</div>

<div class="meta">
<strong>Donor:</strong> ${escapeHtml(donor_name || email)}<br/>
<strong>Donor email:</strong> ${escapeHtml(email)}<br/>
<strong>Tax year:</strong> ${requestedYear}<br/>
<strong>Issued:</strong> ${escapeHtml((new Date()).toLocaleDateString('en-US',{year:'numeric',month:'long',day:'numeric'}))}
</div>

<table>
<thead><tr><th>Date</th><th>Designated Worker</th><th>Organization</th><th>Type</th><th class="amount">Amount</th></tr></thead>
<tbody>
${rows || '<tr><td colspan="5">No contributions receipted by ' + escapeHtml(issuer.name) + ' for this year.</td></tr>'}
</tbody>
<tfoot>
${totalRows || '<tr><td colspan="4">Total contributions ' + requestedYear + '</td><td class="amount">' + escapeHtml(money(0,"usd")) + '</td></tr>'}
</tfoot>
</table>

<p class="statement">
No goods or services were provided by ${escapeHtml(issuer.name)} in exchange for these contributions.
Contributions are shown net of any refunds. Each gift was designated for the ministry of the worker
named above. Gifts made through Ergatas to organizations that issue their own contribution
receipts are not included here &mdash; those receipts come directly from the organization.
</p>

<div class="usa">
<strong>United States donors only.</strong> ${escapeHtml(issuer.name)} is a United States tax-exempt
organization, and this receipt is valid only as a record of a charitable contribution for United
States federal income tax purposes. Donors outside the United States should consult their own tax
authority; this document does not establish deductibility in any other country.
</div>

<div class="noprint"><button onclick="window.print()">Print or save as PDF</button></div>
</body>
</html>`;
}

export async function copyEmailTemplate(sourceTemplate,destTemplate){
    //use mailgun api to fetch source template, then create dest template
    const mg = getMailgun()
    const domain = process.env.MAILGUN_DOMAIN;

    console.info(`copying email template from ${sourceTemplate} to ${destTemplate}`);

    let sourceData;
    try{
        sourceData = await mg.domains.domainTemplates.get(domain,sourceTemplate,{active:"yes"});
        console.info(`fetched source template: `,sourceData);
    }catch(error){
        console.error("failed to fetch source template: ",error);
        throw error;
    }

    //create dest template
    const destData = {
        name: destTemplate,
        description: sourceData.description || `Copy of ${sourceTemplate}`,
        engine: sourceData.version.engine,
        tag: sourceData.version.tag,
        template: sourceData.version.template,
    };

    try{
        const body = await mg.domains.domainTemplates.create(domain,destData);
        console.info(`created destination template: `,body);
        return body;
    }catch(error){
        console.error("failed to create destination template: ",error);
        throw error;
    }
}