...
export async function getWorkerDonations(organization_key){
    ...
    const customerDetails = await stripeUtils.customerDetails(customerIds, org);
    console.log("customer info : ",customerDetails);

    // add customer details to each tx
    allowedTxs.forEach(tx => {
        tx.donor = customerDetails[tx.stripe_customer_id];
    });
...
//info = await  stripeUtils.customerDetails(customer_ids)
