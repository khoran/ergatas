...
export async function sendDonationNotice(possible_transaction_key,external_user_id){
    ...
    try{
        ...
        const profile = (await getDB().getProfileByKey(tx.missionary_profile_key)).data;
...
export async function claimOrg(orgType, organization_key, church_name, church_website,user_key,email, userId){
    console.log("claim submitted for org",orgType, organization_key,church_name, church_website);
    const mailDomain= process.env.MAIL_DOMAIN;
    await sendEmail({
        from: "information@"+mailDomain,
        to: "information@"+mailDomain,
...
export async function updatePermissions(external_user_id){
    console.log("updating permissions for "+external_user_id);

    const filters = (await getDB().getUserOrgSearchFilters(external_user_id)).data;
    ...
    for( const row of filters ){
        console.log("running row: ",row);
        if(row.search_filter != null){
            // run row, add profile_keys into set, insert into cached_user_permissions
            (await getProfilesForFilter(getDB(),row.search_filter)).
                forEach(key => profileKeys.add(key));
            if(user_key == null) //user_key should be the same for all rows
    ...
    if(user_key == null){
        ...
        console.log("updating permission cache for user "+user_key,profileKeys);
