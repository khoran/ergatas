...
import { AlphaAnalyticsDataClient } from '@google-analytics/data/build/src/v1alpha/alpha_analytics_data_client.js';
...
const removeFile = (userId,filename,missionary_profile_key=null) =>{
    const bucket = getUploadBucket();
    const prefix = checkProfileBucketPermissions(userId,missionary_profile_key);
    return bucket.deleteBlob(prefix+"/"+filename);
}
const removeAllFiles =  async (userId,missionary_profile_key=null) =>{
    const bucket = getUploadBucket();
    const prefix = checkProfileBucketPermissions(userId,missionary_profile_key);
...
const getUserBucketFiles = async (prefix,bucket) =>{
...
const userFileLinks = async (prefix) =>{
    const bucket = getUploadBucket();
    var files = await getUserBucketFiles(prefix);
...
const deleteUserFromDB = async (user_key) =>{
...
const buildIndex = async function(pageName,info,url){
    ...
    try{
        ...
        }else if(pageName === "search" && ! url.match(/^\/search\/saved\//)){
...
export async function claimOrg(organization_key, church_name, church_website,user_key,read_only,email, userId){
...
export async function getManagedProfiles(external_user_id){
    ...
    if(user_key == null){
        ...
        // insert new entires for each profileKeys
        //await getDB().cacheUserPermissions(user_key,Array.from(profileKeys));
