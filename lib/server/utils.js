...
export function setCustomCacheControl(res, path) {
    // express-static-gzip may hand us the pre-compressed variant's path, so
    // allow optional .br/.gz suffixes.
    if (/\.[0-9a-f]{12,}\.(js|css)(\.br|\.gz)?$/.test(path) || /[\\/]webfonts[\\/]/.test(path)) {
        //content-hashed build output and versioned webfonts never change in place
        res.setHeader('Cache-Control', 'public, max-age=31536000, immutable');
    } else if (/(service-worker\.js|asset-manifest\.json)(\.br|\.gz)?$/.test(path)) {
        //must revalidate every time or clients could be stuck on an old build
        res.setHeader('Cache-Control', 'no-cache');
    } else if (serveStatic.mime.lookup(path).startsWith('image/')) {
