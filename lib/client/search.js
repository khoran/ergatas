...
export class Search {
    ...
    genSearchParams(filterSet){
        var params={};
        var arraysToCast= [ filterSet.skills.name(),
                            filterSet.organizations.name(),
                            filterSet.movementStages.name(),
                            filterSet.culturalDistances.name()];
        var filterValue;
        var extraPGs = new Set();
        
        Object.values(filterSet).forEach( filter =>{
            if(filter.isDefined()){

                filterValue = filter.obs()();

                if(filter.name() === "ageGroups"){
                    params.birth_years = utils.ageGroupsToBirthYears(filterValue).
                                                    map( x =>parseInt(x));
                } else if(filter.name() === "pgSets"){
                    //for each 'set' in pgSets, add all people group ids to extraPGs unique set
                    if(filterValue != null){
                        Object.values(filterValue).forEach( set =>{
                            set.forEach( id => extraPGs.add(id));
                        })
                    }
                } else if( arraysToCast.indexOf(filter.name()) !== -1 ){
                    //cast values to int
                    params[filter.name()] = filterValue.map( x =>parseInt(x));
                }else
        ...
        var arr = Array.from(extraPGs.values());
        if(arr.length > 0){
    ...
    async doSearch(){
        ...
        try{
            searchResults = await self.da.primarySearch(params,pageSize,sortBy);
            ...
            if(searchResults!= null){
                ...
                if(self.numSearchResults() < self.FUZZY_THRESHOLD)
                    await self.addFuzzyResults(searchResults,params,pageSize,sortBy);
            }
    ...
    async addFuzzyResults(searchResults,params,pageSize,sortBy){
        ...
        //NOTE: don't use self.allResults and others as a source, as there will be a race condition
        ...
        var fuzzyResults= await self.da.primarySearch(params,pageSize,sortBy,true);
    ...
    saveSearch(name,user_key){

        var searchParams = this.filterValues(this.filter);
        return this.da.createSavedSearch(user_key,
                            {
                                name: name,
                                params: searchParams,
                            });
