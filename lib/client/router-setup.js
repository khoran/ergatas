...
export function initRouter(client, {appBase, authBaseUrl, authClientID}){
    ...
    //increments on every navigation; an async template load that finishes
    // after a newer navigation started must not overwrite the newer page
    var navCounter = 0;

    //the very first render on a direct/refreshed load has no meaningful previous
    // page to slide away from (just the server prerender / hydration spinner), so
    // skip the transition animation and swap the real page in instantly.
    var firstNav = true;

    function setPage(pageName,pageFn){

        var _setPage = async function(){
            ...
            try{
                if(self.pageInfo[pageName].alias_for && self.pageInfo[self.pageInfo[pageName].alias_for]){
                    newPage = (await self.getPage(self.pageInfo[pageName].alias_for)).clone();
                }else{
                    newPage = (await self.getPage(pageName)).clone();
                }
            }catch(error){
                //most likely a lazy template chunk failed to load (offline or
                // a stale deploy); page-error is always available synchronously
                console.error("failed to load template for page "+pageName,error);
                newPage = self.getPageSync("page-error").clone();
            }
            if(myNav !== navCounter)
                return; //a newer navigation superseded this one while the template loaded
            if(pageFn)
                pageFn(newPage);

            //remember the page we're leaving so worker-profile views can record where the
            //visitor came from (view_worker_profile source). Must be set before page()
            //renders the new page, which is when the worker component mounts and reads it.
            utils.setNavSource(viewModel.currentPageName());
            viewModel.page(newPage);
            viewModel.currentPageName(pageName);
            viewModel.navDrawerOpen(false); //any navigation closes the mobile nav drawer
    ...
    // Saved-search names are percent-encoded when the URL is built (see
    // encodeURIComponent call sites for "search/saved/..." links). Old links
    // built before that encoding existed may still have raw characters with
    // no "%" escapes, so decodeURIComponent is a safe no-op for those.
    function decodeSearchName(searchName){
        try{
            return decodeURIComponent(searchName);
        }catch(e){
            return searchName;
        }
    }
    ...
    const router = new Router({
        ...
        page404: function (path) {
            ...
            viewModel.page(self.getPageSync("not-found").clone());
        }
    });
    ...
    router.add('search/', async function () {
        ...
        //rehydrate all filters + view from the URL query string (reload / shared link
        /// back-forward). server is needed to re-expand pgSets by name.
        self.search.applyQueryString(window.location.search, viewModel.appState.server);
        setSearch();
    });
    ...
    async function loadPublicSearch(searchName){
        searchName = decodeSearchName(searchName);
    ...
    // Route: /search/public/<searchName>
    router.add(/^search\/public\/([^\/]+)$/, async function (searchName) {
        await loadPublicSearch(searchName);
    });
    ...
    // searchName is free text (see search.js saveSearch) and gets percent-encoded
    // when the URL is built, so match any non-slash segment and decode it here
    // rather than whitelisting characters - a whitelist mismatch here previously
    // sent names with punctuation like "," or "!" to the wiki-page fallback route.
    router.add(/^search\/saved\/([^\/]+)\/([^\/]+)$/, async function (userKey, searchName) {
        searchName = decodeSearchName(searchName);
    ...
    router.add(/^search\/saved\/([^\/]+)$/, async function (searchName) {
        searchName = decodeSearchName(searchName);
        ...
        if(viewModel.loggedIn()){
           ...
           console.log("saved searches require login first, redirecting");
            signInWithRedirect("search/saved/"+encodeURIComponent(searchName));
        }
    ...
    router.add('worker/(:any)', async function (slug) {
        ...
        try{
            //on a fresh direct load the server inlines this profile
            //(window.__ergatas_prefetch) so we can render without the fetch
            //round-trip. Consume it once so later in-app navigations fetch fresh.
            let profile = null;
            if(window.__ergatas_prefetch && window.__ergatas_prefetch.slug === slug){
                profile = window.__ergatas_prefetch.profile;
                window.__ergatas_prefetch = null;
            }
            if(profile == null)
                profile = await self.da.getProfileBySlug(slug);
            if(profile != null){
    ...
    router.add('docs/{name}', function (name) {
        console.log("loading docs article "+name);
        // All docs render inside the single 'docs' shell page; the docs component
        // reads the article name from the URL and swaps content in place.
        setPage('docs');
    });
