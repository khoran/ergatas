...
export function setupPWA(client){
    ...
    window.addEventListener('beforeinstallprompt', function(evt){
        ...
        if(alertify.installPrompt == null) //register lazily; not needed on the boot path
...
export function registerServiceWorker(client){
    ...
    if ('serviceWorker' in navigator) {
        // reload once when a NEW Service Worker replaces an existing one. On the
        // very first visit clientsClaim() also fires controllerchange (controller
        // goes null -> SW); reloading then would restart the page seconds after
        // load, which tanks LCP and looks like a redirect to Lighthouse.
        var hadController = !!navigator.serviceWorker.controller;
        navigator.serviceWorker.addEventListener('controllerchange',function(){
            if(!hadController){
                console.log("SW took control on first install, skipping reload");
                hadController = true;
                return;
            }
            reload();
