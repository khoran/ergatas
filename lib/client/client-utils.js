...
// Tracks the in-app page the user navigated FROM, so worker-profile views can be scoped
// to arrivals from /learn/* or /search (see view_worker_profile in worker.js). Set by the
// router in setPage before it swaps currentPageName.
var navSource = null;
export function setNavSource(source){ navSource = source; }
export function getNavSource(){ return navSource || document.referrer || null; }
export function donate(da,profile,source,ownProfile){
    ...
    dataLayer.push({event:'click_donate',
                'donate-event-source':source,
                missionary_profile_key:profile.missionary_profile_key});
...
// Fires the donation-completion signal (GA4 "confirmed-donation" via GTM). Historically
// only the off-site self-report path (donate-popup.js confirmDonation) pushed this event,
// so on-site Stripe completions went untracked — begin_checkout:confirmed-donation ran
// ~80:1. Both on-site completion points (direct-donation-popup onComplete and the
// redirect-return handler in session.js) now call this so completions are counted. Uses
// the same event name/fields as the off-site path so no GTM change is needed.
export function recordConfirmedDonation(missionary_profile_key,amount,donationType){
    dataLayer.push({event:"donate-confirmation",
                'donate-level':2,
                'missionary_profile_key':missionary_profile_key,
                'confirmed-donation-amount':amount,
...
export function selectedKeys(selectedKeys, keyLookup,keyField,nameField,keysAsInt=true){
    ...
    const byKey = new Map(keyLookup.map(item => [item[keyField], item]));
...
export function selectedKeysAsArray(selectedKeys, keyLookup,keyFieldName,nameFieldName,asObject){
    ...
    const byKey = new Map(keyLookup.map(item => [item[keyFieldName], item]));
    return selectedKeys.map(function(key){
        key = parseInt(key);
...
// Multi-select selectize for choosing ISO 4217 currency codes.
// currencies: array of {code,name,symbol}; the bound observable holds an array of codes.
export function currencySelectizeOptions(currencies){
    return {
        create: false,
        valueField: "code",
        labelField: "name",
        ...
        render:{
            option: function(data,escape){
                return "<div>"+escape(data.code)+" &mdash; "+escape(data.name)+"</div>";
            },
...
// Single-select selectize for choosing one country by name.
// countries: an observable holding an array of {alpha3Code,name,...} (e.g.
//   appState.countryList.listObs()); it may be null until the list loads, so
//   options are populated in onInitialize and refreshed via watchForNewOptions.
// The bound observable holds a single alpha-3 code string (or empty/undefined).
export function countrySelectizeOptions(countries){
    return {
        create: false,
        valueField: "alpha3Code",
        labelField: "name",
        searchField: ["name","alpha3Code"],
        maxItems: 1,
        onInitialize: function(){
            var api = this;
            ko.bindingHandlers.selectize.utils.setOptions(api,countries);
            ko.bindingHandlers.selectize.utils.watchForNewOptions(api,countries);
        },
...
export function contactMissionaryFn(external_user_id,server){
    return async function(name, email,message){
        ...
        try{
            //braces added: previously the push+toast sat outside the if, so the
            //message-to-missionary event fired even when nothing was sent (empty message),
            //inflating the count. Now they only fire when a message is actually sent.
...
export function quickSearchFn(appState,filterName,keyField,asArray=false){
      return function(data){
          //Navigate to a /search URL that encodes just this one filter. The search
          //page's applyQueryString reader clears any prior filters and hydrates this
          //one - setting the observable and navigating to bare /search would be wiped
          //by that reader. filter.name() is the DB field name used in the URL.
          var value = asArray ? [data[keyField]] : data[keyField];
          var params = new URLSearchParams();
          params.set(appState.search.filter[filterName].name(),
                     Array.isArray(value) ? value.join(",") : value);
          var qs = params.toString();
          appState.router.navigateTo('search' + (qs ? "?"+qs : ""));
      }
  }
...
export function getSearchNameFromURL(){
    ...
    // user_key (search_key) is expected to be an integer when present. searchName is
    // free text and gets percent-encoded when the URL is built (see search.js
    // saveSearch), so match any non-slash segment against the raw pathname and decode
    // after extraction rather than whitelisting characters or pre-decoding the whole
    // path - a whitelist mismatch here previously broke names with punctuation.
    var match = /^\/search\/saved\/(?:\d+\/)?([^\/]+)/.exec(url.pathname);
...
//all-time totals plus a recent window and the window before it, for the
//dashboard's stats panel. See getProfileStatsTrend in lib/server/utils.js.
export function pageStatsTrend(server,missionary_profile_key,days){
    return server.postJson("/api/profileStatsTrend",
        { missionary_profile_key: missionary_profile_key, days: days});
}
...
export function celebrateDonation(){
    var colors = ['#f9d71c', '#ff5e5e', '#4fd1c5', '#7c5cff'];
    var duration = 2500;
    ...
    el.className = 'donation-celebration';
    el.textContent = "🎉 Thank you for your generous gift — you're changing lives around the world!";
    ...
    try{
        if(navigator.clipboard && navigator.clipboard.writeText){
            navigator.clipboard.writeText(text)
                ...
                .catch(function(){ alertify.error("Failed to copy link"); });
        }else{
            var tmp = document.createElement('input');
            tmp.value = text;
            document.body.appendChild(tmp);
            tmp.select();
            try{ document.execCommand('copy'); alertify.success("Link copied to clipboard"); }
            catch(e){ alertify.error("Failed to copy link"); }
