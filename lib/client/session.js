...
export function handleDonationResult(client){
    ...
    //mpk lets the server resolve the connected account when retrieving the session.
    var missionary_profile_key = utils.getURLParameter("mpk");

    //unauthenticated: donors are typically anonymous, and the status endpoint doesn't need auth.
    client.server.postJson("/api/checkoutSessionStatus",{
        checkoutSessionId: checkoutSessionId,
        missionary_profile_key: missionary_profile_key,
    }).then( result =>{
        console.log("checkout status: ",result);
        if(result.status === "open"){
            //Payment didn't complete (redirect-based method returned without finishing). Tell the
            //donor why — the specific decline reason when Stripe gives one, otherwise a generic
            //message.
            alertify.error(result.payment_error
                ? `Your payment could not be completed: ${result.payment_error} Please try again.`
                : "Your payment was not completed. Please try again.");
            //Per Stripe's guidance, re-mount checkout so the donor can retry. Hand the session off
            //to the matching direct-donation-popup, which re-opens and mounts it with client_secret.
            if(result.client_secret != null && client.appState && client.appState.pendingDonationResume){
                client.appState.pendingDonationResume({
                    missionary_profile_key: result.missionary_profile_key || missionary_profile_key,
                    client_secret: result.client_secret,
                    stripeAccount: result.stripeAccount,
                });
            }
        }else if(result.status === "complete"){ //checkout completed, show thank you message
            //record the completed donation (GA4 confirmed-donation) on the redirect-return
            //path — the embedded onComplete callback covers the no-redirect case, this
            //covers payment methods that redirect (redirect_on_completion: "if_required").
            if(result.missionary_profile_key != null)
                utils.recordConfirmedDonation(result.missionary_profile_key,
                                              result.amount,result.donation_type);
            utils.celebrateDonation();
        }

        //clear this parameter from the location bar so users don't reload it
        history.pushState({},'',location.origin+location.pathname);
    }).catch(error => console.error("failed to handle donation result: ",error));
...
// Populate the view model from a freshly-fetched user record + token. Shared by
// doRefresh and doLogin; callers handle setCookies / loadProfile themselves since
// those differ (refresh guards on hasProfile, login always runs them).
function applyUserSession(viewModel, user, tokenResult){
    user.email = tokenResult.email; //not saved, just displayed on page
    viewModel.loggedInUser(ko.mapping.fromJS(user));
    viewModel.termsConfirmed(viewModel.loggedInUser().agreed_to_sof());
    viewModel.termsConfirmedCheckBox(viewModel.termsConfirmed());
}
    ...
    try{
        ...
        if( ! self.viewModel.loggedIn()){
            ...
            if(userResults.length === 1){
                ...
                applyUserSession(self.viewModel, user, tokenResult);
            }
        }
    ...
    try{
        ...
        if(tokenResult.access_token != null){
            ...
            try{
                ...
                applyUserSession(viewModel, user, tokenResult);
                setCookies(self);
