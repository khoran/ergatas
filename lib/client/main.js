...
constructor(){
    ...
    // Docs articles are prerendered as a bare content fragment (no sidebar/shell —
    // see buildIndex in lib/server/utils.js) so search engines and no-JS visitors
    // get readable text. That fragment isn't a faithful snapshot of the real page
    // (unlike other prerendered pages), so always route a direct/refreshed load of
    // /docs/<article> through the router to mount the actual docs shell.
    var doNavigate=jQuery("#page_content").children().length == 0 || /^docs\//.test(state);
