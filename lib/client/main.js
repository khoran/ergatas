import * as utils from './client-utils';
import Logger from '../shared/logging';
import {DataAccess} from '../shared/data-access';
import {urlBase64ToUint8Array} from '../shared/shared-utils';
import { registerComponents } from './register-components';
import * as thirdParty from './third-party';
import * as templates from './templates';
import * as pwa from './pwa';
import * as session from './session';
import { initAutoHideNav } from './auto-hide-nav';
import { createViewModel } from './view-model';
import { initRouter } from './router-setup';
import alertify from 'alertifyjs';
import { ServerAPI } from './server-api';
import { CloudStorage } from '../server/cloud-storage';
import { Search } from './search';
import { AppState } from '../server/app-state';

console.log("main starting5");

class Client {

constructor(){
    const self=this;
    const bucketName = process.env.UPLOAD_BUCKET ;
    const bucketBaseUrl= process.env.BUCKET_BASE_URL;
    const authClientID=process.env.AUTH_CLIENT_ID
    const authRedirect=process.env.REDIRECT_URL;
    const postgrestBase= process.env.POSTGREST_URL_BASE;
    const authBaseUrl=process.env.AUTH_URL_BASE;
    const appBase=process.env.APP_BASE || '/';
    const logKey = process.env.LOG_KEY;
    const vapidPublicKey = process.env.VAPID_PUBLIC_KEY;
    const bucketBase=bucketBaseUrl+"/"+bucketName+"/";

    ko.options.deferUpdates = true;

    this.guidedSearchName = "Guided Search Profile";
    this.version = process.env.PACKAGE_VERSION;
    this.domain =  process.env.DOMAIN;
    this.maintenance_mode = process.env.MAINTENANCE_MODE ==="true" ;
    this.router;
    this.viewModel;
    this.pageInfo; //initialized by initTemplates
    this.server = new ServerAPI();
    this.storage = new CloudStorage(bucketBase);

    this.convertedVapidKey = urlBase64ToUint8Array(vapidPublicKey);

    console.info(" VERSION: "+this.version+", maintenance mode: "+this.maintenance_mode);

    //this will replace console.{log,info,error,warn} with remote logging
    new Logger((buffer) => {
        return self.server.postJson("/api/log",{
            key: logKey,
            logs:buffer
        });
    });

    thirdParty.initDataLayer(); //gtm.js itself is injected after window load, see start()


    this.da = new DataAccess(postgrestBase,jQuery.ajax, function(){return self.server.refreshToken();});
    this.search = new Search(appBase,this.da);
    this.server.token.subscribe((newToken) =>{
        //update da whenever the token is refreshed
        console.log("setting new JWT token in da after token obs update");
        self.da.setToken(newToken);
    });


    registerComponents();

    alertify.defaults.notifier.delay=15;
    alertify.defaults.notifier.position="top-center";
    alertify.defaults.transition="zoom";

    if(utils.debugMode())
        window.celebrateDonation = utils.celebrateDonation;
    templates.initTemplates(this);
    this.getPage = (name) => templates.getPage(this,name); //returns a Promise
    this.getPageSync = (name) => templates.getPageSync(this,name); //eager pages only
    this.pageExists = (name) => templates.pageExists(this,name);

    // Re-attach PWA helpers onto the instance so viewModel/router/start() callers resolve.
    this.registerPushSubscription = () => pwa.registerPushSubscription(this);
    this.setSubscriptionStatus = (obs) => pwa.setSubscriptionStatus(this,obs);
    // Re-attach so the router (profile/, confirm-sof/) can call client.isUserVerified().
    this.isUserVerified = () => session.isUserVerified(this);


    //console.warn("+++++++++  mode: "+process.env.NODE_ENV+"  +++++++++++++++++++++++");


    this.appState = new AppState(self.da,self.search,self.storage,self.server);
    this.viewModel = createViewModel(this,{authBaseUrl, clientId:authClientID, redirect:authRedirect, appBase, appState:this.appState});
    this.appState.loggedInUser = this.viewModel.loggedInUser;
    this.router = initRouter(this,{appBase, authBaseUrl, authClientID});
    this.appState.setRouter(this.router);

    pwa.setupPWA(this);

    var code = utils.getURLParameter("code");
    var state= utils.getURLParameter("state") || window.location.pathname.replace(appBase,"")  ;
    var hash= window.location.hash;
    // Docs articles and worker profiles are prerendered as a bare content fragment
    // (no sidebar/shell — see buildIndex in lib/server/utils.js) so search engines
    // and no-JS visitors get readable text. That fragment isn't a faithful snapshot
    // of the real page (unlike other prerendered pages), so always route a
    // direct/refreshed load of those pages through the router to mount the real page.
    var doNavigate=jQuery("#page_content").children().length == 0 || /^(docs|worker|profile-detail)\//.test(state);

    if(document.getElementById("is_search_page") != null){
        console.log("adding "+state+"as a search page");
        this.search.searchPages.push(state); //add current page as a search page
    }

    console.log("navigate?: ",doNavigate);
    console.log("code: "+code+", state: "+state+", hash: "+hash);

    //do a test login here to see if  have a cookie that will keep us logged in
    // also, encrypt the cookies

    if(code){
        session.doLogin(this, state ,code);
    }else{ //see if we can log user in based on refresh_token in cookie
        // Worker/profile-detail direct loads render from an inlined profile (or a
        // fetch that can run in parallel), so don't make them wait on the
        // session-refresh round-trip. The actual router.check() is deferred to
        // start() (after ko.applyBindings) — routing here would set viewModel.page
        // before bindings are applied, making the element binding's init+update
        // both bind the new page node (KO throws "bindings applied multiple
        // times"). doRefresh still runs in parallel so session-dependent bits
        // (favorite/edit buttons, etc.) resolve reactively. Other pages keep the
        // original refresh-then-navigate order.
        this.earlyNav = doNavigate && /^(worker|profile-detail)\//.test(state);
        session.doRefresh(this).then(() =>{
            if(doNavigate && ! self.earlyNav){
                self.router.check();
            }
        });
    }

}
start(){

    const self=this;

    //console.warn(" ============================== BINDINGS APPLIED =========================",(new Date()) - window.performance.timing.navigationStart);
    ko.applyBindings(this.viewModel);

    // Route worker/profile-detail direct loads now — right after bindings are
    // applied — instead of waiting on the session-refresh round-trip (see the
    // constructor). Deferring to here (post-applyBindings) means viewModel.page
    // is set after the element binding's init runs, so it renders exactly once.
    if(this.earlyNav){
        this.router.check();
    }

    session.handleDonationResult(this);

    // Heavy non-critical startup work waits for the load event so it doesn't
    // compete with first render for bandwidth/CPU on slow devices: GTM
    // (~130KB + script eval) and the service worker (precaches all bundles).
    //
    // GTM + the GA scripts it pulls in (~900ms of main-thread work on a
    // mid-range phone) and reCAPTCHA wait further still: until the visitor
    // first interacts (any pointer/key/scroll — dataLayer.push events queue
    // safely in the meantime), or 15s after load as a fallback so
    // non-interacting visits are still recorded. The fallback must sit well
    // past the ~5s network/CPU-quiet window that ends performance traces, or
    // these scripts get pulled back into the measured (and user-perceived)
    // startup cost.
    const startThirdParty = function(){
        if(startThirdParty.done) return;
        startThirdParty.done = true;
        interactionEvents.forEach((e) => window.removeEventListener(e, startThirdParty));
        thirdParty.initTagManager();
        jQuery.getScript("https://www.google.com/recaptcha/api.js?render=6LdotL0ZAAAAALDh_JBTO_JC-CF4ZnSysamIHV3c").
            then(()=>{
                self.server.sendRecaptcha("initial_page_load");
            })
    };
    const interactionEvents = ["pointerdown","keydown","touchstart","scroll"];
    const afterLoad = function(){
        interactionEvents.forEach((e) =>
            window.addEventListener(e, startThirdParty, {once:true, passive:true}));
        setTimeout(startThirdParty, 15000);
        pwa.registerServiceWorker(self);
        initAutoHideNav();
    };
    if(document.readyState === "complete")
        afterLoad();
    else
        window.addEventListener("load", afterLoad);

    this.setSubscriptionStatus(this.viewModel.isSubscribed);
    //reCAPTCHA now loads with GTM in startThirdParty above (interaction or 15s)
}


} // end Client class


export default {Client};
