...
initViewModel(authBaseUrl,clientId,redirect,appBase,appState){
   ...
   /*
    viewModel.sendWelcomeNotice = async function(){
        self.sendNotice("Welcome to Ergatas!",{},"https://ergatasstorage.blob.core.windows.net/dev-public-content/765f2e8e-3700-4080-9247-fb8b6aa3d73b/headshot4.jpg");
    };
    viewModel.noticeTime = ko.observable();
    viewModel.setNoticeTime = async function(){
      self.initPrayerNotifications(viewModel.noticeTime());
    }
    */

}
/*
async sendNotice(body,data,image){
    return navigator.serviceWorker.getRegistration().then(function(reg) {
        console.log("image: ",image);
            reg.showNotification("Ergatas Notice" ,{
...
async initPrayerNotifications(time="08:50"){
    var self=this;

    if(Notification.permission!=="granted")
        return;

    console.info("Permission to notify, sending prayer notification at "+time);

    utils.scheduleFunction(time,async function(){
        console.log("sending notification");
        var profile = await self.da.randomSharableProfile();
        //console.log("profile: ",profile);
        self.sendNotice("Pray for "+profile.missionary_name,
                        {
                            missionary_profile_key: profile.missionary_profile_key,

                        },
                        self.storage.profilePictureUrl(profile.data.picture_url)).
            then(() => dataLayer.push({event:'user','user-action':"prayer-notification"}));
...
initRouter(appBase,authBaseUrl,authClientID){
    ...
    router.add(/^search\/saved\/([-a-zA-Z0-9"' ]+)$/, async function (searchName) {
        ...
        if(viewModel.loggedIn()){
            try{
                ...
                alertify.error("Well this is embarisisng, but I could not find a search saved under the name '"+searchName+"'");
            }
            setSearch();
    ...
    router.add('message-moderation/', function () {
        setLoggedInPage('message-moderation');
    });
    ...
    router.add('daily-prayer/', function (name) {
        setPage("daily-prayer");
    });
    router.add('saved-searches/', function (name) {
        setLoggedInPage("saved-searches");
    });
    router.add('guided-search/', function (name) {
        setLoggedInPage("guided-search");
    });
