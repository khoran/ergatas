    ...
    viewModel.setupNotifications = function(){
        Notification.requestPermission().then((result) => {
            ...
            if (result === 'granted') {
                ...
                self.sendNotice("Congrats, notifications are working!",{});
...
async initPrayerNotifications(){
    var self=this;

    if(Notification.permission!=="granted")
        return;

    console.info("Permission to notify, starting prayer notification interval");

    utils.scheduleFunction("08:50",async function(){
        ...
        self.sendNotice("Pray for "+profile.missionary_name,
            ...
            then(() => dataLayer.push({event:'user','user-action':"prayer-notification","user_id":viewModel.userId()}));
