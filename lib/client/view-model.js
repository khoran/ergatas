...
export function createViewModel(client, {authBaseUrl, clientId, redirect, appBase, appState}){
    ...
    const viewModel = {
        ...
        roles: ko.observable(),
        page: ko.observable(),
        // name of the page currently displayed (set by the router); lets the
        // navbar hide its search box on the home page, which has its own
        currentPageName: ko.observable(window.location.pathname === "/" ? "home" : ""),
        ...
        navDrawerOpen: ko.observable(false),
        toggleNavDrawer: function(){ viewModel.navDrawerOpen(!viewModel.navDrawerOpen()); },
        pwaSupported: ko.observable(false),
        versionMessage: ko.observable(utils.unsupportedBrowserCheck()),
        pageStats: ko.observable(),
        updateApp: function(){},

        //Bible-translation toggle for the verse landing pages (/learn/matthew-28 etc).
        //Initialized from the ?t= query param so ad traffic for e.g. "matthew 28 kjv" lands
        //with KJV pre-selected. All translations are server-rendered as static HTML; this
        //only toggles which one is visible (KO `visible`), so nothing is lost for crawlers.
        selectedTranslation: ko.observable((utils.getURLParameter("t") || "niv").toLowerCase()),
        ...
        //"Create your free profile" CTAs on missionary-facing pages. Fires the
        //missionary_signup_start conversion on click (before auth), then routes to the
        //profile editor. 'source' identifies which CTA/page the click came from.
        signupStart: function(source){
            return function(){
                dataLayer.push({event:"missionary_signup_start",source:source,user_id: viewModel.userId()});
                self.router.navigateTo("profile");
            }
        },
        //switch the visible Bible translation on the verse landing pages. Curried like
        //navigateFn so templates bind `click: setTranslation('kjv')`.
    ...
    // mobile nav drawer: lock body scroll while open, close on Escape
    viewModel.navDrawerOpen.subscribe(open => {
        jQuery("body").toggleClass("nav-drawer-lock", open);
    });
    jQuery(document).on("keyup", e => {
        if(e.key === "Escape")
            viewModel.navDrawerOpen(false);
    });
