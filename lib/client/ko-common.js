...
import {loadShareThis} from './sharethis-loader';
...
ko.bindingHandlers.selectize = {
  init: async function(element, valueAccessor, allBindingsAccessor, viewModel, bindingContext) {
      ...
      const {default: selectize }  = await import(/* webpackChunkName: "selectize" */ './selectize-chunk');
      ...
      // selectize replaces the original <select> with its own text input,
      // which has no accessible name (fails the WCAG 'label' audit). Carry
      // one over from the original element's labelling.
      var labelledBy = element.getAttribute('aria-labelledby');
      var accessibleName = element.getAttribute('aria-label')
          || (labelledBy && jQuery('#'+labelledBy).text().trim())
          || (element.id && jQuery('label[for="'+element.id+'"]').text().trim());
      if(accessibleName)
          jQuery(element).next('.selectize-control').find('input').attr('aria-label', accessibleName);

      // Because the import above is async, init finishes AFTER knockout's first
      // update() call has already run and bailed (loaded was still false). Any
      // value the bound observable held before the control loaded (a default set
      // by the component, or existing data when editing) would therefore never
      // reach the control, since update() only pushes on a later change. Apply
      // the current value now that the control exists.
      var control = jQuery(element)[0].selectize;
      if(control != null && selectizeOptions.onObsUpdate != null && ko.isObservable(value)){
        jQuery.data(element,"selectizeUpdating",true);
 ...
 ko.bindingHandlers.editor= {
  init: async function(element, valueAccessor, allBindingsAccessor, viewModel, bindingContext) {
      ...
      const {default: trumbowyg}  = await import(/* webpackChunkName: "trumbowyg" */ './editor-chunk');
 ...
 ko.bindingHandlers.urlSuffix= {
  init: function(element, valueAccessor, allBindingsAccessor, viewModel, bindingContext) {
      ...
      var value = valueAccessor();
      var allBindings = allBindingsAccessor();
      var urlBases = ko.unwrap(allBindings.urlBase) ;
      var valueRootHostname;
      var donationURL;
      var rootPattern = /^.*?([a-zA-Z0-9-]+\.[a-zA-Z0-9-]+)$/;
      //console.log("value: ",value(), urlBases);


      try{
        if( ! Array.isArray(urlBases)){
          console.error("donation_urls is not an array, cannot validate");
          return;
        }
      }catch(error){
      ...
      try{ //loop over entries in urlBases until we find a match or run out of options
        var matchFound = false;
        for(const urlBase of urlBases){

          const websiteURL = new URL(urlBase.url);
          const rootHostname= websiteURL.hostname.replace(rootPattern,"$1");
          

          if(urlBase.match_type === "domain")
            // this will accept a complete URL, but allow additional sub-domains to be specified
            urlBase.pattern = `(?:https?:\/\/(?:[a-zA-Z0-9\\-]+\\.)*${rootHostname}(\\/.*)?)`;
          else if(urlBase.match_type === "full_domain")
            urlBase.pattern = `(?:https?:\/\/${websiteURL.host}(\\/.*)?)`;
          else if(urlBase.match_type === "full_url")
            urlBase.pattern = `(?:https?:\/\/${websiteURL.host}${websiteURL.pathname}(\\?.*)?)`;
          else{
            console.error("Unknown match type: "+urlBase.match_type+", url: "+urlBase.url);
            urlBase.pattern='';
          }
          ...
          if(value() != null && value() !== ""){
            ...
            //console.log("donationURL: "+donationURL+", root: "+valueRootHostname);

            // see if current value starts with urlBase as it should
            if(rootHostname === valueRootHostname ) {
              $element.val(donationURL.href);
              matchFound = true;
              //console.log("found matching URL for ",urlBase);
            }
          }

        }
        if( ! matchFound)
          console.warn(`root hostnames do not match between website and donation URL. donation url: ${value()}, URLs: `,urlBases);


      }catch(error){
        console.error("failed to process given donation url: "+error);
        //not a fatal error though, so just keep going
      }


      const combinedPattern=`^\\s*(`+urlBases.map(b=>b.pattern).join("|")+`)$`;
      //console.log("combinedPattern: ",combinedPattern);
      $element.attr("pattern",combinedPattern);
...
ko.bindingHandlers.initShareButtons= {
  init: async function(element, valueAccessor, allBindingsAccessor, viewModel, bindingContext) {
      const $element = jQuery(element);
      try {
        // Load ShareThis on demand (no longer a global script on every page).
        const shareThis = await loadShareThis();
        shareThis.load('inline-share-buttons',{
          enabled: true,
          container: element,
          //alignment: 'left',
          //font_size: 14,
        ...
        console.warn('ShareThis failed to load', e);
      }
  }
}
...
ko.bindingHandlers.impactMap= {
  init: function(element, valueAccessor, allBindingsAccessor, viewModel, bindingContext) {
      ...
      async function renderMap(){
        try {
          await import(/* webpackChunkName: "impact-map-chunk" */ './impact-map-chunk');
        } catch(e) {
      ...
      // The map's chunk (~75KB of d3/datamaps) and its SVG render aren't needed
      // for first paint. Deferring to idle time keeps this off the critical
      // rendering path so it doesn't inflate Total Blocking Time.
      if('requestIdleCallback' in window)
        window.requestIdleCallback(renderMap, {timeout: 2000});
      else
  ...
  init: function(element, valueAccessor, allBindingsAccessor, viewModel, bindingContext) {
    if(window.ResizeObserver != null){
      const resizeObserver = new window.ResizeObserver(() => {
        const applyClamp = jQuery.data(element, 'clamped-apply');
        if(typeof applyClamp === 'function')
          applyClamp();
      });
    ...
    ko.bindingHandlers.clamped.update(element, valueAccessor);
  },
  update: function(element, valueAccessor) {
    var config = ko.unwrap(valueAccessor());
    var overflowObs, expanded = false;

    if(config != null && typeof config === 'object' && !Array.isArray(config)){
      ko.unwrap(config.value); // subscribe to content changes
      overflowObs = config.overflow;
      expanded = ko.unwrap(config.expanded) === true;
    }

    const applyClamp = function(){
      var clampLines = 15;
      var maxHeight = utils.getMaxHeight(clampLines, element);
      var sty = element.style;

      // Clear height to measure natural content height
      sty.height = "";
      var isOverflowing = element.scrollHeight > (maxHeight + 2);

      if(ko.isObservable(overflowObs))
        overflowObs(isOverflowing);

      if(expanded || !isOverflowing){
        sty.height = "";
      }else{
        sty.height = String(clampLines * 1.2) + "em";
      }
    };
    ...
    const scheduleClamp = function(){
      if(window.requestAnimationFrame != null)
        ...
        window.setTimeout(applyClamp, 0);
    };

    jQuery.data(element, 'clamped-apply', scheduleClamp);
    scheduleClamp();
  }
}
ko.bindingHandlers.normalizeVideoURL= {
  init: function(element, valueAccessor, allBindingsAccessor, viewModel, bindingContext) {
    var $element = jQuery(element);
    var update = function(){
...
ko.bindingHandlers.supportSlider= {
  init: async function(element, valueAccessor, allBindingsAccessor, viewModel, bindingContext) {
    ...
    // like the selectize binding above, load the library on first use;
    // update() guards on element.noUiSlider until init finishes
    const {default: slider} = await import(/* webpackChunkName: "nouislider" */ './nouislider-chunk');
  ...
  update: function(element, valueAccessor, allBindingsAccessor,viewModel, bindingContext) {
    ...
    if(!Array.isArray(observables) && observables.length >= 2){
      ...
      if(element.noUiSlider == null) //init's dynamic import hasn't finished; init sets initial values itself
        return;
...
ko.bindingHandlers.embedVideo = {
  init: function(element, valueAccessor, allBindingsAccessor, viewModel, bindingContext) {
    console.log("init of embedded video");
    var iframe = document.createElement('iframe');
    iframe.src = ko.unwrap(valueAccessor());
...
ko.bindingHandlers.favorite = {
  init: function(element, valueAccessor, allBindingsAccessor, viewModel, bindingContext) {
    ...
    var isFavoriteObs = config.isFavorite;
    var toggleFavoriteFn = config.onUpdate;

    //console.log("favorite init. isFavoriteObs: ",isFavoriteObs);

    $element.toggleClass('favorite-heart', true);
    // Create the heart icon element
    var heartIcon = document.createElement('i');
    heartIcon.className = 'favorite-heart-icon';
    ...
    ko.computed(function() {
      var isFavorite = ko.unwrap(isFavoriteObs());
      heartIcon.className = 'favorite-heart-icon ' + (isFavorite ? 'fas fa-heart ' : 'far fa-heart');
      element.className = 'favorite-heart ' + (isFavorite ? 'favorite-selected' : 'favorite-not-selected');
      element.title = isFavorite ? '' : 'Add to Favorites';
    });
  },
  update: function(element, valueAccessor, allBindingsAccessor, viewModel, bindingContext) {
    // The computed in init handles updates
  }
};
